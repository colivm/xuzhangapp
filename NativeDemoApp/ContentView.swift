...
struct ContentView: View {
    ...
    private var topBar: some View {
        ...
        .background(
            Rectangle()
                .fill(AppColors.panel.opacity(0.82))
                .overlay(alignment: .bottom) {
                    Rectangle()
                        .fill(AppColors.line.opacity(0.86))
                        .frame(height: 1)
                }
        )
    }
    ...
    private var tabBar: some View {
        ...
        .background(
            Rectangle()
                .fill(AppColors.panel.opacity(0.86))
    ...
    private func tabIcon(for tab: AppTab, isSelected: Bool) -> some View {
        ZStack {
            ...
            Circle()
                .fill(isSelected
                    ? AppColors.accent.opacity(0.34).mix(with: Color(red: 1, green: 0.91, blue: 0.64), by: 0.66)
                    : AppColors.subtext.opacity(0.18).mix(with: Color(red: 1, green: 0.94, blue: 0.76), by: 0.82)
                )
    ...
    private func categoryChip(category: HomeItem.Category, isRecommended: Bool) -> some View {
        ...
        return Button {
                    ...
                    Text("推荐")
                        ...
                        .background(
                            Capsule(style: .continuous)
                                .fill(recordAccent.opacity(0.35))
                        )
                }
            }
            .foregroundStyle(recordInk)
            ...
            .overlay(
                Capsule(style: .continuous)
                    .stroke(
                        isSelected ? recordAccent.opacity(0.45) : AppColors.line.opacity(0.9),
                        lineWidth: 1
                    )
            )
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    ForEach(homeViewModel.noteSuggestions(for: homeViewModel.selectedCategory), id: \.self) { suggestion in
                        ...
                        .background(
                            Capsule(style: .continuous)
                                .fill(Color(red: 0.84, green: 0.90, blue: 0.99).opacity(0.6))
                        )
                        .overlay(
                            Capsule(style: .continuous)
    ...
    private var saveRow: some View {
        ZStack(alignment: .topTrailing) {
            Button {
                ...
                .overlay(
                    Capsule(style: .continuous)
                        .stroke(hasValidAmount ? Color.white.opacity(0.36) : Color.white.opacity(0.55), lineWidth: 1)
                )
...
struct InsightWebView: View {
        ...
        ScrollView {
            VStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    if homeViewModel.insightsLast7Days.isEmpty {
                        Text("近 7 天暂无复盘。多记几笔，就能看到更完整的消费节奏啦。")
                            .font(.system(size: 13))
                            .foregroundStyle(AppColors.subtext)
                    } else {
                        ForEach(homeViewModel.insightsLast7Days) { insight in
                            VStack(alignment: .leading, spacing: 4) {
                                Text(insight.dayKey)
                                    .font(.system(size: 11))
                                    .foregroundStyle(AppColors.subtext)
    ...
    private func generateAndShareWeeklyCard() {
        ...
        let nick = settingsViewModel.displayName.isEmpty ? "序帐用户" : settingsViewModel.displayName
...
struct WeeklyShareCardView: View {
    ...
    var nickname: String = "序帐用户"

    private var t: ShareCardTheme { isPetMode ? .pet : .neutral }
    private let cardSize = CGSize(width: 390, height: 580)
    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                // Charts block. The snapshot is only 390pt wide, so fixed side-by-side
                // charts overflow easily; stack them and let each fill the content width.
                VStack(alignment: .leading, spacing: 10) {
                    trendChart
                    ringChart
                }
    ...
    private var trendChart: some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            ...
            GeometryReader { geo in
                let available = max(geo.size.width - 16, 1)
                let count = max(dailyTrend.count, 1)
                let barW = max(8, min(18, (available - gap * CGFloat(count - 1)) / CGFloat(count)))

                HStack(alignment: .bottom, spacing: gap) {
                    ForEach(Array(dailyTrend.enumerated()), id: \.offset) { _, pt in
                            ...
                            if pt.1 > 0 && pt.1 == peak {
                                RoundedRectangle(cornerRadius: 4)
                                    ...
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 4)
                                            .fill(Color.white.opacity(0.4))
                                            .frame(width: max(3, barW - 6), height: 3)
                                            .offset(y: -(h - 6) / 2)
                                    )
                            } else {
                                RoundedRectangle(cornerRadius: 4)
    ...
    private func legend(_ c: Color, _ t: String) -> some View {
        HStack(spacing: 3) {
            ...
            Text(t)
                .font(.system(size: 8))
                .foregroundStyle(self.t.textMuted)
                .lineLimit(1)
