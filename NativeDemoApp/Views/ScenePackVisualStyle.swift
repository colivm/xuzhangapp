...
struct ScenePackVisualBackdrop: View {
    ...
    var body: some View {
        ZStack {
            LinearGradient(
                colors: [
                    AppColors.surfaceMuted.opacity(isSubtle ? 0.24 : 0.40),
                    primaryColor.opacity(isSubtle ? 0.18 : 0.54),
                    secondaryColor.opacity(isSubtle ? 0.14 : 0.48),
                    themeColor.opacity(isSubtle ? 0.08 : 0.22)
                ],
            ...
            RadialGradient(
                colors: [
                    Color.white.opacity(isSubtle ? 0.14 : 0.22),
                    Color.clear
                ],
                center: .topLeading,
                startRadius: 0,
            ...
            LinearGradient(
                colors: [
                    Color.white.opacity(isSubtle ? 0.08 : 0.13),
                    Color.clear,
                    themeColor.opacity(isSubtle ? 0.04 : 0.08)
                ],
                startPoint: .top,
                endPoint: .bottom
            )
            ...
            LinearGradient(
                colors: [
                    Color.clear,
                    Color.white.opacity(isSubtle ? 0.05 : 0.08),
                    Color.clear
                ],
                startPoint: .top,
                endPoint: .bottom
        ...
        .overlay(
            RoundedRectangle(cornerRadius: compact ? 15 : 16, style: .continuous)
                .stroke(Color.white.opacity(isSubtle ? 0.10 : 0.14), lineWidth: 1)
        )
    ...
    private var sceneLineTexture: some View {
        GeometryReader { proxy in
            Path { path in
                let size = proxy.size
                path.move(to: CGPoint(x: size.width * 0.08, y: size.height * 0.68))
                path.addCurve(
            ...
            .stroke(
                Color.white.opacity(isSubtle ? 0.07 : 0.11),
                style: StrokeStyle(lineWidth: compact ? 1 : 1.3, lineCap: .round)
            )

            ForEach(0..<3, id: \.self) { index in
                Circle()
                    .fill(Color.white.opacity(isSubtle ? 0.08 : 0.12))
    ...
    private func texturePoint(_ index: Int, in size: CGSize) -> CGPoint {
        switch index {
        case 0:
            return CGPoint(x: size.width * 0.20, y: size.height * 0.46)
        case 1:
            return CGPoint(x: size.width * 0.52, y: size.height * 0.58)
        default:
            return CGPoint(x: size.width * 0.78, y: size.height * 0.38)
        }
    }
    ...
    private func symbolOpacity(_ index: Int) -> Double {
        if isSubtle {
            return index == 0 ? 0.09 : 0.05
        }
        return index == 0 ? 0.20 : 0.11
    }
}
