...
enum LifeSceneSemanticService {
    ...
    private static func signals(
        ...
        case "metro_transit":
            if item.scenePackId == "commute" || containsStrongCommuteCue(explicitText) {
        ...
        if containsAny(text, ["午餐", "午饭", "晚餐", "晚饭", "外卖", "简餐", "热饭", "吃一口", "饭点", "夜宵", "馄饨", "盖饭", "肠粉", "黄焖鸡", "冒菜", "生煎", "锅贴", "七欣天", "海底捞", "老乡鸡", "塔斯汀", "袁记云饺", "萨莉亚", "火锅", "烤肉", "烤鸭", "烧鸭", "卤鸭", "鸭肉", "麻辣烫", "披萨", "炸鸡", "汉堡", "便当", "牛肉面", "兰州牛肉面", "兰州拉面", "拉面", "汤面", "面馆", "面食", "面条", "粉面"]) {
        ...
        if !containsTelecomBillCue(text),
           containsAny(text, ["纸巾", "洗衣液", "洗洁精", "清洁", "垃圾袋", "洗发水", "沐浴露", "牙刷", "毛巾", "美团闪购", "给家补货"]) {
        ...
        if !containsTelecomBillCue(text),
           (containsAny(text, ["淘宝", "京东", "拼多多", "购物", "下单", "快递", "衣服", "外套", "裤子", "连衣裙", "半身裙", "短裙", "长裙", "裙装", "护肤", "充电器", "数据线", "充电宝", "耳机", "手机", "文具", "渔具", "鱼竿", "路亚", "露营", "骑行", "摄影", "相机", "镜头", "模型", "手办", "谷子", "潮玩", "吧唧", "徽章", "亚克力", "立牌", "盲盒", "泡泡玛特", "POP MART", "POPMART", "LABUBU", "棉花娃娃", "痛包", "同人本", "乙游周边", "漫展周边", "乐器", "茶具", "咖啡器具"]) || SemanticBoundaryGuard.matchesFlowerShopping(text)) {
    ...
    private static func containsStrongCommuteCue(_ text: String) -> Bool {
        // Legal-entity suffixes describe the payment counterparty, not where
        // the user went.  Strip them before evaluating the broad “公司” cue;
        // contextual titles such as “公司楼下/公司食堂” remain intact.
        let normalized = text
            .replacingOccurrences(of: " ", with: "")
            .replacingOccurrences(of: "　", with: "")
            .replacingOccurrences(of: "有限责任公司", with: "")
            .replacingOccurrences(of: "股份有限公司", with: "")
            .replacingOccurrences(of: "有限公司", with: "")
            .replacingOccurrences(of: "个体工商户", with: "")
        return containsAny(normalized, strongCommuteCues)
    }
