...
enum ScenePackCopyPool {
    static let definitions: [ScenePackDefinition] = [
        ...
        ScenePackDefinition(
            ...
            tiers: [
                ...
                ScenePackTier(maxAmount: 60, notes: ["药店买药", "挂号问诊的一次记录", "换季护理用品补上", "牙膏牙线和口腔护理", "身体小安排记下", "身体提醒先记下", "基础护理用品", "小病小痛记下"]),
        ...
        ScenePackDefinition(
            ...
            tiers: [
                ...
                ScenePackTier(maxAmount: 100, notes: ["健身装备小添置", "瑜伽或球类场地费", "运动场地或课程一回", "买了训练小工具", "今天的运动安排", "给锻炼添点装备", "一场运动记下来", "运动习惯继续保持"]),
    ...
    private static func contextualNotes(
        ...
        if pack.id == "commute" || category == .transport {
            ...
            case 7..<10:
                if shouldUseWorkdayCopy(date: date, historyItems: historyItems) {
                    return ("morningCommute", ["早班准时出门", "早上路上这一程", "早高峰顺利到达", "清晨出门的一笔路费", "赶早路上的交通记录", "地铁公交到站", "早上这趟路走完了", "今天也准时出门"])
                }
                return ("weekendMorningRoute", ["早上出门的一段路", "清晨出门的一笔路费", "早上这趟路走完了", "今天也准时出门", "短途出行记一下", "公交地铁一段路", "这一程顺利到达", "早上的路费"])
            case 17..<21:
                if shouldUseWorkdayCopy(date: date, historyItems: historyItems) {
