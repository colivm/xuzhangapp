...
- 2026-09-09 Archive 二次编译回补：多语句函数 `hasTodayCommuteRecord` 与 `sceneAggregates` 均补齐显式 `return`，修复 Swift 的 “Missing return” 诊断；不改变通勤或生活线索计算逻辑。
...
- 状态：`NOT_STARTED` → `IN_PROGRESS` → `CODE_DONE`（2026-09-09）；本项只处理设置页陪伴入口的命名、说明和可见选项，不改变天气服务或文案生成规则。
- 用户问题：设置页“陪伴语气”入口实际包含宠物和天气互动；“温和/中性”两档缺少可感知反馈，入口价值不清晰，而天气互动是明确有用的能力。
- 目标：将入口统一命名为“陪伴设置”，突出宠物陪伴与天气互动；移除不可感知的语气选择，避免向用户承诺难以验证的控制；保留底层 `aiTone` 字段和解码兼容，历史设置不丢失。
- 允许范围：`NativeDemoApp/Views/SettingsView.swift` 的入口标题、摘要、陪伴 Sheet 文案和可见控件；必要的设置页回归与文案静态检查。不得修改 `AppSettings` 编码字段、天气服务、AI 生成规则、宠物触发规则、会员、同步或其他设置页。
- 冻结边界：天气互动开关、宠物开关、宠物昵称保存语义保持不变；不删除 `aiTone` 模型字段，不迁移或重置已有用户配置；不把“内容风格”伪装成已验证的实时预览能力。
- 实施结果：设置入口及 Sheet 标题统一为“陪伴设置”，图标改为爪印；摘要改为“宠物开 · 天气互动”或“宠物开 · 基础陪伴”。保留宠物开关、昵称和天气互动，并补充天气互动的实际作用说明；移除“复盘语气/温和/中性”可见控件及无调用的展示组件，避免用户误以为有即时可感知的语气控制。`AppSettings.aiTone`、编码键、默认值和 AI 服务传参保持不变，既有用户配置继续可解码。
- 修改文件：`NativeDemoApp/Views/SettingsView.swift`、本文档。
...
- 剩余风险与下一步：需在 macOS/Xcode 完成 Swift 6 Debug/Release 编译，并在真机核对陪伴入口、天气开关、深色模式、VoiceOver 和特大字号；同时确认历史 `aiTone` 配置不会触发迁移异常。外部签收前保持 `CODE_DONE`，下一步按 `FLOW-111` 集中验收。

### 109. APP-DISPLAY-NAME-CONSISTENCY-01：安装包名称与权限提示统一为叙账（2026-09-09）

- 状态：`NOT_STARTED` → `IN_PROGRESS` → `CODE_DONE`（2026-09-09）。
- 用户问题：App Store Connect 项目名称显示“叙账”，但 TestFlight 安装后的桌面名称为 `xLife`，权限提示也使用旧品牌名。
...
### 113. SITE-VISUAL-POLISH-01：官网视觉精致化（2026-09-10）

- 状态：`NOT_STARTED` → `IN_PROGRESS` → `CODE_DONE`（2026-09-10）。
- 用户问题：官网还可以更精致；米色铺得太满；水彩底图被放大到看不见主题；顶栏要 iOS 玻璃且 logo 有呼吸；玻璃下面那条占位底色在下翻时应消失，玻璃本身保留。
- 实施结果：首屏改为去字标水彩底加三台真机；页面回到默认手账冷灰加淡米色光与呼吸青绿；顶栏改为悬浮玻璃并固定叠在内容上，不再占用一条底色带；logo 图标缓慢呼吸。功能卡改为序号/本机/可选，去掉 emoji。
...
### 159. INPUT-LATENCY-READONLY-AUDIT-01：金额、复盘文字与昵称输入延迟排查（2026-09-17）

- 性质与范围：用户要求分析卡顿及其与近期修复的关系，未授权实现。本轮只读检查三个输入入口、共享状态、预览计算与近期 `5516a67` 差异，仅追加本条；无产品/测试/配置修改、无提交推送，不启动 roadmap 任务，不重复执行全量门禁。
- 金额既有负担：RecordView:1914 每次金额变化直接刷新推荐，只有备注输入走 220ms 防抖；inputAmount 位于共享 HomeViewModel 的 @Published，根视图和记录页均订阅对象。prepareRecordPrefillSnapshot:2692 在启动后台计算前，仍在主线程校验备注、读取并 JSON 解码 AppSettings、执行 provisional 分类；有具体备注时 learnedCategory 会扫描历史纠正记录并计算相似度/排序，空备注会提前退出这项扫描。确认级预览的生活印记任务也以 amount 为 key，并创建 items＋draft 数组，金额变化可能重新触发既有聚合；不能把这些都说成近期新增，也不能把后台计算等同于逐键无开销。
- 近期增加的金额开销：新 emotionSceneContext（RecordView:784）是计算属性，获取时调用预览解析和保存规范化解析，还计算情绪/天气上下文；它被 task ID、onChange、切换能力、选择有效性和异步前后校验反复读取，未见共享缓存。Context 包含金额，变化可重启最多 24 次候选扫描；扫描在 utility 子任务，不是直接在主线程循环，但候选循环内部没有取消检查，已开始的过期计算仍可能跑完。无备注且无有效预填的中性草稿会提前返回 nil，因此不能断言每次纯金额输入都会扫描候选。
- 近期分类修复边界：inputTitle/inputAmount/selectedDate 新增 didSet 立即失效旧推荐，可能增加清空/重新发布的状态切换，但这是防止陈旧推荐反写的必要保护；统一推荐决策同时删掉了旧的第二套分类计算，不能把全部修复描述为单向增加负担。正确方向是将失效标记与昂贵重算分开，而不是撤销失效、分类锁或保存校验。
- 复盘与昵称事实：InsightWebView:688 的独立输入组件只绑定文字，查询在运行按钮后才执行，未发现逐字联网、存储或全账本扫描；SettingsView:2944 昵称使用 draftDisplayName，本来就在提交/失焦后才校验、持久化及同步。不能把父级 @State 简单等同于整页必定每键重算。昵称完成按钮先 commitDisplayName 再清焦点，失焦 observer 再调用同一方法，且 SettingsViewModel.updateDisplayName 无相同值短路，存在结束编辑时重复提交的可达路径，但不能据此解释连续打字掉帧。
- 共同风险及排除：共享 ObservableObject 发布可能使已订阅页面刷新，实际 body/布局范围需测量；复盘多行 TextField 的行高变化、键盘合成、背景气泡 8Hz Timeline/阴影在 Sheet 下是否继续更新是待测点。ContentView 只挂当前 Tab，不声称五页同时渲染。主题新代码只按明暗/主题配置变化同步且同 id/mode 不发布，未发现键入字符就重新套主题的调用；近期提交未改 SettingsView/InsightWebView 输入本身。
- 优化方向（未实施）：优先让金额显示立即响应，昂贵推荐/候选按短暂输入停顿合并；草稿 key 只由廉价原始字段构成，一次计算统一预览/保存规范化/情绪结果并按完整身份缓存；将历史纠正索引、设置解码移出逐键同步路径；候选和旧后台任务增加合作式取消、合并发布且减少 nil→结果抖动。保留“新输入立即使旧选择/旧异步结果失效、马上保存仍按当前草稿解析”、分类锁、场景权益和生活印记规则。昵称先做幂等提交，复盘保持已存在的独立输入与单一文本来源，不能重新引入双份草稿导致外部预设/清空不同步。
- 证据限制与下一步：仅源码/提交差异及一次独立文本输入审计，无 Swift/Xcode/iPhone，未测量按键到显示的延迟、body 次数或主线程堆栈，因此确认的是具体负担和回退风险，不是三个入口均已运行复现。后续在同一设备、同一 Release 配置对比 `a7802ff` 与当前修复版本，覆盖冷/热输入、中文拼音/英文、自定义金额键盘、带/不带备注或推荐、100/1,000/5,000 条、完成昵称与连续输入，使用 SwiftUI/Time Profiler/Main Thread Hitches 核对后分小任务修复；生产和测试两套配置检查必须各自完整保留，不改规则。

### 160. QUICK-AMOUNT-NOTE-READONLY-AUDIT-01：快捷金额与快捷备注生成、点击语义排查（2026-09-17）

- 用户验收反馈：用户明确表示“刚刚修复的几个问题我真机验证了，修复好了”，登记为第 153、154、157 节相关问题的用户真机成功反馈；未提供逐项设备矩阵和 XCTest 结果，不扩写为所有测试均已执行。本轮另行只读审计快捷金额、备注短句按钮及帮写区别，不改产品代码、不提交推送、不变更 roadmap 状态。
- 快捷金额来源（HomeViewModel:419，RecordView:2283/2299）：仅金额文本为空时显示“这个时段常记”，最多 3 个。按所选记录日期向前 180 天筛正金额且不晚于该日期的账目，至少 6 笔；同 3 小时时段（小时/3）和同工作日/周末/节假日类型至少 3 笔。金额按分分组，优先主要生活场景、否则统计分类；最终支持数至少 2、占比至少 0.75、金额不超过 9999，按支持数再按最近时间排序。统计的是记录次数，不要求跨多个日期；不是固定 10/20/50，也不是联网 AI。
- 快捷金额点击：写入金额，并尝试自动应用候选分类，不设用户分类锁；已有显式分类锁会阻止自动改类。随后仍走统一预填解析，不直接保存或强行填备注；明确备注、品牌和既有用户锁保留其解析优先级。键盘的 .00/+10/+50 另是固定格式化/加法按钮，没有学习生成逻辑。
- 随金额带出的历史备注（HomeViewModel:468）：须同 180 天、时段、日期类型、分类和精确金额，至少 2 笔支持。排除空白/默认/系统通用标题、纯金额/脏日期及长度不在 2–12 的标题；历史品牌或用户编辑过且语义兼容的标题才可复用。手写权重 2、其他 1，最高分至少 2，因此并不要求同一句在两笔记录中重复；可信标题进入已采纳预填，再由分类兼容与保存规则检查。
- 快捷备注短句按钮（HomeViewModel:2986，RecordView:2833）：当前分类固定 3 句；餐饮随所选日期时段及日类型分支，娱乐/住宿部分看日类型，健康看当前备注是否有运动线索。有与当前金额/类别相符的已采纳预填标题时插到最前去重，通常共 3–4 句。不是全账本常用备注列表；除置顶预填外，不按金额档、品牌、已有具体食物或选定场景包筛整组默认短句。
- 与帮写区别：帮我写一句/换一句走 RecordView:1211/1322/1389，优先已选场景和备注锚点，否则按当前类别/时段/部分金额档轮换，最多试 8 个避免与当前标题相同，并登记 applyGeneratedRecordTitle 来源。短句按钮仍设 lastDraftIntent=.category、记录备注锚点后直接赋 inputTitle，不登记上述来源，也不设置 categoryLockedByUser；5516a67 没有改短句生成或点击实现。不能把已修复普通帮写的来源保护宣称覆盖所有快捷备注入口。
- 已确认独立边界：金额和历史标题统计未排除 draftMeta 非空的 OCR 待整理记录，候选可能受未整理数据影响；没有跨日频次要求，集中补录可形成“常记”。默认短句含早班、加班、热食、出差等具体事实，属于供主动选择的模板，不应被描述成已从账本推断的事实；日历节假日规则并非完整调休表。短句点击后的 clearActiveScenePackIfManualNoteMovedAway 只接受 .note 意图，当前 handler 先设 .category，故此调用不保证释放冲突场景包；显式分类锁本身没有被此按钮取消。
- 优化方向（未实施）：先明确“已入账历史”学习范围与跨日要求，区分预填历史备注/系统模板/真实手写来源；用与保存一致的解析结果筛选分类兼容短句，避免系统短句自我改类；默认文案优先中性且尊重已有食物/品牌，场景包冲突必须有明确意图规则，不通过清用户锁解决。保留旧异步失效、推荐阈值、会员/场景包权益和不覆盖真实备注边界。
- 确定反例补充：日用短句“便利店补一袋日常”在当前 JSON 词典仅命中餐饮“便利店”。未锁分类时按快捷短句当前直接赋值路径，推荐/保存可改为餐饮；已显式锁日用时类别保留，但该短句不算真实手写，保存可按不兼容标题规则替换为日用兜底句。应用 supply/family 后点此句，前述场景清理守卫不通过，active 包仍可能保留。此为静态可达的既有快捷按钮问题，与用户已验证成功的普通帮写入口分开，不宣称已真机复现。
- 验证范围与下一步：源码与近期提交差异交叉审阅，无 Swift/XCTest/真机新增执行，未重复运行全量门禁；仅台账文档 diff 检查。后续如授权实现，分别覆盖新用户/门槛不足、同额多分类、同日批量/跨日、OCR 待整理、改日期/时段、用户锁、快捷短句→帮写→保存与显式场景包；测试和生产两套配置门禁保持完整且不改规则。

### 161. QUICK-NOTE-SCOPED-FIX-01：快捷备注候选、模板与点击来源收口（2026-09-17）

- 状态：`IN_PROGRESS` → `CODE_DONE`（2026-09-17），当前无进行中的实现任务，未标记 VERIFIED。承接主代理前序完整台账阅读及第 158–160 节续读；用户本轮明确授权只优化快捷备注，历史常用备注候选须先评估性能，不扩大范围。
- 允许范围：`HomeViewModel.swift` 快捷备注模板/候选和其历史快照缓存、`RecordView.swift` 快捷备注显示/点击及本入口的场景一致性、对应 XCTest 与专项静态检查；只为快捷备注添加内存候选，不新增持久化字段。
- 冻结边界：不改快捷金额、原精确金额历史预填、普通帮写/换一句算法、手写语义优先级、显式分类锁、换角度/场景包访问权限、情绪/生活印记/奖励、OCR 导入、同步/存储、会员、输入框通用性能、主题及测试/生产配置或现有门禁规则。
- 计划与性能约束：复用按账本修订/记录日期更新的后台历史快照，跨日重复且已入账、语义兼容的备注可进入有限候选池；不在每次按键或 SwiftUI body 内扫描账本。模板以中性、分类一致为先，选中时登记生成来源；场景冲突仅在本入口处理，不清分类锁。补充新用户、重复/跨日、待整理记录、分类/场景冲突、相同文字点击、手写覆盖和陈旧历史保护回归。
- 工作区：保留已有台账第 159–160 节、`backend/.env.staging.example` 及所有未跟踪素材/脚本/输出。不得全量暂存或覆盖用户文件；本轮未获提交/推送授权。
- 验证与下一步：实现后分别完整执行测试分支与生产配置 Windows 门禁，规则不变；本机无 Xcode/Swift，XCTest 与 iPhone 性能数据须明确标为待执行。任务结束更新实际范围、结果及风险，不启动相邻任务。
- 实际实现：只改 `HomeViewModel.swift`、`RecordView.swift` 两个产品文件；新增 `RecordQuickNotePolicy`、历史快照附属候选和快捷入口路由。原快捷金额统计/点击、精确金额预填计算、共享保存解析/语义词典/帮写/情绪/生活印记/会员和项目配置均未修改。
- 历史候选规则：以所选记录日期向前 180 天、正金额、`draftMeta == nil` 的已正式入账记录为范围；按类别＋5 个时段＋工作日/周末/节假日分组，同一标题须在该组至少出现于 2 个不同日期。只复用 2–12 字、有效且可复用的手写/品牌标题，排除通用系统句、金额/脏日期/风控不允许的文字；按不同日期数、最近使用时间、标题稳定排序，每组最多 6 条，不要求金额相同。现有合法金额预填仍可置前；新常用历史规则不反向改变原精确金额预填数据范围或门槛。
- 模板与显示：42 条类别/时段短句（每个当下类别 3 条），移除无证据的早班、加班、出差、热食、同行人物、具体交通方式等假定；餐饮保留早餐/午餐/下午/晚餐/其余时段。显示最多 4 条，其中已兼容预填/新增常用历史合计最多 2 条；按已有品牌、食品类型及宝宝/宠物证据排除明确冲突，不自动填入或覆盖原备注。日历仍使用原有不完整调休规则，未更改日历服务。
- 点击/场景边界：所有短句（含同文再次点击）通过 `applyGeneratedRecordTitle` 登记生成来源，保留当前分类及用户分类锁；规范化后的标题才更新锚点。仅本入口过滤无权使用的 active 包和冲突候选：family 排除无照护事实的具体历史（允许中性模板），supply 排除宝宝/宠物，commute 排除旅行文字；不选择新场景、不清旧场景或分类锁，不使用 `.note` 伪装手写，不消耗或扩大场景权益。后续真手写仍走原有语义与锁规则。
- 性能评估：历史聚合附加在现有 utility 后台快照任务，一遍遍历后仅对跨日重复标题排序/验证；任务内检查取消，沿用 requestID/完整历史键防陈旧发布。无新增请求、存储、图片读取或每键全账本扫描；按类别/时段/日型理论最多 150 组、900 个短标题，输出池与当前最多 10 个候选的品牌/食品证据均缓存，输入时只需小池筛选（有个性化候选时解析当前锚点一次）。这是静态复杂度及调用链判断，预计增量可控；未测量 Swift/iPhone 毫秒数，不声称已消除上一轮输入卡顿。
- 已执行验证：新增 `scripts/quick_note_regression.py` 专项守卫通过，校验全部 42 条模板与实际 JSON 词典的类别兼容、入口生成来源、场景过滤、后台/缓存/跨日门槛及上限。`StateRegressionTests.swift` 新增 18 项 XCTest，覆盖模板时段、锁定/未锁预览保存、180 天/跨日/日型、金额不同、pending/resolved 草稿排除、标题资格与排序、去重及事实锚点；仅编写并静态复核，未在 Windows 运行。
- 双环境门禁：当前 staging 工作区完整执行 `python scripts/validate_release_gate.py --phase windows --release-branch feature/xuzhangapp-staging`；另从真实生产 `49d963a` 创建隔离 worktree，只复制本轮两产品文件、测试与专项脚本，按锁文件安装依赖后完整执行 `--release-branch xuzhang1.0-release-2026`。两者退出码均 0、`release_repository_gate: OK`，含 IAP 模板/路由、分支编译条件、语义/体验/文案/合规/元数据/安全头/迁移/存储全部既有门禁，仅原有 7 条文案软提示。staging 保留 Debug/Release 的 STAGING；生产为 staging=[]，工程配置相对各自基线无差异。两环境验证文件 SHA-256 一致；未提交、推送或更新生产分支。
- 证据位置：`C:/Users/yf/AppData/Local/Temp/xuzhang-quick-note-gates-9a11332d9d804655917d643a91b7f384/staging.log` 与 `production.log`；独立验证副本只用于检查，不是发布。保留用户 `.env.staging.example` 与其他既有素材/输出。
- 剩余风险与下一步：等待 macOS 编译及 18 项 XCTest、iPhone 点击快捷句→换一句→保存（锁/未锁、family/supply、同文点击、再手写）、常用候选跨日/改日期、1,000/5,000 条历史输入表现签收；不将前序用户真机成功反馈视为本轮验证，不启动金额或全局输入性能优化。
- 收尾复核：独立只读复核未发现阻塞项，确认候选缓存不缓存旧锚点过滤结果、手写变化仍实时筛选，点击未改锁/场景。最终 diff whitespace 检查通过。仅清理本轮创建且文件白名单与哈希已验证的生产检查 worktree，日志保留；未删除原有 `xuzhang-audit-20260916195422` worktree 或用户数据。

### 162. RECORD-AMOUNT-INPUT-COALESCE-01：金额输入计算合并与预览复用（2026-09-17）

- 状态：`IN_PROGRESS` → `CODE_DONE`（2026-09-17），当前无进行中的实现任务，未标记 VERIFIED。主代理承接前序完整台账阅读并续读第 161 节；用户明确要求继续优化输入链路、收敛金额输入计算，不启动其他输入框或相邻路线图任务。
- 范围：`HomeViewModel.swift` 金额输入调度、`RecordView.swift` 预览缓存/金额待处理状态及明确动作前刷新、`RecordDraftResolutionService.swift` 本地缓存身份/取消检查、对应 XCTest 和新增专项静态守卫。保留上一轮快捷备注未提交成果及用户其他文件。
- 冻结边界：金额文本即时更新不变；分类/推荐阈值、用户锁、手写/生成来源、场景包权益和内容、情绪候选与生活印记业务规则、快捷金额/备注算法、存储/OCR/会员/主题及复盘/昵称等输入框不改；测试/生产配置与既有门禁规则不改。
- 实施约束：新金额立即使旧推荐失效，重算按短暂停顿合并；后台历史完成和标题联动不得绕过金额合并。保存/帮写/换角度等明确操作先消费待处理金额，恢复原即时临时分类，再按原异步预填与保存规则执行，不等待或同步跑全量历史。预览按完整原始身份复用，输入未稳定时不计算情绪候选/生活印记，离页/切模式/清空/保存取消待执行工作。
- 验证计划：专项覆盖连输合并、动作前刷新、取消/旧结果、身份变化和空结果缓存；两套完整 Windows 门禁分别执行。Windows 无 Swift/Xcode/iPhone，编译/真机帧耗时不得冒充已测；结束更新实际差异、风险与下一步。
- 实际修改：仅本卡三个产品文件、`StateRegressionTests.swift`、新增 `scripts/amount_input_regression.py` 及台账。金额 `inputAmount` 仍即时发布，每次变化先失效旧预填，再以递增请求身份安排 180ms 安静窗口；持续输入取消前一个定时任务，历史完成/备注联动/焦点 observer 统一经过 pending 守卫，不能提前进入设置解码、临时分类和预填准备。无效/清空金额、离页、切到 OCR 以及保存重置取消待执行任务；没有修改其他输入框的防抖时间。
- 明确动作边界：保存（View 与 ViewModel 双入口）、会员/免费帮写、打开/应用场景角度、打开备注编辑、点击快捷备注、显式分类选择在读类别/意图之前消费待处理金额；只运行原来的即时临时分类和启动原异步预填，不等待后台完成、不把全账本推断搬进保存。已有分类锁、生成来源保护及金额/日期/备注/历史/requestID 发布守卫保持。冷历史尚未准备好时仍按原保存合同处理，不承诺同步等待全部推荐。
- 预览收敛：以金额原文、备注、分类/锁、日期/日历、生成来源、编辑/换句意图、场景身份/类别、事实锚点、预填全部内容和天气设置/快照构成廉价键；三份单项非发布缓存复用预览解析、自动情绪和可切换情绪上下文（包括 nil），避免同一草稿被任务身份/按钮可用性/标签/保存兼容读取反复解析。金额 pending 时立即停止使用旧预览解析/情绪选择/生活印记，不运行它们的昂贵准备；文本金额仍即时显示。情绪候选最多 24 次扫描新增循环内取消检查，候选筛选规则未变。
- 反向场景复核：保留休眠情绪选择但不允许 pending 草稿读取或保存它；若仅把 12 格式化为 12.00、最终情绪场景仍相同，可恢复原选择，真正换金额/标题/场景则原身份比较拒绝旧选择。独立复核发现异步 nil-context 路径也会清选择，已在其入口及 onChange 同时加 pending/manual-mode 守卫，避免仅修一条路径；切到 OCR 暂停本页情绪计算，不借此改奖励/生活印记规则。
- 性能结论与边界：源码可确认逐键的昂贵推荐准备被合并，相同输入依赖的预览解析可复用；设置读取/历史分类逻辑本身未重构，只不再随每个有效金额字符立即执行。剩余 SwiftUI 布局、共享 ObservableObject 发布、标题/品牌等其他读取不在本轮全面治理，不能宣称消除所有输入卡顿。无新网络、存储字段、遥测或配置改动。
- 验证：新金额专项与上一轮快捷备注专项均通过；新增 12 项 XCTest（5 项请求合并/过期/取消/flush、3 项缓存含 nil/换键、4 项覆盖 27 种缓存依赖变化）并经静态复核，未在本机执行。`git diff --check` 通过；独立审阅确认明确动作 flush 和缓存依赖无剩余阻塞项。
- 双环境证据：当前 staging 完整运行 `python scripts/validate_release_gate.py --phase windows --release-branch feature/xuzhangapp-staging`；从真实生产 `49d963a` 新建隔离 worktree，复制两轮范围内三产品文件、测试和两个专项脚本，按锁文件安装依赖，完整运行 `--release-branch xuzhang1.0-release-2026`。两者退出码 0、`release_repository_gate: OK`，IAP 模板/路由、全部仓库门禁完整保留；staging Debug/Release 均带 STAGING，生产 staging=[]，两边工程配置/原门禁脚本相对基线未改。验证副本 SHA-256 与当前文件一致，只有既有 7 条文案软提示，未提交/推送/更新生产分支。
- 日志：`C:/Users/yf/AppData/Local/Temp/xuzhang-amount-input-gates-9dad9fd6f6b9464283287f1e7a3192fc/staging.log` 与 `production.log`。保留第 161 节所有代码和测试、用户 `.env.staging.example` 及其他既有文件。
- 剩余风险与下一步：待 macOS 编译/XCTest 和 iPhone 同设备 Release 对比连续金额、删除/清空、1→12→1、12→12.00、输入后立即保存/帮写/改类/选场景、离页返回及 1,000/5,000 条历史。重点验证金额帧响应、预览短暂停顿/恢复、分类/情绪最终一致及保存不漏最后输入；尚无真机耗时数据，不启动复盘/昵称或其他相邻性能任务。
- 收尾：仅清理本轮新建、已检查精确绝对路径/文件白名单/副本哈希的临时生产验证 worktree，保留门禁日志、主工作区与原有审计 worktree；未清用户目录或数据。当前无 IN_PROGRESS 任务。
...
### 168. PERSONAL-AI-COPY-POOL-01：个人 AI 文案池独立任务启动（2026-09-17）

- 状态：原 `IN_PROGRESS` 的盘点阶段停止；**2.0 实现 `NOT_STARTED`**。用户最新决定将个人 AI 文案池放到 2.0，当前不再占用进行中任务。保留已完成的盘点与最小范围设计，不表示联网能力已完成，不将前序未完成的真机验收标为 VERIFIED。
- 本轮实际范围：新增 `PERSONAL_AI_COPY_POOL_TASK_2026-09-17.md` 和本文档；承接本线程完整台账阅读，复核现行规则、冻结边界及最新交付记录，只读检查常用备注聚合、情绪候选/保存、复盘 AI、授权/法律说明及账单同步。没有改产品/配置，没有模型请求、远端变更或提交推送。
- 建议首版：仅记账页个人情绪表达候选，先覆盖明确咖啡/饮品/三餐；常用备注只做本机语义证据，不上传原文、不扩分类关键词、不修改快捷备注或默认标签。保留原候选顺序/6 条上限，空余位置最多补 2 条；独立默认关闭授权、低频有界生成、账户与设备隔离的可清空缓存，失败保留本地。
- 已确认风险：旧联网开关和隐私说明仅覆盖复盘，不能静默扩大用途；代理 feature 白名单无个人短句契约，现有月限额不能直接当作新功能授权。保存会重算情绪候选，因此后台池变化需要草稿快照保护；已有 `LedgerSyncService` 会同步选中并保存后的 emotionTag，不能将“池本机缓存”宣称为“已保存文案永不上传”。
- 进入实现前：请用户确认个人池仅本机、选中后作为正常账单字段遵守既有同步的推荐边界；适用人群/成本配额与合规进入条件在阶段 A 定案。若要求已选文案也永不离开本机，需重新限定保存展示设计，不能自行改变账单 DTO/同步。当前允许编辑仅两份文档，阶段升级先列具体文件及许可。
- 冻结与保护：保留首页/痕迹滑动、金额合并、分类/锁/换角度/权益、生活印记/奖励、历史与同步格式、旧复盘 AI 和两套配置/完整门禁；用户 `.env.staging.example` 与既有未跟踪素材/脚本保留。法律文本继续遵守独立合规任务边界，未在此改写。
- 验证/证据：上述源码只读核对，文档执行 `git diff --check` 并检查改动白名单；本轮无产品修改，不重复执行上一轮已经通过的双环境运行门禁，也不将旧证据计作新功能通过。开始产品实现后仍必须保留并分别完整执行测试、生产检查。
- 下一步与残留风险：延期至 2.0，仅保留任务卡，用户明确重启后再确认同步含义/受众成本/授权边界。当前不实施、不联网、不自动后台续做；性能影响只有设计约束，没有实测结论。1.0 转回封版验收。

### 169. RELEASE-1.0-SIGNOFF-RETURN-01：冻结新增功能并回到封版验收（2026-09-17）

- 状态：封版签收 `BLOCKED`，阻塞来自尚未具备的最终生产候选、Mac/iPhone/StoreKit 证据及既有支付安全缺口；本轮文档/只读梳理完成，当前无产品实现 IN_PROGRESS。用户明确将个人 AI 池推迟至 2.0，优先封版，不等于授权修复所有发现或自动更新生产分支。
- 范围：仅本文档、个人 AI 池任务卡和既有 `RELEASE_1.0_DEVICE_SIGNOFF_TEST_CASES.md` 的收口补充；保留原完整验收矩阵和通过阈值，不减检查、不改产品/配置、不新建重复总清单。
...
### 171. RECORD-PREVIEW-FEEDBACK-386-01：386 记账推荐反馈与夜间通勤候选核对（2026-09-17）

- 状态：`IN_PROGRESS` → `CODE_DONE`，当前无进行中的实现任务，未标记 VERIFIED。用户在 Build 386 反馈已推荐分类却只显示日期、金额输入后推荐偶现延迟约两秒，以及出行“通勤路上”只有单个情绪标签；本轮承接本线程完整台账阅读并核对第 161–170 节边界，只处理该记账链路。
- 初步证据：RecordView 的 whisper 元信息强制仅显示日期，空备注又屏蔽 prefill 的展示层级；金额原 180ms 合并仍在，但后续串行等待历史辅助与预填计算且输入时清空旧结果。第 166 节统一的是候选筛选及餐饮补充，交通未覆盖；夜间“通勤路上”命中旧固定标签，两分支相同，非简单漏同步。截图只有构建号 386，尚无 Archive 对应 commit/环境和 Instruments 数据，不将源码推断当作两秒耗时实测。
- 允许范围：RecordView 分类展示、HomeViewModel/RecordPrefillService 的记账推荐计算链路、RecordPreviewTier/RecordDraftResolutionService 的局部纯策略与对应回归测试、增量专项和本文档。夜间通勤如补候选，只用已确认同场景表达并通过原全部事实/展示/保存守卫，不更改全局默认标签或存量展示。
- 冻结：180ms 合并时长、输入即时显示、用户分类锁/明确备注优先、换角度和场景包权益、快捷金额/备注推荐语义与排序、生活印记/奖励、首页滑动、同步/数据格式、支付与审核登录、两套环境/工程配置及原完整门禁均不改。无本轮提交推送/同步分支授权，不自动交付分支。
- 验证计划：空备注已采用推荐的分类展示、无证据/计算中不冒充分类、手动锁定与备注覆盖、输入撤销/同金额格式变化/快速输入/立即操作和过期任务丢弃、夜间通勤候选同义/循环/保存与白天/天气/上下班反向保护；两套真实分支配置各自完整 Windows gate，原相关专项保留。本机无 Swift/Xcode/iPhone，性能改善和 386 后续包须外部真机签收。
- 现场保护与下一步：保留第 168/169 节、签收文档、用户 `.env.staging.example` 和全部未跟踪资料。先将局部修复与诊断收口并审阅，再记录实际文件、双环境证据、剩余风险；不重启服务器，不启动支付或个人 AI 池任务。
- 实际文件：五个产品文件 `RecordPreviewTier.swift`、`RecordView.swift`、`HomeViewModel.swift`、`RecordPrefillService.swift`、`RecordDraftResolutionService.swift`，以及 `StateRegressionTests.swift`、新增 `scripts/record_preview_feedback_regression.py` 和本文档。没有改原专项或完整门禁、工程、后端、词典、全局 HomeItem/文案 Resolver/场景语义服务。
- 分类展示：保留 whisper/confirm 层级与空备注语义，只拆开“是否生成备注”和“是否已有可展示分类”。whisper 在用户明确锁类或当前正金额匹配已采纳推荐时显示原分类/场景元信息；待计算、旧金额、未采纳或分类不符时不冒充推荐。明确手写、生成来源、换角度、保存分类规则未改。
- 推荐关键路径：运行时历史快照不再串行计算快捷备注历史，原函数默认行为保留供纯计算使用；原快捷备注历史函数改为独立 utility 任务，在推荐所需历史就绪后与预填独立运行，仍使用同一份历史输入、日期、排序与候选规则。独立 key/request ID/取消与发布守卫防止旧账本、旧日期覆盖；空池也记完成，离页取消后同历史可恢复，历史未就绪时仅显示原模板。没有把全量扫描移回输入/getter/保存，也没有增加网络或虚假加载推荐。
- 减少重复计算：RecordPrefillService 的候选每条只分类一次，按原 kind 分组复用原最大 score/tie 选择；支持权重、排序、采纳阈值不变。目标时段/日型移出逐条循环，并在重计算循环/阶段保留取消检查。180ms 窗口和立即操作 flush 未改；这是源码层关键路径缩短，不是两秒耗时复现或固定响应时间承诺。
- 情绪边界：第 166 节统一的是筛选/保存入口和餐饮单例补充，不是全分类候选覆盖。本轮仅补“晚上这段通勤”这一已识别的中性夜间通勤场景，原默认保留，新增“晚间这段通勤／这趟晚间通勤记下／晚上的通勤记一笔”；无商户、场景包为空或 commute，标题及非空锚点须为同场景，预览/自动标签须一致。全部通过原展示一致、类别、事实、生活印记/奖励和保存再校验；原多候选顺序、白天单例不变，天气/上下班冲突不补。周末被原展示层纠正或没有可靠同义候选时，仍不强开按钮；不顺带修全局默认标签的其他差异。
- 回归与交叉审阅：新增 5 项 XCTest（展示层级、拆分快捷备注后推荐/历史等价、夜间通勤循环保存/反向边界/失效）及生活印记奖励两组夹具；XCTest 仅静态复核，未编译执行。新增专项、原金额/快捷备注/餐饮专项均在两套隔离副本通过；当前 staging 原首页滑动专项亦通过。独立审阅确认取消/恢复/空结果/池缓存、当前分类展示与单次分类的等价性无阻塞；`git diff --check` 通过。
- 双环境完整证据：从真实 staging `f83f454`、production `0b6b58e` 分别创建 detached 验证 worktree，各自按 backend lockfile 安装依赖，复制五产品文件、测试、新专项与当前餐饮专项后，完整运行原 `validate_release_gate.py --phase windows --release-branch <对应分支>`。两者退出码 0、`release_repository_gate: OK`；staging Debug/Release 含 STAGING，production staging=[]，只有既有 7 条文案软提示。两套工程、环境模板、原完整门禁及金额/快捷备注门禁相对各自基线无差异。生产副本包含当前服务所依赖的前轮餐饮实现，仅证明配置兼容；生产实际分支仍缺 `4a27e6e`，不把副本验证写成同步交付。
- 日志与保护：完整门禁及依赖日志保留于 `C:/Users/yf/AppData/Local/Temp/xuzhang-record-386-d48aafdb6d8c4a1fac8fd76f24469665/` 的 staging.log、production.log、staging-dependencies.log、production-dependencies.log。验证副本与主工作区范围文件逐一核对 SHA256；用户 `.env.staging.example` 仍为 `48FF5A142D67DBEAD8817432F23CE2A089C9A11C54D0A9D1B41622A30F8CD92C`。没有提交、推送、更新实际生产分支或部署。
- 剩余风险与下一任务：Mac 编译/XCTest 后，在新包复测金额空备注推荐即显示分类、连续输入/删空/金额格式变化/立即保存与帮写、手写吃饭覆盖/手动锁类/场景权益；历史较多时冷开和热开输入对比、快捷备注稍晚补入后点选/离页返回/日期变化；22:48 的“路上花费补上”和“通勤路上”换句→情绪切换→立即保存，以及白天/周末/天气/上下班反例。记录设备、历史条数、commit/Build 与输入至推荐耗时；无 Instruments 数据，不保证每次 180ms 完成。先签收本轮再按用户安排进入支付安全，个人 AI 池仍留 2.0。
- 收尾：仅清理本轮创建、绝对路径/八文件副本哈希/变更白名单已核验的两个验证 worktree 及可重建依赖和两项 Python 检查缓存；保留完整日志、主工作区全部用户改动和原 `xuzhang-audit-20260916195422` worktree，未删除用户数据。
- 后续排版只读核对：用户询问是否推荐文案占位。核对两张 54 元截图与 `LifeEntryPreviewCard.swift`、RecordView：推荐/解释/印记均条件插入，没有隐藏文字固定占位；推荐出现会新增行高和 7–8pt 间距，生成备注后标题由 15.5pt 变 21pt，情绪行无论能否切换均至少 44pt，附加提示/印记还可继续叠加，卡片自然增高带动保存按钮下移。分类原先不显示仍是层级限制，不是文字把它挤走。建议仅评估卡片提示层级与局部留白，保留可点击标签触控面积及大字号换行；本次未修改产品/配置/原测试，不重复运行已通过的双环境门禁。尚无新布局或真机签收，状态及下一任务不变。

### 172. IAP-PRODUCTION-APPLE-401-DIAG-01：生产包订阅鉴权失败只读排查（2026-09-17）

- 状态：`BLOCKED`（待生产运行配置及 Apple 侧密钥证据），无产品实现 IN_PROGRESS。用户报告生产编译包登录已验证成功，但购买提示 `Apple lookup failed (401)`；安装渠道、Build 和内测是否购买成功仍待确认，不将登录成功扩展为支付通过。
- 范围与证据：只读核对 backend `iapService.js`、config、server 的验单链路和 iOS `IAPService.swift`，两分支前两文件一致。错误来自服务器请求 Apple 交易接口，Apple 401 被包装为本服务 502/APPLE_LOOKUP_FAILED，不是本服务手机号 JWT 401；Production 仅在交易不存在时回退 Sandbox，不应通过换生产 endpoint 或放宽 401 回退绕过鉴权。Token 使用 ES256、Issuer/Key ID、bid 和五分钟时效；内联 APPLE_PRIVATE_KEY 优先于文件路径，dotenv 默认不覆盖既有进程环境，均是需核对的配置来源。
- 线上证据缺口与下一步：既有服务器访问通道不可用，本轮未重试 SSH、未登录 Apple 管理后台、未取得实际 PM2/密钥状态。指导用户在生产目录进行不显示私钥/JWT 的只读配置及无效交易号鉴权探测，再区分密钥不配套/类型或权限/撤销、bundle、服务器时间与 PM2 旧环境；探测不购买、不授予会员、不改变配置。凭据身份暂不能定因，不能承诺仅改某一项即恢复。
- 保护与验证：本轮仅更新本文档，未改 iOS、后端、两套配置或任何门禁，未提交/推送/部署/重启；保持第 171 节未提交修改及用户文件。原双环境离线门禁通过不证明线上 Apple 凭据有效，因此不重复跑离线检查充当本故障验证；支付 JWS 签名链缺口仍独立保留。配置原因确认后才安排最小修复，恢复购买及实际 Sandbox/Production 矩阵仍需签收。
- 2026-09-18 用户提供生产目录探测结果：UTC `2026-09-17T16:01:15.601Z`、Production endpoint、bundle `com.xuzhang.app`、keySource=file，仍为 APPLE_LOOKUP_FAILED/Apple 401。生产分支工程 bundle 与其一致；当前目录配置也失败，故不能仅归因于 App 或 PM2，但该探测不代表运行中 PM2 的完整配置快照。已到 Apple 响应说明该路径私钥可读且本地签名完成，不证明密钥类型/归属/有效状态正确；UTC 显示换算为北京时间 9 月 18 日 00:01，尚无 NTP 同步证据。只读核对 Apple 官方创建密钥及 JWT 文档：应在 App Store Connect→用户和访问→集成→App 内购买创建，JWT 的 Issuer/Key ID 与私钥须配套。下一步请用户确认密钥来源、Active 状态及三者配对，不索取私钥/JWT，不盲目撤销共享密钥或更换环境地址；状态继续 BLOCKED，未改产品或服务器。
- 用户随后确认配置使用截图第二把 App 内购买 Key ID。尚未证实服务器原始私钥与其配对及 Issuer/时钟完整正确；下一步采用同一凭据、无效交易号分别对 Apple Production/Sandbox 做只读直接请求并比较 HTTP Date，不能只因生产返回 401 就断言整套密钥无效或授权扩大回退。诊断不调用购买/会员写入、不改持久配置、不输出私钥或签名 JWT；若两端不同，再按实际响应决定路由排查，不预设修复。
- 双端对照新证据：用户同一凭据探测于 UTC 16:12:08，Production=401/空错误体，Sandbox=400、4000006/Invalid transaction id，Apple Date 分别 16:12:09/10，与主机无明显偏差。Sandbox 的无效 ID 响应符合所用占位 ID=0，但不能当作真实交易验单通过或独立证明所有权限有效。排查优先级转向环境路由：现后端明确忽略 signedTransactionInfo、生产先查 Production，仅 not-found 回退，401 必定停止；若实际是 TestFlight/审核沙盒交易，此路径可以解释失败。现有 iOS 已上传 StoreKit JWS，可评估仅将其中 environment 当不可信路由提示，仍从固定 Apple 端点独立取证并校验所有交易/账户/环境条件；不是信任客户端开会员，不应把所有 401 加入回退。真实交易与安装渠道仍需闭环，未实施补丁、未改两套环境或原门禁，后续最小后端实现需用户明确安排。
- 用户已确认生产配置包通过 TestFlight 安装：业务 API 为生产，不会把 Apple 交易变成 Production。进一步核对 Apple `get-transaction-info` 官方文档：401 明确为授权头 JWT 无效，404 为交易不存在；没有取得“首次未上架必然 401”的官方证据，不能将该猜测或路由缺口冒充此 401 的底层定因。官方 2026/05/05 changelog 说明新域名为推荐项、原 itunes.apple.com 域名继续支持，不为此修改两套 endpoint。代理仅用无效占位 Bearer 对公开两端进行负向只读请求，两者均 401；与用户 Sandbox 400 不同，但网络/凭据上下文不同且未取得真实交易，仍不标验单通过。下一有效证据应是本次真实交易的 Sandbox 查验；即使修好 TestFlight 路由，Production 401 仍需独立关闭才能认可真实生产支付。未改代码/配置或开展真实购买。
- 用户询问具体下一步：只读确认现有 server 在 Apple 查验成功后才写入 iap_transactions，失败 catch 只返回错误，没有失败交易号日志；iOS 请求已带真实交易号与 JWS，但没有可供用户直接读取的现成诊断入口。不能要求用户从不存在的失败订单记录中取 ID。建议获授权后只补临时、受控的后端验单诊断：使用实际恢复购买请求在服务器内作双端只读查询，只输出环境/状态/错误码及脱敏关联信息，不输出 JWT、私钥、手机号或完整交易票据，不改变当前权益判定；用户部署后点恢复购买提供证据。尚未获实现/部署授权，因此本轮仍仅文档更新，未实施诊断或路由修改。

### 173. IAP-FAILURE-DIAGNOSTICS-01：限时定向验单失败诊断（2026-09-18）

- 状态：`IN_PROGRESS` → `CODE_DONE`，当前无进行中的实现任务，未标记 VERIFIED。用户明确“加诊断”，授权仅后端临时诊断及必要测试/说明；承接本线程完整台账阅读及第 172 节证据，不授权实际支付修复、提交推送或服务器部署。
- 允许范围：新增 backend 独立诊断模块/专项测试/默认关闭的独立配置示例与操作说明；config 增加可选诊断配置，iapService 仅暴露原签名函数和附加不影响返回的上游状态元信息，server 仅在已认证验单失败后接入不等待的诊断，package.json 增量测试接线，本文档。保留全部既有未提交 iOS/文档/环境素材。
- 设计合同：默认关闭，精确目标手机号、显式到期且最长 24 小时；只针对已配置商品/有效格式交易号的 Apple 查询失败，不处理未登录或账号绑定失败。每进程最多 5 次、单次在途、60 秒冷却，双端固定 Apple GET、并行且各自 8 秒上限/响应大小上限；在原失败响应之后后台运行，诊断结果不能授予权益或改变原错误。日志仅随机关联 ID、枚举环境/错误、HTTP 状态、时差、匹配布尔值，不打印手机号/账号 ID/完整交易号/JWS/私钥/原响应/任意客户端文字；客户端环境只标为未验证提示。
- 冻结：生产/测试主 endpoint 和原环境门禁、原 401 不回退规则、会员/绑定/有效期规则、JWS 签名链现存缺口、iOS、审核登录、数据库/Redis/DTO、现有环境模板与原测试均不改。新增默认关闭示例不写真实号码或凭据；不执行线上购买/诊断请求或修改服务器。
- 验证计划：独立模拟覆盖关闭/误配/到期/错号码/触发错误/重复限频/超时/超大响应/异常日志安全/Production 401＋Sandbox 200 但原 API 仍失败且会员不变；原 backend 全量测试与两套真实配置的完整 Windows release gate 分别运行，不减原检查。结束记录文件、证据、风险及用户部署/恢复购买步骤。
- 实际文件：新增 `backend/src/iapDiagnostics.js`、`backend/scripts/verify-iap-diagnostics.mjs`、`backend/.env.iap-diagnostics.example`、`backend/IAP_FAILURE_DIAGNOSTICS.md`；修改 `backend/src/config.js`、`backend/src/iapService.js`、`backend/src/server.js`、`backend/package.json` 和本文档。只增加默认关闭的可选设置、原签名函数导出和错误内部元信息、失败响应之后的诊断调度、增量测试命令；原购买响应与权益写入规则不变。
- 实现及交叉审阅：目标号码取自认证后的数据库账号；启动误配不允许随后自动变为有效，延迟执行前再次检查到期。固定 GET 不跟随重定向，流式限制响应大小，超时取消；异常调度/日志不影响购买。客户端与查询返回的 JWS 仅解码作未验证提示/匹配布尔值，明确 `signatureVerified=false`、`grantsMembership=false`。审阅发现“primary”字段会误指最终失败端点，已改为 `failedLookupEnvironment/failedLookupStatus`，补齐 HTTP 200 缺票据的元信息，并覆盖原 Production 404→Sandbox 401 链路；未修改 401 回退。
- 双环境验证：分别从真实 staging `f83f454`、production `0b6b58e` 创建 detached worktree，仅复制上述八个 backend 文件，各自 `npm ci --no-audit --no-fund`、完整 `npm test`（原六项＋诊断专项）及原 `python scripts/validate_release_gate.py --phase windows --release-branch <对应分支>` 全部退出 0，均 `release_repository_gate: OK`，仅原有七条文案软提示。staging 工程保留 STAGING，production staging=[]；相对各自基线，两套环境模板、工程、原检查与 package-lock 未变。专项以隔离内存服务和回环 HTTP／模拟 Apple 验证失败先返回、双端状态、敏感字段不落日志、诊断成功不写订单/不授予会员、正常成功/关闭时行为，以及限频/过期/异常边界；不冒充真实 Apple 或线上支付验收。
- 证据与工作区保护：完整 staging/production 的 dependencies、backend、gate 日志保存于 `C:/Users/yf/AppData/Local/Temp/xuzhang-iap-diagnostics-5eab9ed3807e448ea33882f1982618a3/`。两个副本八文件 SHA256 均与当前补丁一致；用户 `.env.staging.example` SHA256 仍为 `48FF5A142D67DBEAD8817432F23CE2A089C9A11C54D0A9D1B41622A30F8CD92C`，第 171 节未提交 iOS 修复、签收文档及其他用户资料保持原样。`git diff --check -- backend` 通过；没有提交、推送、同步实际分支、部署或开启线上诊断。
- 剩余风险与下一任务：诊断默认关闭，现有 401 尚未修复，也没有真实交易双端结果。另行授权交付后仅部署 backend，按操作文档在目标环境限时配置准确手机号，到期建议一小时；确认运行服务后在现有 TestFlight 包点一次恢复购买，只提供脱敏诊断行，取证后关闭。不需要重编 iOS；沙盒订阅过期可能不发起恢复验单，需如实判断没有日志。真实 Sandbox 查询成功不代表完整签名/权益校验或 Production 支付已通过；Production 401 与原 JWS 签名链缺口仍待独立关闭，不捎带修复或修改生产主 endpoint。
- 收尾：核对两个临时 worktree 的绝对父路径、基线 HEAD、八文件哈希及完整变更/忽略白名单后，仅移除本轮验证副本与可重建依赖/检查缓存。六份验证日志保留，源代码仍在主工作区，可从相应分支和补丁重建副本；原 `xuzhang-audit-20260916195422` worktree 和全部用户资料未删除。
- 后续交付授权（2026-09-18）：用户确认“提交推送”，承接上一轮第 1 步，允许仅本诊断补丁提交、推送并定向同步生产分支。交付范围为上述八个 backend 文件与本节台账，不夹带第 168/169/171/172 节、记账页修复、签收文档或用户环境模板；不部署、重启或在线启用。两分支获取远端后均与原验证基线一致，沿用刚完成的双环境完整测试证据并核对交付内容/冻结配置，避免无代码变化的重复全量检查。状态保持 CODE_DONE，下一步仍是用户部署后限时取证，不能以提交完成代替支付验收。
- 交付结果：诊断提交 staging `c0c733a`、production 定向 `cherry-pick -x` 为 `62592c5`，已原子普通推送 origin 两分支，无强推。相对各自已验证基线均仅九个授权文件变化，两分支 backend 完全一致；原环境模板与分支工程校验再次分别通过（staging Debug/Release 含 STAGING，production staging=[]），八个补丁文件和八个关联用户文件的工作区 SHA256 前后一致。原完整双环境门禁证据继续有效，未修改实现或原检查以绕过测试。后续仅补交本文档记录并核对远端；服务器仍未部署/重启/开启诊断，401 与真实交易取证待用户下一步操作。
- 用户部署后真实取证（2026-09-18）：用户提供 backend 的年度恢复请求脱敏诊断；原请求最终为 Production 401/APPLE_LOOKUP_FAILED，后台对照 Production 401、Sandbox 200。Sandbox 返回载荷的 environment=Sandbox，商品/交易/bundle 匹配，未撤销且未过期，时差 0/-1 秒；这是实际交易查询证据，不再只是无效交易号探测，也不等于签名链或会员验收通过。accountMatches=false 仅表示诊断未匹配当前账号：可能缺少/非字符串/不同 appAccountToken，尚不证明数据库已绑定他人；原请求在账号验证之前就被 401 阻断。signatureVerified=false 表示诊断未作签名验证，不是签名验证失败。
- 本轮只读核对：客户端 IAPRestoreFailureCopy 对 APPLE_LOOKUP_FAILED 不透出服务端原因，回落到“当前账号暂时没有可恢复的会员权益”，故此提示不能证明没有购买记录。购买传入 cloudUserId 作为 appAccountToken，后端新用户使用 randomUUID；测试/生产独立数据库即使手机号相同，也可能对应不同内部账号，注销重建亦需核对，但尚未确认用户这笔交易的原始购买环境。下一步先确认是否内测 staging 购买后在生产恢复，再独立设计交易环境路由与必要的归属诊断/提示修复，不能直接放宽账号绑定、401 回退或把生产主 endpoint 改为 Sandbox。提示文案修复需 iOS 新包，后端路由可独立交付，均未在本轮实施；收到证据后可关闭临时诊断。仅补本节证据，未修改产品/两套配置或原检查，未重跑不相关门禁、提交推送或操作服务器；状态保持 CODE_DONE，支付仍未验收通过。

### 174. IAP-SANDBOX-ROUTING-HINT-01：生产业务 API 的 Sandbox 交易查询路由（2026-09-18）

- 状态：`IN_PROGRESS` → `CODE_DONE`，当前无进行中的实现任务，未标记 VERIFIED。用户确认最初在 staging 内测购买、Apple ID 未变，随后同意按最小后端范围继续；承接本线程完整台账阅读及第 142/144/146/173 节边界。此事实与测试/生产内部账号不同的线索相符，不能仅凭 accountMatches=false 就证明票据 token 存在或已绑定别人。
- 允许范围：仅 `backend/src/iapService.js` 的请求路由、新增路由专项测试及 package.json 增量接线、原诊断操作文档的兼容说明、本文档。生产配置保持 Production，仅当客户端 JWS 有界可解析、明确 Sandbox 且交易号/商品/bundle 与当前请求一致时，将其当不可信查询提示向固定 Apple Sandbox HTTPS 查单；所有权益字段仍来自 Apple 查询返回，并通过原环境/商品/账号/期限/撤销及数据库归属校验。提示本身不是验单通过证据，不信任客户端账号、价格、有效期或 URL。
- 冻结：staging 始终 Sandbox；缺失/无效/不匹配提示维持原 Production 首查及仅 not-found 回退，不扩大 401/服务错误回退，不加反向 Sandbox→Production 回退。账号不得跨环境自动迁移/改绑；不修改生产授权 JWT、JWS 签名链既有缺口、会员商品/权益、iOS 提示/页面、数据库/Redis/登录/诊断开关、两套环境模板/工程、原全部检查。保留已有未提交修改；本轮无提交推送/部署授权。
- 迁移与回退：不改持久数据或 DTO，现有 iOS 已提交必要 JWS；如需回退，仅撤回本次路由分支即可恢复原首查顺序，不能通过把生产配置改成 Sandbox 回退。修路由不承诺修好 Production 401，也不保证旧 staging 交易可归属当前生产账号。
- 验证计划：Production 真交易/401、匹配 Sandbox 提示直查、无/坏/超长/伪造与不匹配提示、固定端点、staging 忽略 Production 提示、原 404 回退、Sandbox 401/404/5xx 不反向查询；返回载荷与提示冲突/错商品/错交易/错 bundle/错账号/过期/撤销拒绝；回环 HTTP 验证正确账号可开通、旧账号/已有绑定冲突/缺 token 不新开会员或改写归属。原七项 npm test 不变并追加专项，两套真实分支基线各自运行完整 backend 和 Windows gate，签名与真实支付待外部验收。
- 实际文件与路由：仅修改 `backend/src/iapService.js`、`backend/package.json`、`backend/IAP_FAILURE_DIAGNOSTICS.md`，新增 `backend/scripts/verify-iap-sandbox-routing.mjs`，更新本文档。生产请求的客户端 JWS 最多 32 KiB、严格三段 base64url 格式、可解析且 header.alg=ES256；明确 Sandbox 并匹配交易/商品/bundle 才选固定 Sandbox HTTPS。该解析没有认证签名，不能直接授予会员；账号/有效期等提示字段全部忽略，仍以 Apple 返回载荷运行既有校验和归属决策。Sandbox 直查错误直接返回，无反向回退；原 shouldFallbackToSandbox 函数、生产配置与 staging 路由不变。
- 回归与独立审阅：专项 152 个模拟/回环 HTTP 场景通过，覆盖原 Production 成功/失败/404 回退、32 KiB 边界、空/坏/不匹配提示、staging 始终 Sandbox、直查错误不回退、月度/年度/永久权益及期限规则、Apple 返回与客户端声明矛盾、缺失返回载荷/错账号/缺 token/已归属他人/过期/撤销。真实本地 HTTP 对拒绝前后双方会员及原交易绑定作快照相等断言，验证同账号购买与有/无 token 的已有归属恢复；Apple/DB/Redis/SMS 均隔离为模拟或内存，不代表线上购买成功。独立只读复核未发现新增绕过或越界；既有签名链缺口保持显式未关闭。
- 双环境完整证据：从真实 staging `3105312`、production `b8c9b0a` 分别建立 detached 验证副本，仅复制四个本任务 backend 文件，各自 npm ci、完整 npm test（原七项＋路由专项）、原 Windows release gate 全部退出 0，均 release_repository_gate: OK。两份工程分别验证 staging Debug/Release 含 STAGING、production staging=[]；仅原有七条文案软提示。日志保留在 `C:/Users/yf/AppData/Local/Temp/xuzhang-iap-routing-08b87beb18234bd3bc29a9606624a273/` 的 staging/production dependencies、backend、gate 六份日志；四文件哈希与主工作区一致。
- 冻结与保护核对：副本相对各自基线，原两套模板/config、iOS/工程、server/账号绑定、诊断模块、package-lock、原七项测试和完整门禁无差异。八个既有用户文件 SHA256 前后一致（含 `.env.staging.example` 的 `48FF5A142D67DBEAD8817432F23CE2A089C9A11C54D0A9D1B41622A30F8CD92C`），原未跟踪资料保留；git diff --check 通过。没有提交、推送、更新真实生产分支、部署或发起真实购买，前轮真机/iOS 改动不夹带。
...
- 范围与现场：用户 11 张截图及“32 元 → 手选购物 → 手写夜宵 → 连续换句 → 保存 → 编辑更新变餐饮”链路；交叉审阅 RecordView、HomeViewModel、RecordDraftResolutionService、两个编辑入口、HomeItem/词典、预览卡、预填与快捷备注、存储/同步字段及原测试。当前 staging HEAD 为 58e4975；保留第 171 节五产品文件及测试/专项、原签收文档、环境模板和全部素材。实际仅新增 `RECORD_CONTINUOUS_INTERACTION_AUDIT_2026-09-18.md` 并追加本节，无业务/测试/配置修改，无提交推送、服务器或账本操作。
- 确定原因一及用户最新澄清：以最后一次明确用户意图为准，购物→手写夜宵应在首次保存前成为餐饮；夜宵→主动选购物才保持购物。新建 manualNoteOverrideCategory 的人工锁 guard 阻断后写意图，preferNoteSemanticsForCurrentDraft 已无调用，局部强备注名单又缺夜宵；因此首次存购物已不符合本次顺序，不能称为正确。RecordEditSheet 的 editPreviewResolution 与 VM updateItem 又只按本次分类差异/品牌冲突传锁，没有继承已保存决策；夜宵无品牌且分类未改时被重新解析为餐饮，之后仍保留人工分类标记。编辑碰巧改为餐饮不等于正确执行最后意图。字段在 JSON、SQLite、云 DTO 中已存在，不是同步丢字段；FocusedRecordEditor 亦调用相同更新路径。只继承永久人工锁会把首次错误固定下来。
- 历史修改意图核对：48db02e（6 月 24 日）确有真手写冲突解除旧锁、快捷句不解锁；9f63042（6 月 26 日）收紧强语义/小金额/生活线索时改成有人工锁即返回，并移除解除锁调用。可证明覆盖行为被收紧，不能无证据声称该 guard 源自哪条原用户反馈，也不能整包回退。9 月 17 日 5516a67 保护人情“请客朋友”系统换成“一起吃顿饭”后不漂到餐饮；1676b19 保护日用快捷句“便利店补一袋日常”不因关键词改餐饮，并保留 180ms 合并、动作前 flush 和来源失效；情绪池 164/166 及分类可见性 171 均未处理本次顺序。前述保护有效，应保留系统生成不是新消费事实的边界；本次最新用户规则替换历史沿用的“真手写也永不解锁”局部合同，不改写旧台账证据。
- 编辑历史原因：5567e60（6 月 11 日）为防止品牌默认分类抢回用户选择加入 categoryWasEdited/categoryOverridesBrand；16d702a 迁入统一解析器，d0f8237 新增非品牌语义，2532ceb 加持久化人工纠正标记但未接回解析入口。eb5da74 仅在无修改预览保留原情绪，提交仍重算；8935376 真改备注解除旧品牌、0fc1de3 保留手写原话及受抑制品牌、bc465cb 防止改分类覆盖“徐记花甲鸡爪｜宿豫店”等原文，均需保留。旧购物→真手写瑞幸不能被旧品牌冲突锁阻断，随后再主动选购物则购物胜出。不能以分类值变化冒充用户点击类别，避免污染纠错学习。详见报告历史表，区分文档直接证据和根据 diff 的行为推断。
- 确定原因二：换句直接写 inputTitle，并将 lastDraftIntent 从 note 改成 category；手写夜宵锚点仍生成餐饮句，人工分类锁仍为购物。大字预览独立检查文案与分类兼容，餐饮句退回购物兜底；词典只读匹配确认“晚点补点能量”命中餐饮“补点能量”，“夜里吃点热乎的”命中“热乎”，“这顿先记下”无分类命中，解释截图中上下文案间歇一致。保存解析还可能再次修复冲突标题；快捷备注默认按购物池，情绪按分类/解析标题，均非单一备注状态的可靠呈现。截图非完整录像，不推定保存前最后一次操作。
- 确定原因三及交付：已提交 RecordView 的 whisper 元信息只返回日期，推荐文字却独立展示，帮写升 confirm 后才显示分类。第 171 节当前工作区已有 showsCategory/hasCurrentRecordCategoryRecommendation 修复但未提交；staging/production 提交版本仍为旧显示策略。不能将验证副本通过当作用户包已带修复；截图缺 Build/Archive commit 映射，不断言具体安装包来源。
- 额外同链风险：编辑预览在无修改时显示原情绪，保存/VM 却无条件重算、更新时间、落库并上传。因此仅补类别锁仍不足以保护原自选情绪。无修改判定不能直接使用现有金额/秒数展示容差，需保留未触碰字段原值和精度；同类别主动确认属于来源变化，需与完全无操作区分。原测试直接传 true 验证解析器，没有完整保存→编辑→updateItem 往返。
- 方案合同：主动选类与真实明确手写按实际用户事件顺序裁决，模糊/混合多义备注不猜类别；只在内存补来源和单调事件序号，不依赖 observer 到达顺序。最终类别/来源保存后作为两个编辑入口的基线，加载旧备注不是新手写；完全无修改原样返回且零写入/上传，后续真实改备注/主动选类继续按最后明确意图；仅改金额日期不重新分类。生成句不是新用户事实，候选刷新不写实际备注，大字预览/输入框/保存共用同一修订解析，分类展示不依赖帮写。用户明确“先看候选，再采用”且限定“只能是手写备注后的，其他情况不要动”：仅真实手写及其继续润色分支为“润色这句”，行内候选点击“用这句”才采用并可撤销；空备注帮写、纯生成换句、快捷备注点选、换角度保留原方式，不新增确认/撤销/其他步骤。程序填充、打开输入框或焦点变化不算手写；输入法组词不提前裁决，立即保存先处理最新已提交文本。最后主动选类仍与手写冲突时，可抑制编造事实的候选/情绪；不能为了文案统一反改分类或原话。手写取代旧分类选择时清除失效锁/纠正来源，语义改类不伪记人工点击，旧记录不批量猜测改类。偏好纳入方案，尚未实施，不扩展为发布授权。
- 计划边界：未来定向任务需明确允许修改记录页来源/换句语义、对应草稿解析及两个历史编辑入口；不借第 171 节授权直接扩大实现。仅针对当前连续链路，D3 打开编辑器被当清空、D4 清空金额意图、S1/S2 场景包合同及权益延续仍单列；报告 C19 为对照观察而非默认合并修复。不改全局词典、生活印记/奖励事实、OCR/AI 导入合同、存储/同步格式、首页框架、支付或两套配置；180ms 合并、会员/场景访问规则保持。
- 实际验证：四项原专项 `record_preview_feedback_regression.py`、`quick_note_regression.py`、`amount_input_regression.py`、`meal_emotion_regression.py` 均退出 0；源数据词典匹配和 HEAD/工作区交叉核对完成。专项是源码合同检查，不能证明 UI 连续链通过。本轮无 Swift/Xcode/iPhone，未执行 XCTest；文档变更不重复跑不相关 backend/完整双环境门禁。
- 剩余风险与下一步：报告列出 24 条序列（含 1 条相邻问题观察），要求正反操作顺序、重复选择、真实 VM/编辑入口、保存解码往返、无修改写入计数、异步旧结果、免费/会员、快捷候选及情绪等联合验收。按用户后续安排启动单一明确范围修复，先统一新建与编辑的最后明确意图及最终来源、保护无修改更新，再统一手写备注/候选/预览并完成第 171 节实际包验收；不能把首次存错的购物永久锁住。保留原完整门禁和两环境检查，Mac/真机证据齐备前不标 VERIFIED。当前分析与方案完成，不宣称问题已修复。
- 文档收尾：独立审阅交叉核对编辑锁/无修改路径及历史修改原因；修正前半轮“人工分类永久优先”的误解，最终合同为最后一次明确意图，候选采纳严格限于手写后。`git diff --check` 与报告空白/范围检查通过；八个既有非台账 tracked dirty 文件 SHA256 前后一致，原未跟踪资料保留。本轮新增文件仅上述报告。

### 176. RECORD-CONTINUOUS-INTENT-FIX-01：记账新建至编辑的最后明确意图一致性（2026-09-18）

- 状态：`IN_PROGRESS` → `CODE_DONE`，当前无进行中的实现任务，未标记 VERIFIED。用户在第 175 节分析后要求“这次一次要改好”，据此前已确认的规则实施完整连续链修复；历史改动目的已审阅，完整台账阅读沿用本线程证据。
- 范围：新建真实手写与主动分类事件、仅手写后的候选采纳、预览/提交来源一致、两个编辑入口及更新的基线/无修改保护、相关回归与签收文档。允许定向更新 RecordView、HomeViewModel、RecordDraftResolutionService、RecordEditSheet、FocusedRecordEditor、必要输入控件接线及 StateRegressionTests；保留并联合验证第 171 节未提交补丁。
- 固定合同：购物→手写夜宵为餐饮，夜宵→主动选购物为购物，生成/快捷候选文本不是新手写意图，无修改编辑不重解析/改时间/写入/上传；旧品牌保护、原话保护、情绪事实守卫、180ms 合并、异步失效及权益边界保留。候选确认只限真实手写及其继续润色，其他流程操作方式不变。用户新规则替换旧“手写永不解锁”局部合同，不整包回退历史修复。
- 冻结与保护：不改全局词典、持久化 DTO/账本迁移、OCR/AI 导入合同、会员/场景权益、支付、首页架构、两套环境配置或个人 AI 池。不启动相邻路线任务，不提交推送/部署。工作前保存相关已有脏文件快照，增量补丁保留原工作区改动。
- 验证计划：以第 175 节报告 24 条连续序列为依据，新增真实状态转换、持久化往返、来源标记和无修改保护断言，运行原专项及完整 Windows 门禁，核对两环境兼容；Swift/XCTest/真机无法执行的部分明确留待对应环境，不能用静态通过代替 VERIFIED。实施文件、实际证据、剩余风险及下一任务在收尾追加。
- 实际实现：`RecordDraftResolutionService.swift` 增加新建/编辑共享的 `RecordExplicitIntentState` 与局部明确语义策略，只有真实已提交手写及分类点击登记事件；生成文本不推进分类意图，模糊/混合类别保留现值，具体关键词避免被其包含的短词误判。`HomeViewModel.swift` 的人工分类锁改为从最终来源计算，移除第二份可变布尔锁及失去调用的旧解锁入口；保留手写原始锚点和来源，手写取代旧选择时清除失效纠错来源，旧异步推荐不能覆盖新明确决定。未修改全局词典。
- 记录页范围：`RecordView.swift` 仅真实手写及其衍生改写进入候选、用这句、保留原文和撤销，未采用不写草稿，上下文变化即失效；其他帮写/直接换句/快捷点选/角度操作方式保留。润色保留原事实，不添加商户、加班、热食等推测。预览与保存共用 VM 草稿构造器，明确冲突的手写不补造分类情绪，手写后的快捷候选增加事实过滤。保留第 171 节分类与已采用推荐同显的既有工作区修复。
- 编辑闭环：`RecordEditSheet.swift` 的共享 `RecordEditIntent` / `RecordEditPolicy` 与 `FocusedRecordEditor.swift` 按真实改动合并最新账本行，`HomeView.swift` / `StatsWebView.swift` 仅四处编辑回调传递意图。独立复核补齐首次进入的固定 `initialBaseline`，防止同步刷新后将未操作的旧表单误认为新修改；未动金额/日期保留原精度，未动照片/来源/情绪保留最新行。完全无修改在解析、时间戳、写入权限检查、落库、统计、回放和上传前返回；主动选同类别可仅确认来源。非编辑调用保留原解析合同，仅补全等幂等返回。真实改备注的品牌解绑、兼容品牌、存储天气及照片语义处理保留。
- 输入法边界：三个备注入口共用 `CommittedRecordNoteField`，通过 `markedTextRange` 区分组词与已提交输入，分类选择/保存前同步结束编辑；仅打开关闭旧长备注不截断文本、不登记手写事件。控件保留动态字体和原编辑页对齐。清空金额与场景包的相邻独立合同不扩展处理。
- 测试及门禁变更：`StateRegressionTests.swift` 新增 16 项 XCTest（14 项连续意图、解析/解码编辑往返、来源/精度、最新行合并及候选事实，2 项 UIKit 组词/旧长备注）；当前未编译或执行。新增 `scripts/record_continuous_intent_regression.py` 并接入原 `validate_release_gate.py`，保留全部原门禁。`experience_static_check.ps1` 两条旧源码匹配改为核验共享构造器与冲突情绪弃用，追加委托构造检查，未删除原保护意图。无修改提前返回与各副作用顺序已做源码合同检查，实际写入/上传计数仍待 Swift 环境验证。
- 实际验证：四项既有专项 record_preview_feedback、quick_note、amount_input、meal_emotion 及新增 continuous_intent 源码专项均 PASS；当前 staging `58e4975` 工作区与真实 production `a6f0964` 隔离兼容副本分别完整执行 Windows release gate，最终均退出 0、`release_repository_gate: OK`，仅既有 7 条文案软提示。生产副本使用真实分支标识 `xuzhang1.0-release-2026`，未更新真实生产分支。额外语法解析未发现相对原快照新增诊断，不等同 Swift 编译/类型检查。
