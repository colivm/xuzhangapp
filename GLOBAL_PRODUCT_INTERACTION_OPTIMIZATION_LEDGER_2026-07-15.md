...
- 状态：`NOT_STARTED` → `IN_PROGRESS` → `CODE_DONE`；Windows 代码、语义、静态、文案与完整发布门禁均已通过，缺 macOS/Xcode 编译、XCTest 和 iPhone `FLOW-86` 签收，不标记 `VERIFIED`。当前无 `IN_PROGRESS`，`PERF-AUDIT-04` 与 `ARCH-03` 继续保持 `NOT_STARTED`。
