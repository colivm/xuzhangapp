...
- IAP 绑定：交易带 `appAccountToken` 且与当前账号匹配时以 Apple 为准；不带 token 且服务端已绑定到别的账号时，Production 和 Sandbox 都返回 `TRANSACTION_ALREADY_BOUND`，不会自动改绑。测试账号必须使用各自的专用沙盒 Apple ID；需要给另一个叙账账号独立开通时，应更换 Apple ID 后重新购买。
