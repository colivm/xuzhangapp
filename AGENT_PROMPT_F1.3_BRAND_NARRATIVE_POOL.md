...
```text
你在 xuzhangapp 实现 **Task F1.3 — 品牌叙事池**（仅 iOS；**F1.2 完成后**再开）。
...
## 必做
1. `MerchantBrandCatalog.swift` — 20 品牌 × 4 档 × 6～8 条生活句（非广告腔）
2. `NarrativeCopyResolver` — brand > ScenePack contextual > inferEmotionTag 兜底
3. `HomeItem.merchantBrandId: String?` — Codable 兼容
4. OCR import + 手动保存接入 brand match + emotion 重算
5. 改金额/分类时：有 merchantBrandId 仍走品牌池

## 禁止
Logo 图像匹配、B2.13 习惯引擎、web-preview、StoreKit、git commit（除非用户要求）

## 交付
1. 改动文件列表
2. 20 品牌 emotion 样例（每品牌 2 条示例即可，全文在 catalog）
3. 验收勾选
4. 未做：F1.3b Logo、B2.13 习惯预填
