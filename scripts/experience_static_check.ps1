...
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'let hasVisibleSnapshot = hasPreparedSnapshot \|\| tabState\.coldStartDisplay != nil[\s\S]{0,700}if hasVisibleSnapshot \{[\s\S]{0,180}updateTraceLoadingPresentation\(nil, animated: false\)' 'trace keeps visible old content without a blocking refresh overlay'
