...
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativeAIRewriteService.swift' 'ruleVersion = 4|signal\.kind != \.userText|具体原文只在本机展示' 'old weak narrative rewrites are invalidated and user expressions stay local'
...
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativeAIRewriteService.swift' 'mode: String|"relationship" : "factual"|relationshipClaimsAreSupported|relationshipKinds|ruleVersion = 4' 'narrative AI can only polish certified relationships or facts and preserves bounded claims'
...
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'LedgerDisplayFingerprintPolicy|trace-cold-start-v1|TraceColdStartDisplayStore|JSONEncoder|JSONDecoder|entryLimit' 'trace cold-start display cache is deterministic bounded and discardable'
