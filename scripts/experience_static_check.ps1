...
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'let journeyFact = effectiveNarrativeScope[\s\S]{0,1800}journeyFact: journeyFact' 'trace clue reuses the one computed journey identity'
...
Assert-MultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'guard persistItems\(upserting: \[resolved\]\) else \{ return false \}[\s\S]{0,160}itemDerivedCache = projectedItemDerivedCache' 'edited list projection publishes only after durable local persistence succeeds'
...
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testImmediateEditedItemProjectionKeepsListsCurrentUntilBackgroundPublication' 'edited list immediate projection ordering and membership XCTest coverage'
