...
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'visibleTraceLoadingPresentation|traceLoadingOverlay|AppColors\.bg[\s\S]{0,120}opacity\(0\.82\)|presentation: \.card|scrollDisabled\(visibleTraceLoadingPresentation != nil\)' 'trace loading uses one centered viewport overlay and blocks stale content interaction'
...
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'TraceLoadingPresentationPolicyTests|testInitialMonthTraceShowsOneImmediateAccuratePresentation|testRefreshWithExistingSnapshotDelaysTheSingleOverlay|testClueCopyUsesOneContinuousScopeRegardlessOfLifeRange|testContinuousClueKeyIgnoresLifePeriodAndCustomRangeState|testContinuousClueWindowIncludesCurrentAndTwelvePreviousWeeksOnly' 'trace loading presentation and continuous clue scope XCTest coverage'
...
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'hasCompleteSnapshot: hasPreparedSnapshot[\s\S]{0,240}if hasPreparedSnapshot \{[\s\S]{0,180}updateTraceLoadingPresentation\(nil, animated: false\)' 'trace cold-start summary stays behind the blocking viewport overlay until a complete snapshot is ready'
...
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'TraceSnapshotLifecycleTests|testChapterAndClueKeysReuseOnlyTheSameRealSourceState|testColdStartFingerprintIsStableAcrossOrderingAndChangesWithLedgerContent|testColdStartDisplaySurvivesStoreRecreationButRejectsAnotherContext' 'trace lifecycle key fingerprint and relaunch cache XCTest coverage'
...
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func traceChapterPreparation\([\s\S]{0,1500}prioritizeRecurringMarks: range == \.month[\s\S]{0,180}usesEchoAnchor: true' 'trace week and month prewarm computation uses the requested range only'
...
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'TraceDetailListSourcePolicy\.canReuseDerivedPeriodItems[\s\S]{0,260}ledgerRevision: homeViewModel\.homeDashboardRevision[\s\S]{0,180}derivedRevision: homeViewModel\.itemDerivedCacheRevision[\s\S]{0,650}sourceItems = homeViewModel\.items[\s\S]{0,260}traceDetailPresetInterval' 'trace detail rejects a stale derived collection after ledger revision advances'
