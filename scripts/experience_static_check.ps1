...
Assert-Pattern 'NativeDemoApp/Services/PlaybackService.swift' 'resolution\.isQualified \? label|"照片"|"这笔的一张照片。"|if !isQualified \{ value -= 22 \}' 'unclassified photos keep neutral playback labels and ranking'
...
Assert-MultilinePattern 'NativeDemoApp/Services/LifeMarkService.swift' 'containsAny\(normalized, \["出去玩", "出游", "游玩"\]\)[\s\S]{0,320}id: "travel"[\s\S]{0,220}supportsNounPhraseQuery: true' 'outgoing trip is a trusted read-only journey query phrase'
