...
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'let hasVisibleSnapshot = hasPreparedSnapshot \|\| visibleTraceColdStartDisplay != nil[\s\S]{0,700}if hasVisibleSnapshot \{[\s\S]{0,180}updateTraceLoadingPresentation\(nil, animated: false\)' 'trace only keeps a visible snapshot or cold-start display that matches the selected target'
