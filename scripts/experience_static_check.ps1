...
Assert-MultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'older failed write[\s\S]{0,500}retryUpserts[\s\S]{0,350}persistItems\(upserting: retryUpserts' 'stale persistence failures are re-submitted against the latest projection'
...
Assert-MultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'func syncCloudLedgerNow\(\) async \{[\s\S]{0,600}guard !isSyncingCloudLedger else \{ return \}[\s\S]{0,2200}CloudLedgerMergePolicy\.merge\([\s\S]{0,900}for item in mergeResult\.uploads' 'full sync is re-entrancy guarded and uploads only local-newer records'
