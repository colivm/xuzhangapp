...
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func handleSummaryPlaybackTap\([\s\S]{0,1700}preparedLifeSnapshot\(for: range\)\?\.periodFacts[\s\S]{0,900}preparedFacts\.matches\([\s\S]{0,350}sourceRevision: sourceRevision[\s\S]{0,180}isMember: hasMemberAccess[\s\S]{0,650}buildWeekSummary\([\s\S]{0,180}from: preparedFacts[\s\S]{0,500}buildMonthSummary\([\s\S]{0,180}from: preparedFacts' 'summary playback reuses only the currently visible matching trace facts'
