...
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativeEchoService.swift' 'case repeatRhythm|case returnAfterGap|case comparableChange|case contextReturn|case newContextPair|currentRows\.count >= 2' 'past echo is limited to evidence-backed kinds with a sparse-data abstention'
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativeEchoService.swift' 'signalKind != \.coffee|abs\(delta\) >= 2, relativeChange >= 0\.5|historyByDistance\[1, default: \[\]\]\.isEmpty' 'past echo suppresses routine coffee and requires return or change evidence'
...
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativeAIRewriteService.swift' 'redactedLabel|本机原文|有真实照片的记录|evidenceCount|itemIDsByFactID|signal\.kind != \.userText' 'narrative AI fact packs keep raw user expressions local and redact photos and ledger identifiers'
...
Assert-Pattern 'NativeDemoApp/Services/PlaybackService.swift' 'LifeNarrativeAIRewriteStore\.shared\.rewrite|narrativeRewrite\?\.summary \?\? narrativeEcho\?\.line' 'week month and share read validated narrative rewrites with evidence-backed local fallback'
...
Assert-NoMultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func traceChapterSnapshotCacheKey\([\s\S]{0,650}(customStartDate|customEndDate|selectedCategory)' 'life chapter cache does not depend on detail-only filters'
...
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceChapterPreparation|prewarmTraceChapter|prewarmRange|visiblePreparedLifeSnapshot|preparedLifeSnapshot' 'trace builds the exact selected chapter first and prewarms without cross-range display fallback'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'TraceLoadingPresentationPolicy|TraceClueScopePolicy|refreshDelayNanoseconds: UInt64 = 150_000_000|\u6B63\u5728\u6574\u7406\u672C\u6708\u75D5\u8FF9|\u6B63\u5728\u6574\u7406\u751F\u6D3B\u7EBF\u7D22' 'trace loading policy distinguishes life range and keeps clues continuous'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func tracePreparationStatus[\s\S]{0,1100}allowsHitTesting\(false\)' 'trace preparation is an inline non-hit-testing status instead of a blocking viewport overlay'
Assert-NoPattern 'NativeDemoApp/Views/StatsWebView.swift' 'scrollDisabled\(visibleTraceLoadingPresentation != nil\)|accessibilityHidden\(visibleTraceLoadingPresentation != nil\)|traceLoadingOverlay' 'trace preparation never disables scrolling or hides the usable first screen from VoiceOver'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'snapshotTransaction\.disablesAnimations = true|withTransaction\(snapshotTransaction\)|visibleTraceLoadingPresentation = nil' 'trace snapshots publish atomically without layout animation'
...
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'TraceLoadingPresentationPolicyTests|testInitialMonthTraceShowsOneImmediateAccuratePresentation|testRefreshWithExistingSnapshotDelaysTheInlineStatus|testClueCopyUsesOneContinuousScopeRegardlessOfLifeRange|testContinuousClueKeyIgnoresLifePeriodAndCustomRangeState|testContinuousClueWindowIncludesCurrentAndTwelvePreviousWeeksOnly' 'trace loading presentation and continuous clue scope XCTest coverage'
...
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'TraceChapterCoverPolicy\.make|monthDiaryAnchors: monthDiaryAnchors|selectionLimitIncludingCover' 'trace cover facts and independent month diary photos are prepared with the chapter snapshot'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'snapshot\.coverFacts\.title|traceLifeMonthCoverPhoto|traceLifeMonthCoverHeatmap|traceLifeMonthCoverMetrics|snapshot\.monthDiaryAnchors' 'trace week moment and month photo rhythm cover states are rendered'
...
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'TraceSnapshotVisibilityPolicy|representsSelectedLifeRange|canDisplayChapter|snapshotRange == selectedRange|publishedKey == expectedKey|pendingLifeChapterScrollRange' 'trace only displays the exact selected preset snapshot and persists deferred external scroll intent'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'tracePinnedControls|traceTargetPreparationSurface|schedulePendingTraceScrollIfPossible|pendingLifeChapterScrollRange' 'trace controls stay outside scrolling content and external scroll waits for the selected snapshot'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private var traceViewModeKicker:[\s\S]*?HStack\(spacing: 0\)[\s\S]*?\.frame\(height: 44\)' 'trace pinned mode control has an explicit bounded touch height'
Assert-NoMultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func traceViewModeTab[\s\S]{0,800}maxHeight:\s*\.infinity' 'trace pinned mode buttons never request infinite vertical space'
Assert-NoPattern 'NativeDemoApp/Views/StatsWebView.swift' 'desiredSnapshot|fallbackSnapshot|scrollAnchorID = "trace-mode-picker"' 'trace cannot restore cross-range chapter fallback or early mode-picker scrolling'
Assert-NoMultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'onChange\(of: traceViewMode\)[\s\S]{0,180}scrollAnchorID' 'ordinary trace mode switching cannot command scroll position'
Assert-NoMultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'Color\.clear[\s\S]{0,160}\.id\(traceViewMode == \.life \? "trace-life-card" : "trace-clue-board"\)' 'missing trace content cannot create an invisible scroll target'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testInitialEntryBuildsOnlyVisibleRangeThenPrewarmsTheOther|testSwitchingToMissingMonthDoesNotExposeWeekDuringPreparation|testSwitchingToMissingWeekDoesNotExposeMonthDuringPreparation|TraceSnapshotVisibilityPolicyTests|testLifeRangeMustMatchTheSelectedPresetPeriod|testChapterRequiresSelectedRangeAndExactPublicationKey' 'trace exact-range visibility and on-demand preparation regression coverage'
...
Assert-Pattern 'NativeDemoApp/Services/PlaybackService.swift' 'WeeklyShareCardSnapshot|weeklyShareLifeMark|prepareWeeklyShareCardSnapshot|buildWeeklyShareCardPayload\(\s*from preparedFacts' 'weekly share reuses one immutable period result and carries its ledger revision'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'SummaryPlaybackPreparationComputation|SummaryPlaybackPresentation|weeklyShareSnapshot|TraceInsightQuestionFocusPolicy' 'playback and trace interactions publish prepared immutable results'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'summaryPlaybackPresentation|SummaryPlaybackPreparationComputation\.build|presentation\.weeklyShareSnapshot\?\.payload|preparedClueSnapshot\?\.insight\.questionChips' 'trace playback sheet and follow-up interaction only read prepared snapshots'
Assert-NoPattern 'NativeDemoApp/Views/StatsWebView.swift' 'weeklySharePayload\(for:|traceLifeInsight\(from:|let chips = traceLifeInsight\.questionChips' 'trace redraw and follow-up taps cannot rescan the ledger'
Assert-Pattern 'NativeDemoApp/Views/InsightWebView.swift' 'WeeklyShareExportPreparationComputation\.build|LedgerSnapshotPublicationPolicy\.accepts|CoverExportCoordinator\.renderImage\(from: session\)' 'review share prepares immutable content off actor before main-thread rendering'
Assert-NoMultilinePattern 'NativeDemoApp/Views/InsightWebView.swift' 'private var insightSourceRevision:[\s\S]{0,240}Hasher\(' 'review revision no longer hashes the full ledger'
Assert-Pattern 'NativeDemoApp/Views/InsightWebView.swift' 'private var insightSourceRevision:[\s\S]{0,100}homeViewModel\.homeDashboardRevision|homeJourneyLedgerFacts\.totalCommittedRecordCount|onChange\(of: homeViewModel\.homeDashboardRevision\)' 'review uses the shared ledger revision and prepared availability facts'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testPreparedWeeklyShareSnapshotStaysBoundToPlaybackRevision|testSnapshotPublicationAndPreparedQuestionRotationRejectStaleWork|testWeeklyShareExportPreparesImmutableSessionBeforeRendering' 'PERF-AUDIT-04 snapshot reuse and stale revision XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-96|weeklyShareSnapshot|Main Thread Hitches|旧修订' 'PERF-AUDIT-04 device and Instruments regression matrix'
...
Assert-Pattern 'NativeDemoApp/Services/RecordCalendarContext.swift' 'enum CommuteEvidencePolicy|historicalDays\.count >= 2|cents\(_ amount: Double\)' 'commute recognition uses structured time and repeated historical evidence'
...
if ($aiSuggestionViewSource -notmatch '\.onAppear[\s\S]{0,500}prepareAICommandSuggestionsIfNeeded\(\)' -or
    $aiSuggestionViewSource -notmatch '\.onChange\(of: homeViewModel\.homeDashboardRevision\)[\s\S]{0,500}prepareAICommandSuggestionsIfNeeded\(\)') {
...
Assert-Pattern 'NativeDemoApp/Services/LifeMarkService.swift' 'PreparedAggregationContext|prepareAggregationContext|definitionIDsByItemID|historyItemsByDefinitionID|preparedContext:' 'home life marks can reuse one prepared history matching context'
Assert-NoMultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel+Dashboard.swift' 'static func lifeMarkSnapshot[\s\S]{0,7000}LifeMarkService\.aggregates\([\s\S]{0,180}allItems:' 'home life mark snapshot never rebuilds a full-ledger signature for each visible row'
Assert-Pattern 'NativeDemoApp/Services/FreeScenePackService.swift' 'prepareRewardDecision|rewardDecisionLock|prepareAggregationContext|preparedContext:' 'post-save scene reward and cold-start decisions are serialized and reuse signature-free aggregation'
Assert-Pattern 'NativeDemoApp/Views/RecordView.swift' 'LifeMarkSceneRewardPreparationInput|withTaskGroup|group\.addTask\(priority: \.utility\)|prepareRewardDecision' 'post-save scene reward preparation runs away from the main interaction thread'
Assert-NoMultilinePattern 'NativeDemoApp/Views/RecordView.swift' 'DispatchQueue\.main\.asyncAfter[\s\S]{0,700}LifeMarkService\.aggregates|DispatchQueue\.main\.asyncAfter[\s\S]{0,700}shouldShowColdStartGuide' 'post-save return path never aggregates life marks directly on the main queue'
Assert-NoMultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel+Dashboard.swift' 'var weekLifeThemeText:[\s\S]{0,260}LifeMarkService\.aggregates' 'home week theme getter only reads the prepared snapshot'
Assert-NoMultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel+Dashboard.swift' 'var quickRecordNudgeText:[\s\S]{0,420}LifeMarkService\.aggregates' 'home action subtitle getter only reads the prepared snapshot'
...
Assert-Pattern 'NativeDemoApp/Views/HomeView.swift' 'homeViewModel\.homeJourneyLedgerFacts\.allRecordDayCount' 'home life rhythm reads its prepared all-record day count'
Assert-NoPattern 'NativeDemoApp/Views/HomeView.swift' 'countDaysWithRecords\(\)' 'home rendering no longer scans every ledger item to count record days'
...
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testPreparedLifeMarkContextMatchesLegacyCombinedAndPerItemResults|allRecordDayCount' 'home life mark batching preserves legacy results and prepared record-day facts'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testSceneRewardColdStartPreparedAggregationPreservesLegacyDecision' 'post-save cold-start guidance preserves its previous eligibility result'
...
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-64|5,000 \u6761\u4FDD\u5B58\u540E|\u9010\u884C\u5386\u53F2\u626B\u63CF' 'home life mark nonblocking save-return device matrix'
...
Assert-Pattern 'NativeDemoApp/CoverEngine/Models/CoverContracts.swift' 'struct CoverFactPack|struct CoverContentAtom|struct ContentAllocationPlan|struct CoverRecipe|struct FooterRecipe' 'cover engine immutable fact atom allocation and recipe contracts'
Assert-Pattern 'NativeDemoApp/CoverEngine/Validation/CoverContractValidator.swift' 'enum ContentAllocationEngine|duplicateAtomConsumption|duplicateSemanticKey|duplicateVisibleText|consumedSetMismatch' 'cover allocation consumes each atom and semantic key at most once'
Assert-Pattern 'NativeDemoApp/CoverEngine/Validation/CoverContractValidator.swift' 'brandCountMismatch|footerMetricOutsideFooter|footerMetricSetMismatch|qrCodeNotVerified|blockedMedia|mediaRoleNotAllowed|mediaEvidenceMismatch|recipeIdentityMismatch' 'cover validator owns footer privacy evidence media and source identity rejection'
Assert-NoPattern 'NativeDemoApp/CoverEngine/Models/CoverContracts.swift' 'import SwiftUI|UIImage|ImageRenderer|WeeklyShareCardPayload|WeeklyStoryShareCardView|WeeklyShareCardView' 'cover fact allocation and recipe contracts remain Foundation-only'
Assert-NoPattern 'NativeDemoApp/CoverEngine/Validation/CoverContractValidator.swift' 'import SwiftUI|UIImage|ImageRenderer|WeeklyShareCardPayload|WeeklyStoryShareCardView|WeeklyShareCardView' 'cover contract validator remains detached from rendering and legacy views'
Assert-Pattern 'NativeDemoAppTests/CoverContractEngineTests.swift' 'testSameAtomCannotBeConsumedByTwoVisibleRegions|testDifferentAtomsWithTheSameSemanticKeyCannotBothBeVisible|testNormalizedDuplicateCopyCannotHideBehindDifferentIDs|testFooterMetricOutsideFooterAndConsumedSetDriftAreRejected|testFooterCannotDropOneOfTheCertifiedMetricAtoms|testQRCodeRequiresAnAllowedVerifiedHTTPSDestination|testBlockedMediaAndSecondaryOnlyHeroAreRejectedLocally|testHeroAndCaptionMustRemainBoundToTheirCertifiedEvidence|testRecipeCannotCrossFactRevisionPeriodOrFingerprint' 'cover contract uniqueness footer privacy evidence media and recipe XCTest coverage'
Assert-Pattern 'NativeDemoApp.xcodeproj/project.pbxproj' 'CoverContracts.swift in Sources|CoverContractValidator.swift in Sources|CoverContractEngineTests.swift in Sources' 'cover contract engine and tests wired to Xcode targets'
Assert-Pattern 'NativeDemoApp/CoverEngine/Models/CoverRenderingContracts.swift' 'final class PreparedCoverRenderInput|struct CoverShareSession|previewRenderInput|exportRenderInput|struct ResolvedCoverLayout|struct CoverRenderIdentity' 'cover render input and layout are immutable and shared by preview and export'
Assert-Pattern 'NativeDemoApp/CoverEngine/Flow/CoverShareFlow.swift' 'staleSourceRevision|layoutIdentityMismatch|bodyAtomSetMismatch|footerAtomInBody|mediaPresentationMismatch|preparedImageSetMismatch|makeFooterPresentation|joined\(separator:' 'cover preparation rejects stale identity body footer leakage and image drift'
Assert-Pattern 'NativeDemoApp/CoverEngine/Flow/LegacyWeeklyCoverAdapter.swift' 'LegacyWeeklyCoverSource|LegacyWeeklyCoverAdapter|ContentAllocationEngine\.allocate|LaunchCoverTemplateCatalog\.selectTemplateID|LaunchCoverTemplateLayoutResolver\.resolve|CoverShareFlow\.prepare|privacyRisk == \.safe' 'both legacy weekly sources enter the validated launch-template flow'
Assert-Pattern 'NativeDemoApp/CoverEngine/Rendering/CoverCanvasRoot.swift' 'struct CoverCanvasRoot|CoverTemplateBodyRenderer|GlobalFooterRenderer|CoverExportCoordinator|ImageRenderer|session\.exportRenderInput' 'one canvas root owns synchronous export and the global footer'
$coverRenderRootRaw = Get-Content -Raw -Path 'NativeDemoApp/CoverEngine/Rendering/CoverCanvasRoot.swift' -Encoding UTF8
$globalFooterCallCount = [regex]::Matches($coverRenderRootRaw, 'GlobalFooterRenderer\(').Count
if ($globalFooterCallCount -ne 1) {
    throw "GlobalFooterRenderer must be instantiated exactly once by CoverCanvasRoot; found $globalFooterCallCount"
}
Write-Output 'OK  CoverCanvasRoot instantiates the global footer exactly once'
Assert-NoPattern 'NativeDemoApp/CoverEngine/Rendering/CoverCanvasRoot.swift' 'URLSession|ProgressView|\.task\s*\{' 'cover render and export tree contains no network or asynchronous loading state'
Assert-Pattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'CoverShareSession|LegacyWeeklyCoverAdapter\.prepareSession|CoverCanvasRoot\(renderInput:|CoverExportCoordinator\.renderImage' 'playback preview and export use one prepared cover session'
Assert-Pattern 'NativeDemoApp/Views/InsightWebView.swift' 'LegacyWeeklyCoverAdapter\.prepareSession|CoverExportCoordinator\.renderImage|sourceRevision: sourceRevision' 'review export uses the same cover adapter flow and locked revision'
Assert-Pattern 'NativeDemoAppTests/CoverShareFlowTests.swift' 'testPreviewAndExportReceiveTheExactSameLockedRenderInput|testFoundationRootKeepsAllFooterAtomsOutOfTheTemplateBody|testFlowRejectsAStaleExpectedRevisionBeforeRendering|testFlowRejectsFooterAtomsPlacedInsideTheBody|testFlowRejectsLayoutMediaPresentationDriftFromRecipe|testBothLegacyEntrancesUseTheSameAdapterForNoPhotoAndPreparedPhotoInputs|testLegacyAdapterFiltersReceiptLikeMediaBeforeFactAndLayoutPreparation|testSynchronousExportUsesTheLockedCanvasWithoutAsyncLoaders' 'cover share flow identity footer adapter privacy and synchronous export XCTest coverage'
Assert-Pattern 'NativeDemoApp.xcodeproj/project.pbxproj' 'CoverRenderingContracts.swift in Sources|CoverShareFlow.swift in Sources|LegacyWeeklyCoverAdapter.swift in Sources|CoverCanvasRoot.swift in Sources|CoverShareFlowTests.swift in Sources' 'cover flow rendering adapter and tests wired to Xcode targets'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-73|PreparedCoverRenderInput|1080×1920' 'unified cover flow preview export footer and stale revision device matrix'
Assert-MultilinePattern 'NativeDemoApp/CoverEngine/Templates/LaunchCoverTemplates.swift' 'static let orderedTemplateIDs:[\s\S]*?\.heroStory,[\s\S]*?\.magazine,[\s\S]*?\.memoryFocus,[\s\S]*?\.journal,[\s\S]*?\.film,[\s\S]*?\.minimal,[\s\S]*?\.quote,[\s\S]*?\.timeline,[\s\S]*?\.postcard,[\s\S]*?\.scrapbook,[\s\S]*?\.editorial,[\s\S]*?\.memoryWall,[\s\S]*?\.travelNote,[\s\S]*?\.bookCover,[\s\S]*?\.natureDiary,[\s\S]*?\.coffeeStory,[\s\S]*?\.warmHome,[\s\S]*?\.nightStory,[\s\S]*?\.ocean,[\s\S]*?\.quietEditorial,[\s\S]*?\]' 'launch cover catalog exposes the ordered twenty-template contract'
Assert-Pattern 'NativeDemoApp/CoverEngine/Templates/LaunchCoverTemplates.swift' 'supportedPhotoCounts|maximumRenderedPhotoCount|requiresEvidenceBoundHero|requiredHeroOrientation|automaticSceneKeys|minimumRecordedDayCount|maximumLeadCharacterCount' 'twenty-template catalog keeps hard photo hero scene day and copy capability gates'
Assert-Pattern 'NativeDemoApp/CoverEngine/Templates/LaunchCoverTemplates.swift' 'selectTemplateID|manuallyAvailableTemplateIDs|allocationRequest|mediaRecipes|LaunchCoverTemplateLayoutResolver|arrangedMedia' 'twenty templates share one deterministic selector allocation media recipe and layout resolver'
Assert-NoPattern 'NativeDemoApp/CoverEngine/Templates/LaunchCoverTemplates.swift' 'Vision|CoreImage|CIImage|CIFilter|URLSession|AIReportService|AICoverDirector|WeeklyShareCardPayload|UIImage|SwiftUI' 'launch template catalog stays detached from scoring color AI legacy payload images and rendering views'
Assert-Pattern 'NativeDemoApp/CoverEngine/Rendering/CoverCanvasRoot.swift' 'CoverTemplateDecoration|case \.memoryFocus|case \.film|case \.postcard|case \.scrapbook|case \.editorial|case \.memoryWall|case \.travelNote|case \.bookCover|case \.natureDiary|case \.coffeeStory|case \.warmHome|case \.nightStory|case \.ocean|case \.quietEditorial' 'single cover body renderer preserves the additional fourteen visual grammars'
Assert-Pattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'selectedCoverTemplateID|manuallyAvailableTemplateIDs|shareCardStylePicker|dynamicTypeSize\.isAccessibilitySize|coverPickerAnimation|scrollTargetBehavior|accessibilityAddTraits' 'playback picker exposes structurally eligible twenty-template choices with narrow-screen accessibility and reduced-motion handling'
Assert-NoMultilinePattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'private func selectCoverTemplate[\s\S]{0,700}(LocalCoverMediaAnalyzer|MemoryAttachmentThumbnail|prepareShareCardPhotos|CoverAIDirectorCoordinator\.shared\.prepare|generateCoverDirectorDecision)' 'manual template switching performs no image IO analysis or AI request'
Assert-NoMultilinePattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'private func saveWeeklyStoryCard[\s\S]{0,1800}(LocalCoverMediaAnalyzer|MemoryAttachmentThumbnail|prepareShareCardPhotos|CoverAIDirectorCoordinator\.shared\.prepare|generateCoverDirectorDecision)' 'cover save performs no image IO analysis or AI request'
Assert-MultilinePattern 'NativeDemoAppTests/LaunchCoverTemplateTests.swift' 'testCatalogContainsExactlyTwentyCompleteUniqueTemplates[\s\S]*testManualEligibilityAndMediaRecipesCoverZeroThroughSevenPhotos[\s\S]*testAutomaticSceneCandidatesRemainSeparateFromManualCatalog[\s\S]*testSecondaryOnlyAndReceiptMediaNeverBecomeHero[\s\S]*testLongQuoteAndMinimalCopyFallBackToExpandedJournalLayout[\s\S]*testTimelineRequiresAtLeastThreeCertifiedRecordDays[\s\S]*testTwentyLayoutsKeepAtomMediaFooterAndCanvasContractsAligned[\s\S]*testBookCoverRequiresPortraitHeroAcrossSelectionAndRecipe' 'twenty-template catalog eligibility privacy fallback layout hierarchy footer and orientation XCTest coverage'
Assert-Pattern 'NativeDemoApp.xcodeproj/project.pbxproj' 'LaunchCoverTemplates.swift in Sources|LaunchCoverTemplateTests.swift in Sources' 'launch templates and tests wired to Xcode targets'
Assert-Pattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'private enum LifeSliceShareCardStyle|private struct WeeklyStoryShareCardView' 'legacy playback share styles and renderer remain available pending device verification'
Assert-Pattern 'NativeDemoApp/Views/InsightWebView.swift' 'struct WeeklyShareCardView' 'legacy review share renderer remains available pending device verification'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-77|20 \u5957|0/1/2/3/4/7|Dynamic Type|VoiceOver|Reduce Motion|\u8FDE\u7EED\u4FDD\u5B58 20 \u6B21' 'twenty-template capability accessibility rapid switching save and export device matrix'
Assert-Pattern 'NativeDemoApp/CoverEngine/Analysis/CoverMediaAnalysis.swift' 'import CoreImage|import Vision|VNDetectFaceRectanglesRequest|VNGenerateAttentionBasedSaliencyImageRequest|struct CoverMediaAnalysis|struct CoverCropSafety|struct CoverDynamicPalette' 'cover media analysis stays local and exposes quality crop safety and dynamic palette contracts'
Assert-Pattern 'NativeDemoApp/CoverEngine/Analysis/CoverMediaAnalysis.swift' 'actor LocalCoverMediaAnalyzer|maximumConcurrentAnalysisCount = 2|maximumCachedResultCount = 64|stableImageIdentity|currentVersion|withTaskGroup|Task\.isCancelled' 'cover media analysis is bounded cancellable and cached by stable image identity plus rule version'
Assert-NoPattern 'NativeDemoApp/CoverEngine/Analysis/CoverMediaAnalysis.swift' 'URLSession|VNRecognizeTextRequest|VNClassifyImageRequest|VNDetectHumanBodyPoseRequest|ageEstimate|gender|emotion' 'cover media analysis has no network OCR identity demographic or emotion inference'
Assert-Pattern 'NativeDemoApp/CoverEngine/Templates/LaunchCoverTemplates.swift' 'CoverMediaRoleScoring\.orderedHeroCandidates|CoverMediaRoleScoring\.orderedSecondaryCandidates' 'launch media roles use deterministic local quality and complement ordering'
Assert-Pattern 'NativeDemoApp/CoverEngine/Flow/LegacyWeeklyCoverAdapter.swift' 'analysis\?\.isHeroEligible|selectedAnalyses|dynamicPalette|mediaAnalysesByID' 'legacy adapter demotes unsafe hero candidates and locks analysis plus palette into preparation'
Assert-Pattern 'NativeDemoApp/CoverEngine/Flow/CoverShareFlow.swift' 'mediaAnalysisMismatch|invalidDynamicPalette|mediaAnalysesByID|dynamicPalette' 'cover preparation validates analysis identity and hero palette before locking render input'
Assert-Pattern 'NativeDemoApp/CoverEngine/Rendering/CoverCanvasRoot.swift' 'renderInput\.dynamicPalette|renderInput\.mediaAnalysesByID|CoverCropOffsetResolver\.offset' 'preview and export consume the same locked palette and crop focus'
Assert-NoMultilinePattern 'NativeDemoApp/CoverEngine/Rendering/CoverCanvasRoot.swift' 'private func coverImage\([\s\S]{0,2600}\.offset\([\s\S]{0,180}placement\.frame\.y[\s\S]{0,80}\.clipped\(\)' 'resolved media slots are not clipped again in their pre-offset local coordinate space'
Assert-Pattern 'NativeDemoAppTests/CoverShareFlowTests.swift' 'testExportKeepsAnOffsetMediaSlotVisibleInsideItsResolvedFrame|rgbaPixel' 'cover export has pixel-level coverage for an offset media slot'
Assert-Pattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'shouldPrewarmCoverShare|coverSharePrewarmKey|LocalCoverMediaAnalyzer\.shared\.analyze|analysesByAnchorID|cover-media-analysis-v|withTaskCancellationHandler|selectShareCardStyle|prepareCoverShareSession\(for: coverShareContextKey\)' 'playback final chapter prewarms one cancellable versioned result and template switching publishes without reanalysis flicker'
Assert-NoMultilinePattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'private func saveWeeklyStoryCard\([\s\S]{0,1800}(LocalCoverMediaAnalyzer|VNDetectFaceRectanglesRequest|CIContext)' 'cover save only renders the locked input and never analyzes media'
Assert-Pattern 'NativeDemoAppTests/CoverMediaAnalysisTests.swift' 'testEvidenceBoundHeroUsesHighestEligibleQualityScore|testLowResolutionFlatImageCannotBecomeHero|testStableImageIdentityCachesOnceAndRetargetsPaletteSource|testCropOffsetClampsProtectedFocusInsideFillOverflow|testDynamicPaletteRequiresHeroTextContrast' 'cover media quality cache role crop and palette XCTest coverage'
Assert-Pattern 'NativeDemoApp.xcodeproj/project.pbxproj' 'CoverMediaAnalysis.swift in Sources|CoverMediaAnalysisTests.swift in Sources' 'cover media analysis and tests wired to Xcode targets'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-75|Vision|Core Image|20 \u6B21|12MP' 'cover media scoring palette cancellation and performance device matrix'
Assert-Pattern 'NativeDemoApp/CoverEngine/Director/AICoverDirector.swift' 'struct CoverAIDirectorRequest|struct CoverAIDirectorResponse|decodeStrict|CoverAIDirectorRequestFactory|CoverAIDirectorValidator|CoverAIDirectorDecisionStore|actor CoverAIDirectorCoordinator' 'optional cover director has redacted contracts strict decoding local validation cache and coordinator'
Assert-Pattern 'NativeDemoApp/CoverEngine/Director/AICoverDirector.swift' 'maximumTemplateCandidateCount = 5|maximumCachedDecisionCount = 24|requestTimeoutNanoseconds: UInt64 = 8_000_000_000|withThrowingTaskGroup|inFlight|latestRequestID|activeRequestKey' 'cover director requests are bounded timed cancellable coalesced and stale-safe'
Assert-Pattern 'NativeDemoApp/CoverEngine/Director/AICoverDirector.swift' 'request\.mediaCandidates\.isEmpty \|\| !response\.mediaRoles\.isEmpty' 'cover director rejects zero media whenever safe candidates exist'
Assert-MultilinePattern 'NativeDemoApp/CoverEngine/Flow/LegacyWeeklyCoverAdapter.swift' 'currentDirectorInput\.request\.mediaCandidates\.isEmpty[\s\S]{0,100}\|\| !decision\.mediaSelections\.isEmpty[\s\S]{0,200}CoverAIDirectorValidator\.isStillValid' 'legacy adapter keeps a final zero-media AI rejection before local fallback'
Assert-NoPattern 'NativeDemoApp/CoverEngine/Director/AICoverDirector.swift' 'import UIKit|import SwiftUI|UIImage|URLSession|VNDetectFaceRectanglesRequest|CoverNormalizedRect|CoverRGBA|imageData|photoData|imageReference' 'cover director owns no images rendering network face rectangles or pixel palette samples'
Assert-Pattern 'NativeDemoApp/Services/AIReportService.swift' 'generateCoverDirectorDecision|"feature": "cover_director"|"directorRequest": directorPayload|timeoutInterval: 10|decodeStrict' 'iOS cover director uses the fixed authenticated proxy and strict response contract'
Assert-NoMultilinePattern 'NativeDemoApp/Services/AIReportService.swift' 'generateCoverDirectorDecision[\s\S]{0,1200}"messages"' 'iOS cover director cannot send free messages or client prompts'
Assert-Pattern 'ai-proxy/coverDirectorContract.js' 'COVER_DIRECTOR_RESPONSE_JSON_SCHEMA|additionalProperties: false|validateCoverDirectorRequest|buildCoverDirectorMessages|normalizeCoverDirectorResponse|mediaAlias|isEvidenceBoundToLead' 'proxy owns a closed cover director schema prompt and evidence-bound media validation'
Assert-Pattern 'ai-proxy/coverDirectorContract.js' 'rawRequest\.mediaCandidates\.length > 0 && object\.mediaRoles\.length === 0' 'proxy rejects zero media whenever the redacted request has safe candidates'
Assert-Pattern 'ai-proxy/server.js' 'cover_director_contract_rejected|buildCoverDirectorMessages|normalizeCoverDirectorResponse|AI_UPSTREAM_TIMEOUT_MS_COVER_DIRECTOR' 'proxy validates director input before the model and applies a dedicated hard timeout'
Assert-Pattern 'backend/src/server.js' 'isCoverDirectorRequest|AbortController|9_000|signal: abortController\.signal' 'authenticated backend bounds the cover director proxy hop without changing other AI requests'
Assert-Pattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'coverAIDirectorPrewarmKey|!showShareCardPrivacyConfirm|prepareCoverDirectorDecision|presentShareCardPrivacyConfirm|selectedShareCardStyle == nil|currentCoverDirectorDecision|cancelAndRejectPendingResult' 'cover AI prewarms only before preview and never overrides manual choice or a locked preview'
Assert-NoMultilinePattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'private func saveWeeklyStoryCard\([\s\S]{0,1800}(generateCoverDirectorDecision|CoverAIDirectorCoordinator|AIReportService)' 'cover save never starts or waits for AI direction'
Assert-Pattern 'NativeDemoAppTests/CoverAIDirectorTests.swift' 'testDirectorRequestContainsOnlyRedactedStructuralFactsAndAliases|testStrictResponseDecoderRejectsAdditionalFields|testValidatorRejectsStaleUnknownAndUnboundChoices|testValidAIDecisionStillPassesTheFullLocalRenderContract|testStaleDecisionFallsBackToTheCompleteLocalRecipe|testDecisionCacheIsBoundedAndUsesTheExactRequestKey' 'cover director redaction schema rejection fallback cache and full render XCTest coverage'
Assert-Pattern 'NativeDemoAppTests/CoverAIDirectorTests.swift' 'testZeroMediaDecisionFallsBackWhenSafePhotosExistButRemainsValidWithoutPhotos' 'cover director zero-media rejection local fallback and true no-photo XCTest coverage'
Assert-Pattern 'ai-proxy/coverDirectorContract.test.js' 'accepts a redacted bounded cover director request|rejects free text image fields identifiers and unknown request keys|normalizes a valid response and binds it to the exact request|rejects stale identities unknown tokens and unsupported media roles' 'cover director proxy contract has executable privacy and rejection coverage'
Assert-MultilinePattern 'ai-proxy/coverDirectorContract.test.js' 'publishes exactly the twenty template IDs[\s\S]*accepts bounded requests and responses for all twenty templates[\s\S]*memoryWall accepts four through six[\s\S]*quietEditorial accepts decoration only[\s\S]*bookCover accepts a portrait hero[\s\S]*allows media-6 variants and rejects media-7 variants' 'cover director proxy tests lock twenty templates media bounds decoration roles and portrait-only book cover'
Assert-Pattern 'ai-proxy/coverDirectorContract.test.js' 'rejects zero media when safe candidates exist and allows a true no-photo response' 'cover director proxy locks automatic photo use without breaking no-photo input'
Assert-Pattern 'NativeDemoApp.xcodeproj/project.pbxproj' 'AICoverDirector.swift in Sources|CoverAIDirectorTests.swift in Sources' 'cover director implementation and tests are wired to Xcode targets'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-76|AI \u5BFC\u6F14|\u6293\u5305|\u8D85\u65F6|\u65E7\u4FEE\u8BA2|20 \u6B21' 'cover director privacy fallback locking and performance device matrix'
...
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativePlanningService.swift' 'case \.receipt: return 18|case nil:[\s\S]{0,180}userExpressionValue\(for: item\) >= 82 \? max\(base, 82\) : 18' 'receipt and unqualified legacy photos cannot become narrative leads'
...
Assert-Pattern 'NativeDemoApp/Services/PlaybackService.swift' 'allowedKinds: \[\.userText, \.photo, \.change\]' 'week and month playback representative items exclude composition-only scene signals'
...
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativePlanningService.swift' 'specificExpressionTerms|isHighConfidenceManualExpressionSource|item\.source == \.manual|item\.userEditedCategory != true|informationGain >= 82 && signal\.narrativeValue >= 82|case \.structuredScene:[\s\S]{0,40}return false|periodRecordTitle|withoutTerminalPunctuation' 'edited-title and imported-title metadata cannot replace information gain or composition-only abstention'
Assert-NoPattern 'NativeDemoApp' '有 1 条用户主动写下的记录|主动记录' 'internal edited-title provenance never becomes a visible narrative count'
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativeAIRewriteService.swift' 'ruleVersion = 5|signal\.kind != \.userText|具体原文只在本机展示' 'old weak narrative rewrites are invalidated and user expressions stay local'
Assert-Pattern 'NativeDemoApp/Services/PlaybackService.swift' 'preferredItemID: UUID\? = nil|leftPreferred|rightPreferred|preferredItemID: narrativeRepresentative\?\.id' 'trace and playback memory anchors can bind the primary visual to a narrative lead'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'preferredNarrativeAnchorItemID|lead\.kind == \.photo \|\| lead\.kind == \.userText \|\| lead\.kind == \.change' 'trace cover prefers a qualified photo from the current narrative evidence'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'return insight\.leadQuestion|return insight\.previewLine' 'clue overview projects evidence copy instead of duplicating the life-card rewrite'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceLifeVisualCaption|本周画面|本月画面|lead\.evidenceItemIDs\.contains\(anchor\.itemID\)' 'non-lead photos are explicitly presented as independent period visuals'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testEditedTitleMetadataDoesNotBecomeAVisibleNarrativeCount|testImportedTitlesCannotBecomeUserExpressionLeads|testSpecificUserExpressionCanLeadWithoutTurningIntoMetadataCopy|testNestedWeekAndMonthUseTheSameUniqueExpressionEvidence|testNarrativePhotoLeadIsAlsoThePrimaryVisibleAnchor|testUserExpressionLeadStaysLocalInsteadOfBecomingAnAICountFact' 'active-record regression imported-title boundary nested-scope visual evidence and local-expression XCTest coverage'
...
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativeEchoService.swift' 'contextReturnCandidate|newContextPairCandidate|currentDays\.count >= 2|activeHistory\.count >= 4|pairOrder|isHighConfidenceLateCommute' 'high-value relationships require independent days history and certified ordering'
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativeEchoService.swift' 'LifeNarrativeSignalPolicy\.isAdministrativeRecord|signalKind != \.coffee|!activeHistory\.contains|historicalDistinctDayCount|baselinePeriodCount' 'administrative routine and previously seen combinations cannot become relationship stories'
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativePlanningService.swift' 'hasNarrativeLead|relationshipEcho|relationshipSignal|case \.rhythm:[\s\S]{0,40}return false|lead\?\.id\.hasPrefix\("echo:"\)' 'narrative planner promotes only certified leads and explicitly abstains from rhythm fallback'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'relationshipEvidenceLines|历史基线覆盖|上一组可核对记录|relationshipEcho: relationshipEcho' 'trace life and clue surfaces project current and historical evidence from one relationship plan'
Assert-Pattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'isRelationshipLead|leadSignalID\?\.hasPrefix\("echo:"\)|title = narrativeHeadline' 'share projection keeps the concise certified relationship headline above its evidence summary'
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativeAIRewriteService.swift' 'mode: String|"relationship" : "factual"|relationshipClaimsAreSupported|relationshipKinds|ruleVersion = 5' 'narrative AI can only polish certified relationships or facts and preserves bounded claims'
Assert-Pattern 'ai-proxy/narrativeRewriteContract.js' 'ALLOWED_MODES|RELATIONSHIP_KINDS|contextReturn|newContextPair|relationshipClaimsAreSupported|受控历史窗口的限定不能省略' 'narrative proxy rejects unsupported relationship upgrades and unbounded first claims'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testLateCommuteReturnsOnlyWithTwoCurrentAndHistoricalDays|testParkingAndTravelDoNotBecomeLateCommuteContext|testNewCoffeeLateCommutePairNeedsTwoDaysAndFourActiveHistoryWeeks|testNewPairAbstainsForOneDayOrWhenHistoryAlreadyContainsThePair|testMixedPairOrderNeverClaimsAfter|testRelationshipIdentityIsSharedByLifeCardClueAndShareProjection|testWeakCompositionAndRhythmDoNotStartRemoteNarrativeRewrite|testRelationshipFactPackUsesCertifiedModeAndKeepsBoundedClaims|testRelationshipDiscoveryStaysDeterministicAtAllReleaseScales' 'relationship thresholds abstention cross-surface identity AI claims and release-scale XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-63|晚间通勤|新组合|主动弃权|关系 ID|5,000' 'evidence relationship and abstention device performance matrix'
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativeEchoService.swift' 'maximumHistoryPeriodDistance = 12|PeriodBucketRow|\(0\.\.\.maximumHistoryPeriodDistance\)\.contains\(distance\)|sceneByItemID|scene\(for:' 'weekly trace echo bounds history and classifies each retained record once'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testEchoIgnoresRowsOutsideTwelvePeriodWindowAtReleaseScale|0\.\.<5_000|XCTAssertEqual\(withAncientHistory, baseline\)' 'weekly trace bounded history equivalence release-scale XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-97|12 个周期|5,000|历史回声|正在整理本周痕迹' 'weekly trace bounded history device performance matrix'
...
Assert-Pattern 'backend/src/auth.js' 'createRequireAuth|getUserById\(userId\)|ACCOUNT_NOT_FOUND' 'protected backend routes reject a valid token after its account is deleted'
Assert-Pattern 'backend/scripts/verify-deleted-account-token.mjs' 'users\.delete\(user\.userId\)|statusCode === 401|ACCOUNT_NOT_FOUND' 'deleted-account token invalidation has executable regression coverage'
...
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'mutating func openLifeChapter|lifeCardRange = range|selectedPeriod = range == \.week \? \.week : \.month|pendingLifeChapterScrollRange = range' 'trace range routing records a deferred target instead of scrolling before publication'
...
try {
    if (Get-Command npm -ErrorAction SilentlyContinue) {
        ...
        $aiProxyContractOutput = node --test (Get-ChildItem -LiteralPath . -Filter '*.test.js' | ForEach-Object FullName)
    }
...
Assert-Pattern 'NativeDemoApp/Views/SettingsView.swift' '陪伴设置|天气会结合当天的天气|基础陪伴' 'companion setting explains its real product scope'
Assert-NoPattern 'NativeDemoApp/Views/SettingsView.swift' '复盘语气|语气偏好|webToneButton\(' 'obsolete tone controls are not exposed in settings'
...
Assert-NoMultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceLifeMonthDiaryStrip[\s\S]{0,500}snapshot\.memoryAnchors' 'monthly diary cannot reuse the three-anchor cover pool'
...
Assert-MultilinePattern 'NativeDemoApp/Services/LifeMarkService.swift' 'private static func semanticText\(for item: HomeItem\) -> String \{(?:(?!displayEmotionTag)[\s\S])*?\n    \}' 'life mark fact matching excludes generated emotion copy'
Assert-MultilinePattern 'NativeDemoApp/Services/LifeSceneSemanticService.swift' 'private static func semanticText\(item: HomeItem, brand: MerchantBrandDefinition\?\) -> String \{(?:(?!displayEmotionTag)[\s\S])*?\n    \}' 'life scene fact matching excludes generated emotion copy'
Assert-Pattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'OCRCommuteScenePolicy\.inferredScenePackID|scenePackId: scenePackId' 'OCR import persists evidence-gated commute scene identity'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'LifeMarkFactAuthorityTests|testGeneratedDisplayCopyDoesNotCreateGroceryOrSocialFacts|testTrustedTitleBrandAndScenePackStillCreateFacts|testOCRTransitRouteUsesWorkdayHistoryInsteadOfExactAmount|testExplicitCommuteWinsButSingleNonWorkdayTransitDoesNot' 'fact authority and OCR commute XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-65|金额/进站口|系统展示文案|真实盒马|100/1,000/5,000' 'fact authority and OCR commute device regression matrix'
Assert-MultilinePattern 'NativeDemoApp/Services/PhotoMemoryPromptPolicy.swift' 'private static func semanticText\(for item: HomeItem\) -> String \{(?:(?!displayEmotionTag)[\s\S])*?\n    \}' 'photo role inference excludes generated display copy'
Assert-Pattern 'NativeDemoApp/Services/PhotoMemoryPromptPolicy.swift' 'static func anchorReason\(for item: HomeItem\) -> PhotoMemoryPromptReason\?|explicitReceiptKeywords|resolvedAnchorRole|isQualified: false|refreshedAutomaticAnchorMetadata|isAutomaticallyAssignedAnchor' 'photo roles require evidence and automatic metadata can be reevaluated'
Assert-Pattern 'NativeDemoApp/Services/PlaybackService.swift' 'resolution\.isQualified \? label|"照片"|PhotoMemoryPromptPolicy\.unclassifiedAnchorCaption|if !isQualified \{ value -= 22 \}' 'unclassified photos keep neutral playback labels and ranking'
Assert-Pattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'PhotoMemoryPromptPolicy\.refreshedAutomaticAnchorMetadata|if let reason = PhotoMemoryPromptPolicy\.anchorReason' 'record editing and cover selection respect evidence-based photo roles'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'TracePhotoEvidenceBindingPolicy|anchor\?\.itemID|photoEvidenceItem|insight\.highlightedItemID' 'trace photo category and clue evidence bind to the exact record id'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceCluePhotoEvidence|photoEvidenceItem: snapshot\.photoEvidenceItem|照片对应记录|TracePhotoEvidenceBindingPolicy\.primaryCategory' 'trace shows the bound photo title amount and correct category icon'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'PhotoMemoryFactBindingTests|testUnknownCoffeeAndRoutineTransitPhotosStayUnclassified|testReceiptAndExperienceRolesRequireMatchingEvidence|testEditingReevaluatesAutomaticRoleButPreservesExplicitMetadata|testPrimaryPhotoCategoryAndClueEvidenceUseTheExactItemID|testUnclassifiedPhotoInsightNamesTheRecordWithoutInventingAScene' 'photo evidence role edit and exact-id XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-66|餐饮主图|交通角标|可乐/咖啡|照片缩略图|编辑标题' 'photo role binding and evidence device regression matrix'
Assert-MultilinePattern 'NativeDemoApp/Services/PhotoMemoryPromptPolicy.swift' 'roadTravelEvidenceKeywords[\s\S]{0,800}sceneHint: \.travelTransport[\s\S]{0,180}assetRole: \.place|containsAny\(text, roadTravelEvidenceKeywords\)[\s\S]{0,350}sceneHint: \.travelTransport[\s\S]{0,180}assetRole: \.place' 'toll photos bind to route context before generic vehicle context'
Assert-MultilinePattern 'NativeDemoApp/Services/PhotoMemoryPromptPolicy.swift' 'static func automaticAnchorCaption\([\s\S]{0,1600}和这段路一起留下[\s\S]{0,1000}unclassifiedAnchorCaption' 'automatic photo copy describes record relationships instead of image contents'
Assert-NoPattern 'NativeDemoApp' 'return "这次买的东西。"' 'trace and playback cannot restore the invented purchased-object caption'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'PhotoMemoryPromptPolicy\.automaticAnchorCaption' 'trace snapshot consumes the shared automatic photo copy'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'PhotoMemoryPromptPolicy\.automaticAnchorCaption' 'month diary consumes the shared automatic photo copy'
Assert-MultilinePattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'lifeSliceResolvedPhotoCaption\([\s\S]{0,500}PhotoMemoryPromptPolicy\.resolvedAnchorCaption' 'summary playback consumes the shared resolved photo copy'
Assert-NoPattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'fallback(?:Caption)?:.*这次买的东西' 'summary templates cannot restore the purchased-object fallback'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceLifeResolvedAnchorCaption\([\s\S]{0,400}PhotoMemoryPromptPolicy\.resolvedAnchorCaption' 'trace display consumes the shared resolved photo copy'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'item\.category\.rawValue.*item\.amount\.formatted' 'month diary keeps the record category beside its amount'
Assert-MultilinePattern 'NativeDemoApp/Services/LifeMarkService.swift' 'queryIntent\(from text: String\)[\s\S]{0,650}跨城路线[\s\S]{0,220}definitionQueryIntent\([\s\S]{0,120}id: "travel"' 'generated cross-city route suggestion is a recognized travel query'
Assert-MultilinePattern 'NativeDemoApp/Views/InsightWebView.swift' 'intent\.id == "travel"[\s\S]{0,180}跨城路线[\s\S]{0,180}return "跨城路线"' 'cross-city route query keeps a truthful result label'
Assert-MultilinePattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testTollPhotoUsesRouteRelationshipWithoutInventingItsContents[\s\S]{0,2600}和这段路一起留下' 'toll photo relationship XCTest coverage'
Assert-MultilinePattern 'NativeDemoAppTests/StateRegressionTests.swift' '这段跨城路线是怎样连起来的[\s\S]{0,900}过去 31 天的跨城路线记录' 'generated journey suggestion XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-92|过路费.*家人照片|这次买的东西|跨城路线是怎样连起来的' 'toll photo copy and journey suggestion device regression matrix'
Assert-Pattern 'NativeDemoApp/Services/NarrativeCopyResolver.swift' 'TrustedUserMomentNarrativePolicy|item\.hasMemoryImages|item\.source == \.manual|item\.userEditedTitle == true|下班路上，也把这一刻留了下来|preferredNarrative' 'trusted user photo moment copy requires local factual evidence'
Assert-MultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel+Dashboard.swift' 'trustedUserMoment[\s\S]{0,500}todayPrimaryLine|todayUserMomentLine[\s\S]{0,220}todayLateCommuteLine[\s\S]{0,220}todayLifeMarkLine' 'today story prioritizes fresh trusted expression before aggregate marks'
Assert-Pattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'TrustedUserMomentNarrativePolicy\.emotionTag\(for: resolved\)|TrustedUserMomentNarrativePolicy\.emotionTag\(for: updated\)|trustedMomentTagBeforeRemoval|memory-remove' 'record edit photo attach and final photo removal refresh the trusted moment emotion tag'
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativePlanningService.swift' 'TrustedUserMomentNarrativePolicy\.line\(for: item\)|value \+= 34|fact: fact' 'narrative planning promotes the certified local moment without changing life mark priority'
Assert-Pattern 'NativeDemoApp/Services/PlaybackService.swift' 'trustedMomentCaption|trustedMomentLine|TrustedUserMomentNarrativePolicy\.line\(for: item\)' 'photo anchors and playback record copy use the trusted moment line'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'narrativeInsightTitle[\s\S]{0,900}TrustedUserMomentNarrativePolicy\.line\(for: item\)' 'trace clue headline projects the same trusted moment from its evidence item'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'TrustedUserMomentNarrativeTests|testDownWorkPhotoUsesTheRealMomentBeforeTheLateNightThreshold|testMomentProjectionRequiresManualUserTextAndAnActualPhoto|testTodayMomentOutranksStableCoffeeWithoutChangingLifeMarkPriority|testTrustedMomentBecomesTheNarrativeLeadInsteadOfRoutineCoffee|testPhotoAnchorUsesMomentCopyInsteadOfGenericUtilityCaption' 'trusted moment priority safety and cross-surface XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-67|20:43|稳定咖啡|也把这一刻留了下来|真实照片|用户原话' 'trusted user moment narrative device regression matrix'
Assert-MultilinePattern 'NativeDemoApp/Views/InsightWebView.swift' 'private func reviewOverviewDeltaBadge[\s\S]{0,900}Button \{\s+openReviewTask\(\.compare\)[\s\S]{0,900}打开做对比并填入最近七天与前七天' 'review overview delta is an accessible compare entry'
Assert-Pattern 'NativeDemoApp/Models/InteractionStateModels.swift' 'containsMatchedRollingDayPeriods|rollingDayCounts|rollingDayNumber|currentCounts\.intersection\(previousCounts\)' 'AI recognition supports matched recent-N and previous-N day pairs'
Assert-MultilinePattern 'NativeDemoApp/Models/InteractionStateModels.swift' 'ReviewTaskResolutionPolicy[\s\S]{0,400}case \.compare:[\s\S]{0,60}return \.compare[\s\S]{0,120}case \.commuteDraft:[\s\S]{0,60}return \.backfill' 'review task state follows final recognition intent'
Assert-Pattern 'NativeDemoApp/Views/InsightWebView.swift' 'resolvedReviewTaskIntent\(for: command\)|let resolvedTaskIntent = engine\.resolvedReviewTaskIntent|activeReviewTask = resolvedTaskIntent|aiCommandResolvedReviewTaskForTesting' 'AI command commits the recognized task before execution'
Assert-NoPattern 'NativeDemoApp/Views/InsightWebView.swift' 'aiCommandTask\(for kind: AICommandKind\)|if let resolvedTask = aiCommandTask' 'AI result kind no longer overwrites the recognized review task'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testMatchedRollingDayPeriodsResolveCompareFromAQueryTask|testFinalRecognitionOwnsTaskStateInsteadOfThePreviousSelection|testRollingDayComparisonUsesCurrentAndImmediatelyPreviousWindows' 'rolling comparison and task transition XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-68|橙色差额|最近 N 天|前 N 天|查记录.*做对比|对比最近 7 天和前 7 天的消费' 'review compare entry intent and state device regression matrix'
Assert-MultilinePattern 'NativeDemoApp/Views/InsightWebView.swift' 'aiCommandHasExplicitTimeRange[\s\S]{0,1800}过去一年[\s\S]{0,120}今年[\s\S]{0,120}去年' 'AI query recognition treats rolling and natural years as explicit time ranges'
Assert-Pattern 'NativeDemoApp/Views/InsightWebView.swift' 'aiCommandRecentMonthRange\(months: 12, label:' 'AI rolling-year query uses a twelve-month window'
Assert-MultilinePattern 'NativeDemoApp/Views/InsightWebView.swift' 'containsAny\(text, \["今年"[\s\S]{0,450}dateInterval\(of: \.year[\s\S]{0,1500}AICommandTimeRange\(label: "去年"' 'AI natural-year queries use current and previous calendar-year boundaries'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testYearPhrasesResolveAsExplicitQueryRanges|testRollingAndNaturalYearRangesUseTheirOwnCalendarBoundaries' 'rolling and natural year query XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-69|过去一年/近一年/最近一年|今年|去年|滚动 12 个月|自然年' 'year query range device regression matrix'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'TraceSnapshotLifecycleKeyPolicy|ledgerRevision|dayKey|preparedWeekSnapshotKey|preparedMonthSnapshotKey|preparedClueSnapshotKey|chapterContentRevision|clueContentRevision' 'trace snapshot validity lives with the shared tab lifecycle and real revisions'
Assert-NoPattern 'NativeDemoApp/Views/StatsWebView.swift' '@State private var weekTraceNeedsRefresh|@State private var monthTraceNeedsRefresh|@State private var clueTraceNeedsRefresh|@State private var lifeInsightRefreshID|onChange\(of: homeViewModel\.items\)|traceItemsSignature' 'trace page reconstruction cannot reset refresh truth or rebuild content signatures'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'LedgerDisplayFingerprintPolicy|trace-cold-start-v2|TraceColdStartDisplayStore|JSONEncoder|JSONDecoder|entryLimit' 'trace cold-start display cache is deterministic bounded and discardable'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'restoreTraceColdStartDisplayIfNeeded[\s\S]{0,1400}TraceColdStartDisplayStore\.shared\.entry' 'trace restores a matching cold-start display before preparing'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private var visibleTraceFirstScreenPresentation[\s\S]{0,1300}TraceFirstScreenPresentationPolicy\.exactCache[\s\S]{0,900}TraceFirstScreenPresentationPolicy\.loadedLedgerFacts' 'trace stage one prefers an exact cached summary and otherwise exposes only loaded-ledger facts'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func traceFirstScreenCard[\s\S]{0,2600}TraceFirstScreenRecordEntryPolicy\.title[\s\S]{0,1700}openTraceDetail\(\)' 'trace organizing first screen exposes a scoped record entry without changing clue mode'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func prepareTraceDetailListForPresentation[\s\S]{0,1300}buildTraceDetailList\(input\)[\s\S]{0,900}traceDetailListPreparationGate\.accepts' 'trace first-screen record entry prepares its list off the main render path and rejects stale results'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'private var traceDetailListLoadingState' 'trace record entry uses an inline list loading state'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'if isPreparingTraceDetailList[\s\S]{0,500}traceDetailFocusedList\(snapshot: snapshot\)' 'trace detail keeps direct ledger records visible while the refresh pass runs'
Assert-NoPattern 'NativeDemoApp/Views/StatsWebView.swift' 'hasPreparedSnapshot \|\| visibleTraceColdStartDisplay' 'trace cold-start summary cannot masquerade as a complete visible snapshot'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'TraceSnapshotLifecycleTests|TraceFirstScreenProgressivePolicyTests|testChapterAndClueKeysReuseOnlyTheSameRealSourceState|testColdStartFingerprintIsStableAcrossOrderingAndChangesWithLedgerContent|testColdStartDisplaySurvivesStoreRecreationButRejectsAnotherContext' 'trace lifecycle key fingerprint relaunch cache and progressive first-screen XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-70|Tab 返回零重复准备|冷启动展示缓存|真实失效键|缓存损坏|100/1,000/5,000' 'trace cache lifecycle device regression matrix'
Assert-Pattern 'NativeDemoApp/ContentView.swift' '@StateObject private var lifetimeArchiveStore = LifetimeArchiveSnapshotStore\(\)|environmentObject\(lifetimeArchiveStore\)|task\(id: homeViewModel\.homeDashboardRevision\)|lifetimeArchiveStore\.prepareIfNeeded' 'member lifetime archive prewarms in the shared app content lifecycle'
Assert-NoPattern 'NativeDemoApp/Views/MemberPricingView.swift' '@State private var lifetimeArchiveSnapshot|@State private var lifetimeArchiveSnapshotRevision|@State private var lifetimeArchivePreparationTask|LifetimeArchiveSnapshot = \.empty|lifetimeArchiveSnapshotRevision = nil' 'member sheet no longer owns or clears the archive snapshot lifecycle'
Assert-MultilinePattern 'NativeDemoApp/Views/MemberPricingView.swift' 'if let lifetimeArchiveSnapshot = lifetimeArchiveStore\.snapshot[\s\S]{0,300}lifetimeArchivePreparingSection' 'member archive renders prepared data separately from a missing snapshot'
Assert-Pattern 'NativeDemoApp/Views/MemberPricingView.swift' '首次准备|不会用 0 条或 0 天冒充' 'member archive missing state never presents fake zero metrics'
Assert-Pattern 'NativeDemoApp/Views/MemberPricingView.swift' 'LifetimeArchiveCacheStore|LifetimeArchiveCacheContext|LedgerDisplayFingerprintPolicy|replacingSourceRevision|JSONEncoder|JSONDecoder|preparedEmpty\(sourceRevision:' 'member archive has a discardable fingerprinted relaunch cache and real empty state'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testArchiveDiskCacheSurvivesRecreationAndRejectsAnotherLedgerOrDay|testSharedArchiveStorePublishesARealPreparedEmptySnapshot' 'member archive relaunch cache and prepared-empty XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-71|长期档案|后台预热|不得先显示假 0|旧内容承接|缓存损坏|100/1,000/5,000' 'member lifetime archive cache device regression matrix'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-72|跨周期正文兜底|灵动岛|待定位意图|连续切换 20 次' 'trace selected-range safe-area and deferred-scroll device regression matrix'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'mutating func selectViewMode\(_ mode: TraceViewMode\)[\s\S]{0,320}guard mode == \.life[\s\S]{0,180}selectedPeriod = TraceRangeContextPolicy\.period\(for: lifeCardRange\)[\s\S]{0,120}useCustomRange = false[\s\S]{0,120}showsCustomDatePanel = false' 'trace life mode normalizes clue-only year and custom range state'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'static func chapterKey\([\s\S]{0,320}"chapter-v3"[\s\S]{0,220}range\.rawValue[\s\S]{0,220}String\(contentRevision\)' 'trace chapter identity is derived from the requested life range and real source revisions'
Assert-NoMultilinePattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'static func chapterKey\([\s\S]{0,520}(selectedPeriod|usesCustomRange)' 'trace chapter identity cannot depend on the currently selected clue filters'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'static func continuousClueKey[\s\S]{0,700}TraceClueScopePolicy\.identifier[\s\S]{0,700}windowKey' 'continuous clue identity has one bounded rolling scope and no life period'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private var traceClueItems[\s\S]{0,900}TraceClueScopePolicy\.items[\s\S]{0,260}from: homeViewModel\.items' 'clue items use the continuous bounded window instead of the life period'
Assert-NoMultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceClueHeroCard[\s\S]{0,220}traceRangeKicker' 'continuous clue hero has no week or month range control'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'continuousClueColdStartScopeKey|periodLabel: TraceClueScopePolicy\.periodLabel|scope: TraceClueScopePolicy\.scope' 'continuous clue cold start and computation use one scope'
Assert-NoMultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'if traceViewMode == \\.clues[\s\S]{0,240}discardPreparedClueSnapshot[\s\S]{0,240}selectedPeriod' 'life period changes cannot invalidate the continuous clue snapshot'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func traceChapterProgressiveInput\([\s\S]{0,1500}prioritizeRecurringMarks: range == \.month[\s\S]{0,240}usesEchoAnchor: true' 'trace week and month progressive computation uses the requested range only'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'struct TraceFirstScreenCacheIdentity' 'trace first-screen cache identity contract exists'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'enum TraceFirstScreenPresentationPolicy' 'trace first-screen presentation contract exists'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'currentDisplayPolicyVersion' 'trace first-screen display policy is versioned'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'enum TraceFirstScreenRecordEntryPolicy' 'trace record entry policy exists'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'struct TraceProgressivePreparationIdentity' 'trace progressive preparation identity contract exists'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'enum TraceProgressivePublicationPolicy' 'trace progressive publication contract exists'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'enum TraceProgressiveLifecyclePolicy' 'trace progressive lifecycle contract exists'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'static func buildProgressiveChapter[\s\S]{0,950}input\.allItems[\s\S]{0,500}static func buildProgressiveClue[\s\S]{0,500}TraceClueScopePolicy\.items' 'trace period and continuous-window filtering run inside the background computation path'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'static func buildClueIfCurrent[\s\S]{0,700}observesCancellation[\s\S]{0,3500}guard !shouldStop\(\) else \{ return nil \}' 'trace clue computation checks cooperative cancellation between expensive stages'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'Task\(priority: \.userInitiated\)[\s\S]{0,250}if hasCompleteSnapshot[\s\S]{0,180}traceCoalescingDelayNanoseconds[\s\S]{0,1500}buildProgressiveTrace' 'trace starts a missing current snapshot immediately at user-initiated priority'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'TraceProgressivePublicationPolicy\.accepts[\s\S]{0,1000}storeChapterSnapshot[\s\S]{0,650}storeClueSnapshot' 'trace validates request revision scope member day and content identity before every primary cache write'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'Task\(priority: \.utility\)[\s\S]{0,1400}prewarmTraceClue[\s\S]{0,1800}prewarmTraceChapter' 'trace uses one sequential utility task for clue and remaining-range prewarm'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' '@Environment\(\\\.scenePhase\) private var scenePhase' 'trace observes the foreground and background scene phase'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' '@State private var lastResumedTracePreparationIdentity' 'trace owns the last resumed progressive identity'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func handleTraceScenePhaseChange[\s\S]{0,1500}TraceProgressiveLifecyclePolicy\.identityToResume[\s\S]{0,700}lastResumedTracePreparationIdentity = resumedIdentity' 'trace foreground lifecycle resumes the latest identity only once'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'final class TraceFirstScreenProgressivePolicyTests' 'trace progressive first-screen XCTest suite exists'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testEmptyAndSmallLedgersAlwaysHaveAnInteractiveFirstScreen' 'trace empty and small-ledger first-screen XCTest coverage'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testTraceRecordEntryIsLimitedToTheLifeFirstScreen' 'trace organizing record entry scope XCTest coverage'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testCachedFirstScreenRequiresExactFingerprintDayMemberScopeAndPolicy' 'trace exact first-screen cache identity XCTest coverage'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testStageTwoPublicationRejectsEveryStaleIdentityDimension' 'trace stale stage-two publication XCTest coverage'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testBackgroundCancelsAndForegroundResumesOnlyOneLatestIdentity' 'trace lifecycle resume XCTest coverage'
Assert-MultilinePattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testProgressiveClueMatchesDirectFullComputationAtReleaseScales[\s\S]{0,300}\[0, 5, 100, 1_000, 5_000\]' 'trace clue release-scale progressive equivalence XCTest coverage'
Assert-MultilinePattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testProgressiveWeekAndMonthChaptersMatchDirectFullComputationAtReleaseScales[\s\S]{0,300}\[0, 5, 100, 1_000, 5_000\][\s\S]{0,250}\[SummaryPlaybackRange\.week, \.month\]' 'trace week and month release-scale progressive equivalence XCTest coverage'
Assert-MultilinePattern 'NativeDemoApp/Services/AnalyticsService.swift' 'import os\.signpost[\s\S]{0,500}enum TraceFirstScreenPerformanceSignpost[\s\S]{0,1200}name: "TraceFirstInteractive"[\s\S]{0,500}stage=first-interactive[\s\S]{0,1600}name: "TraceFullReady"[\s\S]{0,500}stage=full-ready' 'trace first-interactive and full-ready Instruments signposts exist'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func scheduleTracePreparation\(\)[\s\S]{0,3000}TraceFirstScreenPerformanceSignpost\.firstInteractive[\s\S]{0,5000}TraceFirstScreenPerformanceSignpost\.fullReady' 'trace preparation emits both performance stages from one monotonic start'
Assert-MultilinePattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-106[\s\S]{0,1100}first-interactive[\s\S]{0,100}full-ready[\s\S]{0,300}Time Profiler[\s\S]{0,150}Main Thread Hitches[\s\S]{0,150}Allocations/Memory Graph[\s\S]{0,150}Energy Log[\s\S]{0,1500}10 分钟' 'trace two-stage first-screen device and Instruments matrix'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func traceViewModeTab\(_ mode: TraceViewMode\)[\s\S]{0,420}tabState\.selectViewMode\(mode\)' 'trace mode control uses the normalized shared state transition'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func schedulePendingTraceScrollIfPossible\(\)[\s\S]{0,1600}requiresAnchorReset[\s\S]{0,420}scrollAnchorID = nil[\s\S]{0,180}await Task\.yield\(\)[\s\S]{0,850}pendingLifeChapterScrollRange == pendingRange[\s\S]{0,850}scrollAnchorID = targetAnchorID' 'repeated trace deep links reset yield revalidate and reissue the same anchor without animation'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testSelectingLifeNormalizesClueOnlyRangesToTheRememberedLifeRange|testSelectingCluesDoesNotRewriteTheExistingRangeState|TraceDeferredScrollPolicyTests|testRepeatedTargetRequiresResetBeforeReissuingTheAnchor|testMissingOrDifferentTargetDoesNotNeedAnAnchorReset' 'trace mode normalization and repeated-anchor XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' '自定义日期/本年|预热快照直接复用|同一外部深链|无长期骨架' 'trace mode prewarm and repeated deep-link device boundaries'
Assert-Pattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' '@Environment\(\\\.scenePhase\) private var scenePhase|SummaryPlaybackSceneLifecyclePolicy|handleScenePhaseChange|suspendCoverWorkForInactiveScene' 'summary playback owns explicit foreground and background lifecycle policy'
Assert-MultilinePattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'StoryDynamicBackdrop\([\s\S]{0,180}isActive: scenePhase == \.active' 'summary playback dynamic canvas is suspended outside the active scene'
Assert-MultilinePattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'private func suspendCoverWorkForInactiveScene\(\)[\s\S]{0,900}preparedShareCardPhotos = nil[\s\S]{0,400}CoverAIDirectorCoordinator\.shared\.cancelAndRejectPendingResult' 'background transition releases prepared cover images and cancels AI work'
Assert-MultilinePattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' 'private func prepareShareCardPhotos\(for sourceKey: String\) async[\s\S]{0,160}scenePhase == \.active[\s\S]{0,80}sceneAllowsCoverWork' 'cover image decoding cannot start while playback is inactive'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'SummaryPlaybackSceneLifecyclePolicyTests|testPlaybackResumesOnlyWhenTheActiveSessionWasInterrupted|testCoverPrewarmRequiresAnActiveSceneAndAnEnabledGeneration|testCoverPrewarmStaysDeferredBeforeTheLastChapter' 'summary playback lifecycle XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-78|播放完成页|前后台往返 20 次|动态背景|图片分析|Jetsam' 'summary playback foreground recovery device matrix'
Assert-Pattern 'NativeDemoApp/Services/OCRService.swift' 'OCRDateEvidencePolicy|excludingLines: \[selected\.line\.text\]|isMonetaryLine|explicitDateLabels' 'OCR date parsing rejects the selected amount line and requires explicit date evidence'
Assert-NoPattern 'NativeDemoApp/Services/OCRService.swift' '\(\?:20\\d\{2\}\[-/\.年\]\)\?\\d\{1,2\}\[-/\.月\]\\d\{1,2\}' 'OCR no longer treats every bare decimal-like token as an unlabeled date'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'OCRDateEvidencePolicyTests|testPaymentAmountCannotBecomeADateWhenTheScreenshotHasNoDateEvidence|testCurrencyAndAmountLabelsAreRejectedAsDateCandidates|testExplicitAndDateLabeledDatesRemainSupported' 'OCR amount-versus-date and explicit-date XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-79|LAWSON.*¥4\.20|支付金额 4\.20|使用导入当天|金额行本身不提供日期' 'OCR amount-versus-date device regression matrix'
Assert-Pattern 'NativeDemoApp/Services/OCRService.swift' 'OCRCategoryEvidencePolicy|semanticValueLabels|blockedSemanticValues|static func semanticText\(title: String, rawText: String\)' 'OCR category evidence policy isolates trusted product and merchant fields'
Assert-MultilinePattern 'NativeDemoApp/Services/OCRService.swift' 'private func inferOCRCategory\(title: String, rawText: String\)[\s\S]{0,500}OCRCategoryEvidencePolicy\.resolve' 'all OCR category inference uses the evidence policy with a legacy fallback'
Assert-MultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'private func reviewedOCRDraft\(_ draft: OCRReceiptDraft\)[\s\S]{0,900}OCRCategoryEvidencePolicy\.resolve[\s\S]{0,180}return reviewed' 'OCR draft review rechecks category from trusted evidence'
Assert-NoMultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'private func reviewedOCRDraft\(_ draft: OCRReceiptDraft\)[\s\S]{0,800}HomeItem\(' 'OCR draft review does not build a semantic HomeItem from all OCR text'
Assert-Pattern 'NativeDemoApp/Services/LifeSceneSemanticService.swift' 'containsStrongCommuteCue|有限公司|股份有限公司|有限责任公司|个体工商户' 'legal entity suffixes cannot become commute evidence'
Assert-NoPattern 'NativeDemoApp/Services/LifeSceneSemanticService.swift' 'containsAny\(explicitText, strongCommuteCues\)' 'commute classification uses the suffix-aware evidence guard'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'OCRCategoryEvidencePolicyTests|testProductAndMerchantEvidenceWinsOverPaymentProcessorMetadata|testLineSeparatedTrustedValuesStayAndMetadataRowsStayOut|testLegalEntitySuffixDoesNotCreateCommuteSceneButRealCompanyContextDoes' 'OCR payment metadata and legal entity commute boundary XCTest coverage'
Assert-Pattern 'NativeDemoApp/Resources/RecordSceneLexicon.regression.json' 'ocr-wonton-payment-processor-is-metadata|巧婆红汤馄饨|拉卡拉支付股份有限公司' 'OCR payment processor regression fixture'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-100|馄饨.*拉卡拉|商品/商户证据优先|收单机构' 'OCR product evidence priority device regression matrix'
Assert-Pattern 'NativeDemoApp/Services/PlaybackSupportServices.swift' 'PlaybackLateWorkCommutePolicy|preferredStrongItem|lateWorkCommutePlaybackTitle\(for: \$0\) == "晚下班路上"|auxiliaryEmotionLabel' 'playback guarantees only evidence-backed work commute facts'
Assert-MultilinePattern 'NativeDemoApp/Services/PlaybackService.swift' 'let representative = narrativeRepresentative[\s\S]{0,160}lateWorkCommuteItem[\s\S]{0,140}selection\.primary' 'weekly playback preserves a higher narrative lead before the late-work fallback'
Assert-MultilinePattern 'NativeDemoApp/Services/PlaybackService.swift' 'private func playbackRecordCopy\([\s\S]{0,700}lateWorkCommutePlaybackLine[\s\S]{0,420}mainLine: lateWorkCommuteLine' 'playback representative records consume the dedicated late-work copy'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testStrongLateWorkCommuteBecomesWeeklyRepresentativeAndUsesDedicatedCopy|testHigherValueLeadKeepsItsWeeklyVoiceAndLateWorkCommuteMovesToOpeningSupport|testOrdinaryLateTransitDoesNotReceiveTheStrongWorkCommuteGuarantee|testStrongLateWorkCommuteUsesTheMatchingMonthlyHalfChapter' 'late-work playback selection copy and ordinary-transit XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-80|00:08.*加班打车.*¥50\.90.*真实照片|专用正文|不抢 Lead|普通夜间地铁/公交' 'late-work playback week and month device regression matrix'
Assert-Pattern 'NativeDemoApp/Services/NarrativeCopyResolver.swift' 'DiningCopyEvidencePolicy|stableRecordSeed|stableDiningSeed|convenienceStoreEmotionTag|genericEmotionTag|shouldReplaceStoredTag' 'dining copy uses one evidence policy and stable record context'
Assert-Pattern 'NativeDemoApp/Models/HomeItem.swift' 'shouldReplaceStoredTag|recordID: item\.id|specificEmotionTag' 'legacy dining tags and existing records consume the same evidence policy'
Assert-NoPattern 'NativeDemoApp/Services/MerchantBrandCatalog.swift' '一口热食很及时|饭团和饮料在手边|便当让中午落下|忙里拿点吃的' 'convenience fallback pools cannot invent food from a merchant name'
Assert-NoPattern 'NativeDemoApp/Services/MerchantBrandCatalog.swift' '这顿热乎热闹|早上吃了口热的|今天从热餐开始|忙里吃了顿热的|热乎的一顿|想吃口热饭|忙完吃了顿热饭|热乎水饺记下|热乎一碗落下|忙完吃了顿热乎的' 'dining brand emotion pools avoid unsupported temperature claims'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'DiningCopyEvidencePolicyTests|testBrandOnlyLawsonUsesNeutralStableVarietyWithoutInventingFood|testExplicitConvenienceFoodEvidenceKeepsTheSpecificNeutralLabel|testLegacyUnsupportedHeatTagIsCorrectedFromTheRecordEvidence|testConvenienceBrandFallbackPoolsContainOnlyMerchantLevelFacts|testDiningBrandFallbackPoolsDoNotAddUnsupportedTemperatureClaims' 'dining evidence deterministic variety and legacy correction XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-81|至少 8 笔同名“罗森”|零“热食/热乎/一口热的”|同一 item ID|删除证据后立即回到中性商户事实' 'dining copy evidence and relaunch device regression matrix'
Assert-Pattern 'NativeDemoApp/Views/SettingsView.swift' 'SettingsBackupSummaryPolicy|自动备份已开启 · 联网整理已开启|自动备份已开启|联网整理已开启|仅保存在本机|金额、分类、备注和日期会自动备份；照片仍保存在本机。' 'backup status and detail copy use natural language'
Assert-NoPattern 'NativeDemoApp' '账单字段|账单字段云端开|联网整理开|联网梳理已开|仅本地保存' 'iOS user copy removes cloud field jargon and terse switch labels'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'SettingsBackupCopyPolicyTests|testAllBackupAndOnlineOrganizationStatesUseNaturalCopy' 'all backup and online organization copy states have XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-84|自动备份已开启 · 联网整理已开启|仅保存在本机|删除云端备份' 'backup copy device regression matrix'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private struct TraceSwipeRow[\s\S]{0,900}@GestureState private var dragTranslation[\s\S]{0,2200}simultaneousGesture\(swipeGesture\)' 'trace swipe drag state stays local to one detail row'
Assert-NoPattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceSwipeDragState|traceDeletingItemID' 'trace detail has no parent per-frame swipe or delayed deletion state'
Assert-NoPattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'TraceSwipeDragState' 'obsolete shared trace swipe state is removed'
Assert-NoMultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func deleteRecord\([\s\S]{0,900}DispatchQueue\.main\.asyncAfter' 'trace detail deletion has no fixed animation delay'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func deleteRecord\([\s\S]{0,500}homeViewModel\.deleteItem\(id: item\.id\)[\s\S]{0,450}TraceDetailListSnapshotComputation\.deleting[\s\S]{0,450}traceDetailListSnapshot = updatedSnapshot' 'trace detail deletion immediately commits stable ID and publishes one coherent snapshot'
Assert-MultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'func deleteItem\(id: UUID\) -> Bool[\s\S]{0,220}deleteItems\(ids:[\s\S]{0,800}persistItems\(deleting: existingIDs\)' 'view model deletes persisted records by stable UUID'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func traceDetailListPreparationInput\([\s\S]{0,500}let ledgerRevision = homeViewModel\.homeDashboardRevision[\s\S]{0,1200}TraceDetailListSourcePolicy\.canReuseDerivedPeriodItems\([\s\S]{0,160}ledgerRevision: ledgerRevision[\s\S]{0,160}derivedRevision: homeViewModel\.itemDerivedCacheRevision[\s\S]{0,700}sourceItems = homeViewModel\.items[\s\S]{0,180}traceDetailPresetInterval' 'trace detail rejects a stale derived collection after ledger revision advances'
...
Assert-MultilinePattern 'NativeDemoApp/Services/PlaybackService.swift' 'struct PeriodExperienceFacts:[\s\S]{0,900}sourceRevision:[\s\S]{0,180}isMember:[\s\S]{0,500}periodStart:[\s\S]{0,180}periodEnd:[\s\S]{0,700}func matches\([\s\S]{0,500}isMember == expectedMembership' 'period experience facts carry ledger period and membership identity'
Assert-MultilinePattern 'NativeDemoApp/Services/LifeMarkService.swift' 'static func preparedAggregateSets\([\s\S]{0,1000}let rows = preparedAggregateRows\([\s\S]{0,900}visible: rankedAggregates\([\s\S]{0,350}isMember: visibleIsMember[\s\S]{0,350}member: rankedAggregates\([\s\S]{0,350}isMember: true' 'life marks derive trace-visible and playback-member sets from one prepared candidate collection'
Assert-MultilinePattern 'NativeDemoApp/Services/PlaybackService.swift' 'func preparePeriodExperienceFacts\([\s\S]{0,4200}prepareAggregationContext[\s\S]{0,900}preparedAggregateSets[\s\S]{0,450}visibleLimit: 8[\s\S]{0,180}memberLimit: 24' 'period fact preparation builds one life mark context and preserves the original visibility limits'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'static func buildChapter\([\s\S]{0,1400}preparePeriodExperienceFacts[\s\S]{0,500}let rawMarks = periodFacts\.lifeMarks[\s\S]{0,650}let narrativePlan = periodFacts\.narrativePlan[\s\S]{0,1800}periodFacts: periodFacts' 'trace chapter snapshot owns the reusable period fact sheet'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'enum SummaryPlaybackPreparationComputation[\s\S]{0,1000}preparedFacts\.matches\([\s\S]{0,350}sourceRevision: input\.sourceRevision[\s\S]{0,180}isMember: input\.isMember[\s\S]{0,800}buildWeekSummary\([\s\S]{0,180}from: periodFacts[\s\S]{0,500}buildMonthSummary\([\s\S]{0,180}from: periodFacts' 'summary playback reuses only the currently visible matching trace facts'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testTraceChapterCarriesReusableFactsWithoutChangingFreeLifeMarks|testPreparedWeekFactsReusePreservesPlaybackOutputAndIdentity|testPreparedMonthFactsReusePreservesPlaybackOutputAndRejectsNextMonth' 'period fact equivalence membership and invalidation XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-87|周期事实底稿|连续打开/关闭同周期周记或月章 20 次|100/1,000/5,000|不再次扫描完整历史|润色晚到' 'trace and playback period fact reuse device regression matrix'
Assert-MultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel+Dashboard.swift' 'struct HomeLifeMarkSemanticSignature:[\s\S]{0,1200}categoryCorrectionFrom:[\s\S]{0,500}draftStatus:' 'home life mark rows store a bounded semantic signature'
Assert-MultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel+Dashboard.swift' 'static func retainedVisibleLines\([\s\S]{0,900}previousSignature == nextSignature' 'home life mark rows retain copy only when the exact record semantics are unchanged'
Assert-MultilinePattern 'NativeDemoApp/Views/RecordEditSheet.swift' 'enum RecordEditCategoryMutationPolicy[\s\S]{0,500}currentTitle' 'manual category mutation policy preserves the current title'
Assert-MultilinePattern 'NativeDemoApp/Views/RecordEditSheet.swift' 'private func selectCategory[\s\S]{0,500}titleAfterSelectingCategory' 'the record editor uses the title-preserving category policy'
Assert-MultilinePattern 'NativeDemoApp/Services/LifeMarkService.swift' 'struct LifeJourneyFact:[\s\S]{0,900}routeCities:[\s\S]{0,900}isRoadTrip:[\s\S]{0,500}isClosedLoop:' 'journey facts retain route road and closure identity'
Assert-MultilinePattern 'NativeDemoApp/Services/LifeMarkService.swift' 'let isRoadTrip = !tollRows\.isEmpty \|\| energyRows\.count >= 2[\s\S]{0,700}guard !activityRows\.isEmpty' 'journey facts require road and away-activity evidence instead of amount guessing'
Assert-Pattern 'NativeDemoApp/Services/LifeMarkService.swift' 'static func allFacts\(' 'journey service exposes all certified journeys'
Assert-Pattern 'NativeDemoApp/Services/LifeMarkService.swift' 'static func primaryFact\(' 'primary journey compatibility accessor remains available'
Assert-MultilinePattern 'NativeDemoApp/Services/PlaybackService.swift' 'struct PeriodExperienceFacts:[\s\S]{0,1200}journeyFact:' 'period facts retain the certified local journey fact'
Assert-MultilinePattern 'NativeDemoApp/Services/PlaybackService.swift' 'return PeriodExperienceFacts\([\s\S]{0,800}journeyFact: journeyFact' 'period fact preparation publishes the one computed journey fact'
Assert-Pattern 'NativeDemoApp/Services/PlaybackService.swift' 'preparedFacts\.journeyFact\?\.line' 'week and month playback outros reuse local journey wording'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'let detectedJourneyFacts:[\s\S]{0,1800}journeyFact = detectedJourneyFacts.first[\s\S]{0,1800}journeyFact: journeyFact' 'trace clue reuses the one computed journey identity'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'isJourneyLead[\s\S]{0,1800}theme = \.relation' 'trace clue renders journey route evidence as a relationship'
Assert-Pattern 'NativeDemoApp/Services/LifeNarrativeAIRewriteService.swift' 'ruleVersion = 5' 'journey privacy invalidates older remote narrative cache keys'
Assert-MultilinePattern 'NativeDemoApp/Services/LifeNarrativeAIRewriteService.swift' 'plan\.leadSignalID == journeyFact\?\.id[\s\S]{0,900}signal\.id\.hasPrefix\("journey:"\)' 'cross-city route and evidence stay local and cannot be replaced by remote narrative copy'
Assert-Pattern 'NativeDemoApp/Services/LifeMarkService.swift' 'returnCompletionGraceInterval: TimeInterval = 3 \* 60 \* 60' 'journey return completion uses a fixed short grace window'
Assert-MultilinePattern 'NativeDemoApp/Services/LifeMarkService.swift' 'let nextDeparture[\s\S]{0,500}let completionAnchor = returnCompletionAnchor\([\s\S]{0,900}completionAnchor: completionAnchor' 'journey closure checks one bounded explicit return anchor before publishing the candidate'
Assert-MultilinePattern 'NativeDemoApp/Services/LifeMarkService.swift' 'private static func returnCompletionAnchor\([\s\S]{0,900}min\(graceDeadline, journeyDeadline\)[\s\S]{0,260}item\.createdAt >= nextDeparture\.createdAt[\s\S]{0,260}calendar\.isDate\(item\.createdAt, inSameDayAs: homeArrival\.createdAt\)[\s\S]{0,300}tollKeywords[\s\S]{0,180}longDistanceTransitKeywords[\s\S]{0,180}hasExplicitHomeDestination' 'journey return completion stays within the journey and home run and requires same-day travel evidence'
Assert-MultilinePattern 'NativeDemoApp/Services/LifeMarkService.swift' 'private static func hasExplicitHomeDestination[\s\S]{0,1200}negativeCues[\s\S]{0,500}onwardCues[\s\S]{0,300}"后去"[\s\S]{0,120}"开往"[\s\S]{0,300}\["回", "返", "到", "到达", "抵达"\][\s\S]{0,700}!isNegated, !continuesAway' 'journey return completion binds a non-negated final destination to the inferred home city and rejects a stated onward trip'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' '到南京后去宿迁的过路费|到南京后开往宿迁的过路费' 'journey return completion rejects direct and driving onward-destination wording'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testExplicitReturnRoadAnchorAfterOrdinaryHomeRowsCompletesTheJourney|testExplicitReturnDestinationSupportsArrivalWording|testReturnCompletionRejectsWrongDirectionAndLateAnchors|testReturnCompletionStopsBeforeTheNextDeparture|testReturnCompletionCannotInvalidateAJourneyAtTheMaximumDurationBoundary|testReturnCompletionRejectsAnExplicitTitleWithoutAHomeCityArrival|testReturnCompletionDoesNotCrossTheCalendarDayBoundary|testDelayedRailReturnCompletesWithoutInventingARoadTrip|testRemovingDelayedReturnAnchorRestoresTheCityBoundaryWithoutStaleEvidence' 'delayed journey return anchor and strict-boundary XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-105|过路费回南京|到南京的过路费|从南京回宿迁|3 小时|下一次离城' 'delayed journey return anchor device regression matrix'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testLifeMarkRefreshDropsOnlyEditedOrDeletedRowsBeforeAsyncRebuild|testClosedWeekendRoadTripIsOneCertifiedFactAcrossClueChapterAndPlayback|testJourneyLanguageRequiresRoadReturnAndAwayActivityEvidence|testJourneyFactsStayLocalAndDeterministicAtReleaseScales|testFlowerClamAndChickenFeetPhrasesResolveToDining|testManualCategoryChangePreservesTheUsersOriginalTitle|testSaturdayLateDiningNeedsExplicitWorkEvidenceBeforeSayingOvertime' 'journey semantic edit cache privacy and release-scale XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-88|南京 → 宿迁 → 连云港 → 宿迁 → 南京|周末跨城自驾|不得出现“老家”|花甲鸡爪|旧“超市买菜和家用”' 'journey and single-record semantic consistency device regression matrix'
Assert-Pattern 'NativeDemoApp/Views/Components/MemoryAttachmentViews.swift' 'cacheCountLimit = 40|cacheCostLimitBytes = 32 \* 1024 \* 1024|thumbnailMaxPixelSize = 480|originalDisplayMaxPixelSize = 1_600|originalPagePreloadRadius = 1|MemoryAttachmentImageLoader' 'memory attachment display decoding has bounded pixels cache and serialized loading'
Assert-NoPattern 'NativeDemoApp/Views/Components/MemoryAttachmentViews.swift' 'Task\.detached|kCGImageSourceThumbnailMaxPixelSize: 900|128 \* 1024 \* 1024|countLimit = 96' 'memory attachment views cannot launch unstructured full-size decode bursts'
Assert-MultilinePattern 'NativeDemoApp/Views/Components/MemoryAttachmentViews.swift' 'private func memoryHeroImage[\s\S]{0,450}shouldLoadOriginalPage[\s\S]{0,700}variant: \.original' 'photo detail loads only the selected and adjacent display pages'
Assert-Pattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'ItemDerivedCachePublicationPolicy[\s\S]*120_000_000|LedgerBackgroundComputationLane|ledgerDisplayFingerprint' 'rapid ledger revisions coalesce and serialize full-ledger derived work'
Assert-Pattern 'NativeDemoApp/ViewModels/HomeViewModel+Dashboard.swift' 'homeSnapshotCoalescingDelayNanoseconds|buildHomeLifeMark|buildHomeQuickRecord' 'home life and quick-record snapshots share the bounded ledger computation lane'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceCoalescingDelayNanoseconds|buildTraceChapter|buildTraceClue|ForEach\(group\.items\)' 'trace preparation is coalesced serialized and avoids per-render row tuple arrays'
Assert-NoPattern 'NativeDemoApp/Views/StatsWebView.swift' 'ForEach\(Array\(group\.items\.enumerated\(\)\)' 'trace detail does not allocate a full enumerated row array during rendering'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'chapterCacheLimit = 2|clueCacheLimit = 4|trace-cold-start-v2' 'trace full snapshots and order-independent cold-start identity are memory bounded'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testRapidInteractionMemoryPolicyBoundsDisplayImagesSnapshotsAndCoalescing|testRapidStableIDDeletionUpdatesCountTotalAndDayGroupsWithoutResurrection' 'rapid interaction memory policy and incremental deletion XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-89|9 图|Memory Graph|Allocations|Jetsam|连续.*20' 'rapid interaction memory and freeze device regression matrix'
Assert-MultilinePattern 'NativeDemoApp/Services/LifeMarkService.swift' 'containsAny\(normalized, \["出去玩", "出游", "游玩", "跨城路线", "跨城行程"\]\)[\s\S]{0,320}id: "travel"[\s\S]{0,220}supportsNounPhraseQuery: true' 'outgoing trip is a trusted read-only journey query phrase'
Assert-MultilinePattern 'NativeDemoApp/Views/InsightWebView.swift' 'intent\.id == "travel"[\s\S]{0,350}LifeJourneyFactService\.primaryFact[\s\S]{0,350}journey\.evidenceItemIDs[\s\S]{0,300}evidenceIDs\.contains' 'outgoing trip query reuses certified journey evidence IDs'
Assert-MultilinePattern 'NativeDemoApp/Views/InsightWebView.swift' 'aiCommandRequiresCertifiedJourney\(command\)[\s\S]{0,100}matchedItems = \[\]' 'cross-city route recommendation abstains without a certified journey'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testAICommandOutgoingTripQueryReusesCertifiedJourneyEvidence' 'outgoing trip recognition evidence identity and abstention XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-90|出去玩|journey evidence IDs|普通周末餐饮|100/1,000/5,000' 'outgoing trip query device regression matrix'
Assert-MultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'enum ItemDerivedCacheImmediateMutationPolicy[\s\S]{0,2600}todayPositiveItems[\s\S]{0,700}currentWeekItems[\s\S]{0,700}currentMonthItems[\s\S]{0,700}currentYearItems' 'edited items immediately project into every home list backed by the derived cache'
Assert-MultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'func persistItems\([\s\S]{0,1200}LocalStore\.saveHomeItemChanges[\s\S]{0,900}publishImmediateItemDerivedProjection' 'edited list projection publishes only after durable local persistence succeeds'
Assert-NoPattern 'NativeDemoApp/Services/RecordMemoryContextService.swift' 'existingItems' 'record emotion enrichment does not carry or scan unused full-ledger history'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testImmediateLedgerMutationProjectionAddsAndRemovesHomeRows' 'edited list immediate projection ordering and membership XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-91|列表.*详情|连续编辑 20 次|Main Thread Hitches|100/1,000/5,000' 'edited list publication and hitch device regression matrix'
Assert-NoPattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'baseEmotionTag: resolution\.emotionTag,\s*excluding: updated\.id' 'record memory calls match the history-free emotion enrichment signature'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'if lockedPreview != nil' 'locked trace preview uses a boolean presence check without an unused binding'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'let isMember = hasMemberAccess[\s\S]{0,1200}SummaryPlaybackPreparationInput\([\s\S]{0,500}isMember: isMember[\s\S]{0,500}group\.addTask' 'summary background work captures a main-actor membership snapshot'
Assert-MultilinePattern 'NativeDemoApp/Views/SummaryPlaybackSheet.swift' '@MainActor\s+private var customShareBackgroundCard[\s\S]{0,450}let accent = AppColors\.accent[\s\S]{0,220}return ZStack' 'custom background sendable label receives main-actor theme snapshots'
Assert-NoPattern 'NativeDemoApp/Services/LedgerSyncService.swift' 'case \.badStatus\(let code, let body\)' 'ledger sync error copy does not bind unused response values'
Assert-Pattern 'NativeDemoApp/Services/UserContentRiskService.swift' 'if riskReason\(for: normalized\) != nil' 'manual note risk validation avoids an unused reason binding'
Assert-NoPattern 'NativeDemoApp/CoverEngine/Flow/LegacyWeeklyCoverAdapter.swift' 'let support = context\.support' 'legacy cover session does not retain an unused support binding'
Assert-Pattern 'NativeDemoApp/Views/HomeView.swift' 'onChange\(of: activeIndex\) \{ _, newValue in' 'playback strip uses the iOS 17 two-parameter change callback'
Assert-MultilinePattern 'NativeDemoApp/Services/WeatherCompanionService.swift' 'import WeatherKit[\s\S]{0,1800}WeatherService\.shared[\s\S]{0,4200}including: \.current[\s\S]{0,420}converted\(to: \.celsius\)[\s\S]{0,350}legacyWeatherCode' 'weather companion uses WeatherKit current weather and preserves the existing snapshot contract'
Assert-MultilinePattern 'NativeDemoApp/Services/WeatherCompanionService.swift' 'case \.drizzle[\s\S]{0,180}return 61[\s\S]{0,260}case \.blizzard[\s\S]{0,220}return 71' 'WeatherKit conditions preserve the established rain and snow code groups'
Assert-NoPattern 'NativeDemoApp' 'api\.open-meteo\.com|OpenMeteoResponse' 'the iOS client no longer ships the unlicensed Open-Meteo direct integration'
Assert-Pattern 'NativeDemoApp/NativeDemoApp.entitlements' 'com\.apple\.developer\.weatherkit' 'the app declares the WeatherKit entitlement'
Assert-MultilinePattern 'NativeDemoApp.xcodeproj/project.pbxproj' 'com\.apple\.WeatherKit[\s\S]{0,100}enabled = 1[\s\S]*?CODE_SIGN_ENTITLEMENTS = NativeDemoApp/NativeDemoApp\.entitlements[\s\S]*?CODE_SIGN_ENTITLEMENTS = NativeDemoApp/NativeDemoApp\.entitlements' 'WeatherKit capability and entitlements are wired into Debug and Release'
Assert-MultilinePattern 'NativeDemoApp/Views/SettingsView.swift' 'WeatherKitAttributionView[\s\S]*?legalPageURL[\s\S]{0,1400}WeatherService\.shared\.attribution' 'weather settings show Apple Weather attribution and link the returned legal page'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'WeatherKitConditionBridgeTests|testPrecipitationConditionsKeepExistingRainAndSnowGroups' 'WeatherKit rain snow and ordinary-condition mapping has XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-93|com\.apple\.developer\.weatherkit=true|api\.open-meteo\.com' 'WeatherKit signing attribution fallback and privacy have a device regression matrix'
Assert-MultilinePattern 'NativeDemoApp/Services/LegalConsentStore.swift' 'termsVersion = "1\.0"[\s\S]{0,160}privacyVersion = "1\.0"[\s\S]*?struct LoginPolicyConsentRecord[\s\S]{0,240}acceptedAt: Date' 'login consent stores the exact policy versions and acceptance time'
Assert-MultilinePattern 'NativeDemoApp/ViewModels/SettingsViewModel.swift' 'func sendSMSLoginCode\(\) async \{[\s\S]{0,220}guard legalConsentStore\.hasAcceptedCurrentPolicies[\s\S]{0,650}let phone = loginPhone[\s\S]{0,500}sendSMSCode\(phone: phone\)' 'SMS sending is consent-gated before reading or transmitting the phone number'
Assert-MultilinePattern 'NativeDemoApp/ViewModels/SettingsViewModel.swift' 'func verifySMSLogin\(\) async \{[\s\S]{0,220}guard legalConsentStore\.hasAcceptedCurrentPolicies[\s\S]{0,650}let phone = loginPhone[\s\S]{0,700}loginWithSMS\(phone: phone, code: code\)' 'SMS verification cannot bypass current policy consent'
Assert-Pattern 'NativeDemoApp/Views/SettingsView.swift' 'LoginPolicyConsentRow|hasAcceptedLoginPolicies' 'settings account login uses the shared explicit consent row'
Assert-Pattern 'NativeDemoApp/Views/MemberPricingView.swift' 'LoginPolicyConsentRow|hasAcceptedLoginPolicies' 'member account login uses the shared explicit consent row'
Assert-NoPattern 'NativeDemoApp/Views/SettingsView.swift' '登录即表示.*同意' 'settings login cannot imply consent from logging in'
Assert-NoPattern 'NativeDemoApp/Views/MemberPricingView.swift' '登录即表示.*同意' 'member login cannot imply consent from logging in'
Assert-MultilinePattern 'NativeDemoApp/Views/Components/LoginPolicyConsentRow.swift' 'Button[\s\S]{0,100}isAccepted\.toggle\(\)[\s\S]{0,1000}minHeight: 44[\s\S]{0,500}accessibilityValue\(isAccepted \? "已勾选" : "未勾选"\)' 'shared consent row is explicit tappable and accessible'
$null = [xml](Get-Content -Raw -Path 'NativeDemoApp/Resources/PrivacyInfo.xcprivacy' -Encoding UTF8)
Write-Output 'OK  privacy manifest is well-formed XML'
Assert-MultilinePattern 'NativeDemoApp/Resources/PrivacyInfo.xcprivacy' 'NSPrivacyTracking</key>[\s\S]{0,40}<false/>[\s\S]{0,100}NSPrivacyTrackingDomains</key>[\s\S]{0,40}<array/>' 'privacy manifest disables tracking and declares no tracking domains'
Assert-MultilinePattern 'NativeDemoApp/Resources/PrivacyInfo.xcprivacy' 'NSPrivacyCollectedDataTypePhoneNumber[\s\S]*NSPrivacyCollectedDataTypeUserID[\s\S]*NSPrivacyCollectedDataTypePurchaseHistory[\s\S]*NSPrivacyCollectedDataTypeOtherFinancialInfo[\s\S]*NSPrivacyCollectedDataTypeOtherUserContent[\s\S]*NSPrivacyCollectedDataTypePreciseLocation[\s\S]*NSPrivacyCollectedDataTypeCoarseLocation[\s\S]*NSPrivacyCollectedDataTypeOtherDiagnosticData' 'privacy manifest covers the current collected data categories'
Assert-MultilinePattern 'NativeDemoApp/Resources/PrivacyInfo.xcprivacy' 'NSPrivacyAccessedAPICategoryUserDefaults[\s\S]{0,180}CA92\.1[\s\S]*NSPrivacyAccessedAPICategorySystemBootTime[\s\S]{0,180}35F9\.1' 'privacy manifest declares required reasons for UserDefaults and system uptime'
Assert-MultilinePattern 'NativeDemoApp.xcodeproj/project.pbxproj' 'A10000420000000000000001 /\* Resources \*/[\s\S]{0,260}PrivacyInfo\.xcprivacy in Resources' 'privacy manifest is wired into the app target resources'
Assert-MultilinePattern 'NativeDemoApp.xcodeproj/project.pbxproj' 'A10000410000000000000001 /\* Sources \*/[\s\S]{0,700}LegalConsentStore\.swift in Sources[\s\S]{0,260}LoginPolicyConsentRow\.swift in Sources' 'consent store and shared row are wired into the app target sources'
Assert-NoMultilinePattern 'NativeDemoApp.xcodeproj/project.pbxproj' 'U10000330000000000000001 /\* Resources \*/[\s\S]{0,260}PrivacyInfo\.xcprivacy in Resources' 'privacy manifest is not misplaced in the test target resources'
Assert-NoMultilinePattern 'NativeDemoApp.xcodeproj/project.pbxproj' 'U10000310000000000000001 /\* Sources \*/[\s\S]{0,260}LegalConsentStore\.swift in Sources|U10000310000000000000001 /\* Sources \*/[\s\S]{0,260}LoginPolicyConsentRow\.swift in Sources' 'app consent sources are not duplicated in the test target'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'LegalConsentStoreTests|testNewInstallRequiresExplicitConsentAndAcceptancePersistsVersionAndTime|testPolicyVersionChangeRequiresFreshConsentWithoutDeletingTheOldRecord|testRevokingConsentRemovesTheStoredRecord' 'policy consent persistence version invalidation and revocation have XCTest coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-94|手机号没有出设备|PrivacyInfo\.xcprivacy|CA92\.1|35F9\.1' 'login consent and privacy manifest have a device release matrix'
Assert-Pattern 'APP_STORE_METADATA_zh-Hans.json' '叙账 - 用账单叙述生活|support@xuzhang\.app|yilei wang|NSPrivacyCollectedDataTypeOtherFinancialInfo|NSPrivacyCollectedDataTypeCoarseLocation' 'App Store metadata contains the reviewed identity contact and privacy facts'
...
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'enum DiscoverCardKind|struct DiscoverCard: Identifiable, Codable, Equatable, @unchecked Sendable|struct DiscoverSnapshot: Equatable, @unchecked Sendable' 'Discover uses explicit four-layer card and snapshot models'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'struct DiscoverEvidenceSummary|isFeatured: Bool|evidenceDisplayText' 'Discover cards expose featured presentation and one evidence-count contract'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'enum DiscoverEvidenceResolutionPolicy|static func resolve\([\s\S]{0,240}evidenceIDs' 'Discover detail resolves evidence IDs against the current ledger'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'static func buildDiscoverSnapshot\([\s\S]{0,2600}recentDiscoveries:[\s\S]{0,1800}lifePatterns:[\s\S]{0,1500}sceneAssets:[\s\S]{0,1500}echoes:' 'Discover snapshot builds four evidence-backed sections'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'prepareAggregationContext|preparedAggregateSets|journeyFact: journeyFact' 'Discover clue computation reuses one mark aggregation and one journey fact'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'TraceClueScopePolicy\.items\(from: items|editorialScore|novelty:|confidence:|storyValue:|stableEvidenceIDs|editorialEchoLine' 'Discover cards use the bounded scope and rank novelty confidence and story value with bounded evidence'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' '最近开始出现|又回来了|最近变得更频繁|最近少了一些|暂时没再出现' 'Discover detects first appearance recovery increase decrease and disappearance changes'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'struct TraceClueComputationInput:[\s\S]{0,1500}init\([\s\S]{0,900}scope: TraceClueScope = \.period[\s\S]{0,900}self\.scope = scope' 'Discover clue input exposes an explicit backward-compatible scope initializer'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'compactMap \{ index -> TraceRhythmPoint\? in' 'Discover rhythm compactMap has an explicit optional result type'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'struct TraceRhythmPoint: Identifiable[\s\S]{0,500}init\([\s\S]{0,260}startDate: Date\? = nil[\s\S]{0,260}self\.startDate = startDate' 'Discover rhythm point exposes an explicit start date initializer'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'else if let recentBucket, decreased, previousHasMeaningfulEvidence \{[\s\S]{0,180}let delta = max\(previousCount - recentCount, 1\)[\s\S]{0,520}56 \+ delta \* 7' 'Discover decrease scoring owns its local delta'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceDiscoverEditorialBoard|AI 最近发现|过去的回声|traceDiscoverCard|DiscoverDetailSheetView|照片墙|记录墙|openDiscoverRecord' 'Discover page renders the four editorial sections and opens evidence walls'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceDeepInsightSupportingLines\([\s\S]{0,220}journeyFact|同一天发生的真实记录' 'Discover why-copy cites concrete same-day or journey evidence'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'DiscoverMemoryWallLayoutPolicy|DiscoverMemoryWallRow|case hero|case pair' 'Discover photo wall uses deterministic editorial row compositions'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'photoWallRows[\s\S]{0,500}LazyVStack[\s\S]{0,800}discoverPhotoWallRow|discoverPhotoTileHeight' 'Discover detail photo wall renders lazy varied-height rows instead of a uniform grid'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' 'private var recordDayGroups|discoverRecordDayGroup|discoverRecordTimelineRow' 'Discover detail record wall groups evidence into a chronological timeline'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'DiscoverEditorialPolicyTests|testEmptyDiscoverSnapshotDoesNotInventCards|testRecentDiscoveriesOnlyPublishAChangeWithEvidenceAndStableRanking|testStablePresenceDoesNotBecomeARecentDiscovery|testDiscoverPublishesRecoveryWithCurrentAndHistoricalEvidence|testDiscoverPublishesDecreaseAndDisappearanceFromPriorEvidence|testDiscoverIgnoresRecordsOutsideTheBoundedRollingWindow|testWeekendRoadTripDiscoveryIsFeaturedWithOneEvidenceBreakdown|testDiscoverDetailEvidenceResolutionDropsDeletedRecordsAndKeepsOrder' 'Discover change, abstention, featured journey and stale-detail XCTest coverage'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testDiscoverMemoryWallUsesStableEditorialRhythmInsteadOfAUniformGrid' 'Discover photo wall editorial rhythm has deterministic XCTest coverage'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'DiscoverMemoryWallPhotoPolicy|expandedPhotoLimit = 8|normalizedCoverMemoryImageIndex|DiscoverJourneyHierarchyPolicy|DiscoverEchoEvidencePolicy' 'Discover photo selection journey hierarchy and independent echo policies are explicit'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'enum TraceLifeMarkDetailPolicy|isHighValue\(|isJourneyAsset\(|detailCard\(' 'high-value life assets expose stable detail and journey deduplication policy'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'isJourneyAsset\(|hasJourneyAsset: Bool|canonicalQuestion' 'Journey asset rows remain visible while duplicate explanatory cards are suppressed by identity'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceModels.swift' 'return itemsWithPhotos\.flatMap \{ item -> \[DiscoverMemoryWallPhoto\] in[\s\S]{0,220}guard let coverIndex = item\.normalizedCoverMemoryImageIndex else \{ return \[\] \}' 'Discover expanded photo selection fixes its Swift closure element type'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func discoverPhotoTile\([\s\S]{0,900}memoryImageData\(at: photo\.imageIndex\)[\s\S]{0,300}memoryImageReference\(at: photo\.imageIndex\)' 'Discover photo wall renders the selected image itself instead of a record cover surrogate'
Assert-NoMultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func discoverPhotoTile\([\s\S]{0,1200}onOpenRecord' 'Discover photo tiles stay read-only in the clue detail'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private func discoverRecordTimelineRow[\s\S]{0,220}onOpenRecord\(item\)' 'Discover record wall keeps its record-detail route'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' '核心证据|路线边界记录|标记出发、城市切换与返程|连续线索概览' 'Discover separates journey evidence roles from the rolling overview'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testDiscoverMemoryWallExpandsFewPhotosWithEachCoverFirst|testDiscoverMemoryWallUsesOnePreferredPhotoPerRecordWhenPhotoCountIsLarge|testJourneyHierarchySuppressesOnlyTheDuplicateLegacyNarrative|testDiscoverEchoNeedsCurrentAndHistoricalEvidenceOutsideTheJourney' 'Discover journey hierarchy echo and multi-photo rules have XCTest coverage'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'testHighValueLifeAssetsKeepOneJourneyNarrativeAndAStableDetailEntry' 'high-value asset entry and journey deduplication XCTest coverage'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'private static func journeyDiscoverCard|journey.endDate < recentStart|sceneAssets.removeAll' 'older certified journeys keep one stable scene-asset entry'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'let featuredJourneyID = journeyFacts.first' 'multiple journeys choose one featured lead'
Assert-Pattern 'NativeDemoApp/Views/StatsTraceSnapshotStore.swift' 'for journey in journeyFacts where journey.endDate < recentStart' 'multiple journeys retain older durable assets'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private var traceClueItems: \[HomeItem\][\s\S]{0,700}if let snapshot = visiblePreparedClueSnapshot[\s\S]{0,180}return snapshot.items' 'trace redraw reuses the published clue snapshot instead of rescanning the ledger'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private var traceCategoryClues: \[TraceCategoryClue\][\s\S]{0,260}return snapshot.clues' 'trace redraw reuses prepared category clues'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'private var traceLifeMarks: \[LifeMarkAggregate\][\s\S]{0,260}return snapshot.marks' 'trace redraw reuses prepared life marks'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'let journeyCardID[\s\S]{0,520}snapshot\.discover\.recentDiscoveries\.first[\s\S]{0,520}snapshot\.discover\.sceneAssets\.first' 'journey deduplication checks recent and persistent Discover sections'
Assert-Pattern 'NativeDemoApp/Views/StatsWebView.swift' '只统计这段行程绑定的证据|连续线索窗口的节奏' 'Journey detail and rolling rhythm labels expose their actual evidence scope'
Assert-MultilinePattern 'NativeDemoApp/Views/InsightWebView.swift' 'enum AICommandDailyBarWindowPolicy[\s\S]{0,1800}scopedDates' 'AI daily bar policy scopes matched record dates'
Assert-Pattern 'NativeDemoApp/Views/InsightWebView.swift' 'AICommandDailyBarWindowPolicy\.dates' 'AI daily bars use the matched-date window policy'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'AICommandDailyBarWindowPolicyTests|testWindowFollowsMatchedRecordsInsteadOfQueryEnd|testOlderCertifiedJourneyRemainsAStableSceneAssetAfterRecentWindow' 'older journey and AI daily-window regression coverage'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-101|AI 最近发现|Novelty|Evidence ID|场景资产' 'Discover editorial device regression matrix'
Assert-Pattern 'RELEASE_GATE_AND_DEVICE_MATRIX_v1.md' 'FLOW-103|hero/pair|照片墙|记录墙' 'Discover evidence walls have a dedicated device regression matrix'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceFilters.swift' 'var tracePeriodFilter: some View[\s\S]{0,1500}filterButtonLabel\(currentTracePeriodLabel\)' 'trace custom range filter reuses the committed period label'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'var currentFilterSummary: String \{[\s\S]{0,180}currentTracePeriodLabel' 'trace detail summary reuses the committed period label'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsTraceFilters.swift' 'var traceCustomDatePanel: some View[\s\S]{0,900}traceInlineDatePicker\(title: "开始", selection: customStartDateDraftBinding\)[\s\S]{0,180}traceInlineDatePicker\(title: "结束", selection: customEndDateDraftBinding\)' 'trace inline date controls edit draft endpoints only'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'func applyTraceCustomRangeDraft\(\)[\s\S]{0,2600}LedgerBackgroundComputationLane\.shared[\s\S]{0,80}\.buildTraceDetailList\(input\)[\s\S]{0,1400}TraceDetailListSnapshotPublicationPolicy\.accepts[\s\S]{0,1000}transaction\.disablesAnimations = true[\s\S]{0,260}traceDetailListSnapshot = snapshot[\s\S]{0,180}tabState\.commitCustomRange' 'trace custom range uses the shared background lane and atomically publishes the accepted snapshot without layout animation'
Assert-NoMultilinePattern 'NativeDemoApp/Views/StatsTraceFilters.swift' 'useCustomRange\s*\?\s*"具体时间段"' 'trace custom range cannot regress to a generic applied label'
Assert-NoPattern 'NativeDemoApp/Views/StatsWebView.swift' '"自定义时间"' 'trace detail summary cannot regress to a generic custom-time label'
Assert-Pattern 'NativeDemoAppTests/StateRegressionTests.swift' 'TraceCustomRangePolicyTests|testCustomRangeLabelUsesOneDateForSingleDay|testCustomRangeLabelUsesMonthAndDayForSameYearRange|testCustomRangeLabelIncludesYearsAcrossYearBoundary|testCustomRangeNormalizationOrdersReverseDatesAtDayBoundaries|testCustomRangeDraftEditingDoesNotMutateCommittedRange|testCancellingCustomRangeEditingRestoresCommittedRange|testCommittingCustomRangePublishesBothEndpointsOnce|testCustomRangePublicationRejectsStaleSnapshotKey|testCustomRangePublicationRejectsInvalidatedRequest' 'trace custom range formatting draft commit and stale-publication XCTest coverage'
Assert-Pattern 'NativeDemoApp/ContentView.swift' 'isDarkMode|highlightOpacity|onAccent' 'theme surfaces and primary actions expose dark-mode readability tokens'
Assert-MultilinePattern 'NativeDemoApp/ContentView.swift' 'lifeMarkSceneRewardOverlay[\s\S]{0,5000}foregroundStyle\(AppColors\.onAccent\)[\s\S]{0,1400}AppColors\.stroke\.opacity\(AppColors\.isDarkMode' 'home reward overlay uses a contrast-safe accent foreground and themed rim'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceLifeSliceFallbackColors[\s\S]{0,420}if AppColors\.isDarkMode[\s\S]{0,420}appMixed\(with: tint' 'trace fallback tiles use iOS 17-compatible theme-aware dark palettes'
Assert-NoPattern 'NativeDemoApp/Views/StatsWebView.swift' '\.mix\(with:' 'trace fallback tiles do not call the iOS 18-only Color.mix API'
Assert-MultilinePattern 'NativeDemoApp/Views/StatsWebView.swift' 'traceLifeRangeTab[\s\S]{0,520}foregroundStyle\(isSelected \? AppColors\.onAccent[\s\S]{0,260}fill\(isSelected \? AppColors\.accent : Color\.clear\)' 'trace range selection keeps readable text on the accent state'
...
Assert-Pattern 'NativeDemoApp/Models/InteractionStateModels.swift' 'enum CloudLedgerOwnershipPolicy|case localLedgerBelongsToAnotherAccount|case mergeUnownedLocalLedger|ownerAfterEnablingSync|ownerAfterClearingLocalLedger' 'local ledger ownership is one testable policy'
Assert-Pattern 'NativeDemoApp/Models/InteractionStateModels.swift' 'enum CloudLedgerMergePolicy|struct Tombstone|deletedByRemote|applyingSyncedFields\(from remote' 'cloud merge honors tombstones and preserves local-only fields'
Assert-MultilinePattern 'NativeDemoApp/Models/InteractionStateModels.swift' 'static func applyingSyncedFields\(from remote: HomeItem, onto local: HomeItem\) -> HomeItem \{[\s\S]{0,900}return next' 'remote winner copies only synced fields onto the local record'
Assert-NoMultilinePattern 'NativeDemoApp/Models/InteractionStateModels.swift' 'static func applyingSyncedFields\(from remote: HomeItem, onto local: HomeItem\) -> HomeItem \{[\s\S]{0,900}next\.(memoryImageDatas|memoryImageReferences|coverMemoryImageIndex|memoryAnchorRole|memoryAnchorCaption)\s*=' 'remote winner never overwrites local photos cover or memory anchors'
Assert-Pattern 'NativeDemoApp/Services/LocalStore.swift' 'local_ledger_owner_user_id_v1|loadLocalLedgerOwnerUserId|saveLocalLedgerOwnerUserId' 'local ledger owner persists outside AppSettings so logout keeps it'
Assert-NoMultilinePattern 'NativeDemoApp/ViewModels/SettingsViewModel.swift' 'func logoutCloud\(\) \{[\s\S]{0,900}saveLocalLedgerOwnerUserId' 'logout keeps the local ledger and its owner marker'
Assert-Pattern 'NativeDemoApp/ViewModels/SettingsViewModel.swift' 'hasPendingLoginLedgerOwnershipCheck|func loginLedgerDecision\(localItemCount:|var localLedgerBelongsToAnotherAccount' 'login exposes the ledger ownership decision to the settings view'
Assert-Pattern 'NativeDemoApp/Views/SettingsView.swift' 'loginLedgerDecision\(localItemCount: homeViewModel\.items\.count\)|case \.mergeUnownedLocalLedger, \.localLedgerBelongsToAnotherAccount|这台设备上的记录属于另一个账号|原账号的云端记录不受影响' 'switching accounts explains whose records the local ledger belongs to before merging'
Assert-MultilinePattern 'NativeDemoApp/Views/SettingsView.swift' 'private func requestCloudSyncChange\(_ enabled: Bool\) \{[\s\S]{0,1200}localLedgerBelongsToAnotherAccount[\s\S]{0,200}showLoginCloudSyncMergeConfirm = true' 'manually enabling backup on a foreign local ledger still asks about ownership'
Assert-Pattern 'NativeDemoApp/Services/LedgerSyncService.swift' 'struct LedgerCloudSnapshot|func fetchSnapshot\(\)|LedgerTombstoneDTO|let tombstones: \[LedgerTombstoneDTO\]\?' 'cloud fetch carries tombstones alongside items'
Assert-MultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'func syncCloudLedgerNow\(\) async \{[\s\S]{0,600}guard !isSyncingCloudLedger else \{ return \}[\s\S]{0,1200}CloudLedgerMergePolicy\.merge\([\s\S]{0,700}for item in mergeResult\.uploads' 'full sync is re-entrancy guarded and uploads only local-newer records'
Assert-NoMultilinePattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'func syncCloudLedgerNow\(\) async \{[\s\S]{0,2600}for item in merged \{' 'full sync no longer re-uploads every merged record'
Assert-Pattern 'NativeDemoApp/ViewModels/HomeViewModel.swift' 'private func markLocalLedgerOwner\(|markLocalLedgerOwner\(context\.userId\)' 'every successful upload records the local ledger owner'
Assert-Pattern 'NativeDemoApp/Services/AuthService.swift' 'enum IAPRestoreFailureCopy|accountMismatchCodes|TRANSACTION_ALREADY_BOUND' 'restore purchases surfaces the real server rejection reason'
Assert-Pattern 'NativeDemoApp/Views/MemberPricingView.swift' 'firstVerifyFailure|IAPRestoreFailureCopy\.message\(for: firstVerifyFailure\)' 'restore loop keeps the first verify error instead of silently continuing'
