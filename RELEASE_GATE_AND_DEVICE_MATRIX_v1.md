...
| FLOW-30 | 开启首页宠物，覆盖默认/深色/高对比主题、待机、连续点击、保存后消息、强提示遮挡、切 Tab、锁屏/后台、低电量、Reduce Motion 和 VoiceOver | 首页停留 60 秒观察待机；连续开关气泡 20 次并在消息返回前后切 Tab；触发第一笔/照片/奖励/会员提示；切后台再返回；分别开启低电量、Reduce Motion 和 VoiceOver | 首页不出现系统 emoji；待机、点击和说话三组像素帧清晰且不染主题色，承载底板/气泡继续适配主题；新点击优先且每次只播放一组回应，气泡关闭后回待机，重新显示组件不重播旧点击；强提示时宠物按原优先级让位；后台、低电量和 Reduce Motion 显示稳定帧且无持续任务，返回前台可恢复；VoiceOver 只朗读“宠物助手”及操作提示，不逐帧朗读；60 秒无持续首页重算、明显 hitch、任务堆叠或内存增长 | `NOT_RUN` |
